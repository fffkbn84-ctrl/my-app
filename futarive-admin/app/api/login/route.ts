// futarive-admin/app/api/login/route.ts
//
// admin ログインをサーバー側で仲介し、ブルートフォース対策（10回失敗で30分ロック）を行う。
// login_lockouts テーブル（email をキーに failed_attempts/locked_until を持つ）を
// service_role で読み書きする。実際のサインインは cookie 連携のため
// lib/supabase/server.ts の anon キークライアントで行う。
//
// 必要な環境変数:
//   - NEXT_PUBLIC_SUPABASE_URL
//   - NEXT_PUBLIC_SUPABASE_ANON_KEY（既存）
//   - SUPABASE_SERVICE_ROLE_KEY（既存・webhooks/billing-disputed と共通）
...
export async function POST(req: NextRequest) {
  ...
  // 3. admin ロール確認（カウンセラー/相談所オーナーのアカウントを弾く）
  ...
  if ((adminRow as { role?: string } | null)?.role !== 'admin') {
    ...
    return NextResponse.json(
      { error: 'この画面は運営専用です。管理者アカウントでログインしてください。' },
      { status: 403 }
    )
  }
