...
| **金 18:00** | ことさん | リール | シェア数・フォロー数 | `series/kotosan-reel.md` |
...
#### 🔴 次の実作業：9/22（火）連載第2週をリールで

- [ ] **ふうかさんへ：9/19 のリール用に作った「少し離れたスマホ2台」の 1:1 PNG をチャットに貼る**
      （作業ディレクトリは毎回リセットされるので Claude 側には残っていない）
- [ ] 5枚目の固定画像（カードの束）も同様に貼る
- [ ] 3〜5カットに組み直して MP4 で書き出す（`kinda-pair-28.md`「形式」の表）
...
- [ ] **リール3本/週が回るか。** `ig-strategy` §10 の「30%超えるまで週1本」を意図的に破っている。
      消耗したら土曜を隔週に
- [ ] **ことさんのテスト判定日**。火曜が連載で埋まったため金曜の週1本にし、**判定を 11/6 にずらした**
      （設定書§13 は 10/16。要ふうか確認）
