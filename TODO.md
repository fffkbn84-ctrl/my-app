...
### 🟢 2026-09-19 IG の週3枠すべてに型がついた（main マージ済み）

> **正は `docs/sns/series/` の4ファイル。**毎回ゼロから考えない。
> Claude は該当曜日の型を読み、**その回の `【SCENE】` だけ新しく考えて出す**（ふうかさんに考えさせない）。

| 曜日 | 型 | 正 | 1枚目/1秒目 | 生成枚数 | 合否指標 |
|---|---|---|---|---|---|
| 火 20:00 | ふたりの話題（連載28週） | `series/kinda-pair-28.md`「画像の型」 | 質問・62px | 0〜1枚 | 保存数 |
| 木 20:00 | つくる日記 | `series/tsukuru-nikki.md` | 作り手の判断・54px | 1枚 | プロフィール遷移 |
| 土 12:00 | 入口リール | `series/nyuguchi-reel.md` | `topics.ts` の `ask`・58px | 3枚 | 3秒視聴継続率 |
| 不定 | 言いにくい気持ち（外枠） | `series/iinikui-kimochi.md` | 1枚1文・54px | 全枚 | 保存数・シェア数 |

スクリプトは `docs/sns/tools/ig-carousel/`（準備手順は同ディレクトリの README）。

#### 🔴 9/22（火）の作業：ふうかさんに「B画像」を貼ってもらう

**9/19 の土曜リールで作った3枚のうち、B（少し離れたクレイのスマホ2台）を
そのまま 9/22 連載第2週の1枚目に使う。生成は0枚でよい。**

- [ ] **ふうかさんへ：9/19 のリール用に ChatGPT で作った「少し離れたスマホ2台」の 1:1 PNG をチャットに貼る**
      （セッションの作業ディレクトリは使い捨てなので、Claude 側には残っていない）
- [ ] 5枚目は 28週使い回す固定画像（カードの束）。これも同様に手元の PNG を貼る
- [ ] 土曜リールと火曜カルーセルは**毎週同じ話題**なので、この使い回しは今後も毎週成立する

> **生成は必ず 1:1 で。** スクリプトがモチーフだけ切り出して 4:5 と 9:16 の両方に置き直すため。

#### 今週の決定（破らない）

- **最終枚に一人称を置かない。** Kinda への接続はキャプションに逃がす（`iinikui-kimochi.md` §2-b）
- **層名を1つだけ大きく置かない。** 4つ（ふれる・知る・重なる・描く）を毎回そろえて並べる。
  注釈（「話にふれる、という意味です」）も書かない（`kinda-pair-28.md`「4枚目の書き方」）
- **「いちばん外側の」「いちばん深い」と書かない**（層は進捗ではなく深さ）
- **`#ひとり開発` `#個人開発` は使わない**（`ig-strategy-2026-09.md` §6）
- **つくる日記に CTA を置かない**（役割は1つ＝プロフィール遷移）
- **リールは無音で投稿しない／カバーは1カット目を手動指定する**

#### 数字の入力（投稿から24〜48時間後）

- [ ] 9/19 リール → Notion `振り返りメモ` に **3秒視聴継続率**（W27 は12〜20%。9/12 は公開40分で100%）
- [ ] 9/17 つくる日記 → **プロフィール遷移数**
- [ ] 9/15 連載第1週 → **保存数**
- [ ] 9/14 言いにくい気持ち → **保存数とシェア数**（型検証の合否。この型だけ指標が違う）
- [ ] 週次で**フォロワー増減**を `docs/sns/metrics/2026-09.csv` に（IG戦略の主指標）

#### 数字が出たら判断すること

- [ ] **`#サービスづくり` を残すか。** `#ひとり開発` と同じ理屈（読者が作り手側）が当てはまる。
      プロフィール遷移の数字を見て、外して婚活側5枠にするか決める
- [ ] **「言いにくい気持ち ２」をやるか。** 保存数とシェア数で判断（人に送りたくなるかがこの型の合否）
- [ ] **リールの1枚目に絵を置き続けるか。** 3秒視聴継続率が伸びなければ、文字だけの1秒目を試す
