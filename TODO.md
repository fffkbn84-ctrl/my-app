...
- [ ] **予約枠（`slots`）を実際に入れて、予約が最後まで通ることを確認する**。現状は未来の空き枠が全体で0件のため、サイトを見た相手が予約導線を試せない。小山楓華（＝ふうか本人）の枠を counselor 管理画面から投入すれば足りる。手順は下の「予約枠の入れ方」参照。
- [ ] **小山楓華の未公開口コミ3件の公開可否を判断する**（admin → 口コミ）。公開してよければ「公開済み口コミ0件」の状態が解消する。
- [ ] **`hello@kinda.jp` の受信を確認する**（下の 2026-07-10 ブロックにも記載）。営業メールの返信先が受け取れないと致命的。
...
- 足りていないのは「本人のプロフィール」ではなく「**未来の空き日時**」だけ。DB 上の最後の枠は 2026-06-27 開始で、それ以降が入っていない。
- 目安：直近2〜3週間に数コマ入れておけば、相手が触ったときに予約導線が最後まで動く。
