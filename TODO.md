...
- [ ] **木曜の1本目「夜の窓」＝ 9/24(木) 20:00（無理なら 10/1）。** v1（ものだけ）は綺麗に出たが**投稿しない**。
...
- [ ] **木曜1枚リールの次の候補：間取り「同じ間取り、4つの暮らし」**（`ig-strategy` §15 ①-2）。夜の窓の数字を見てから作る。**v3 で画像が決まり、MP4 の試作まで済み**（`docs/sns/packs/ig-reel-madori-trial.md` §7）。投稿日とキャプションは夜の窓の数字を見てからふうかさんが生成 → Claude が 9:16・文字入れ・7秒 MP4。
