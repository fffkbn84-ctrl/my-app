...
| **C：法務・契約** | **1 週** | **利用規約・特商法表記の本番値差替・規約レビュー** | **🟢 1 周目完了（2026-05-23）／実値差替 + 弁護士レビューが残** |
...
1. **Phase C 残作業**（実値が固まり次第）
   - 法人設立 or 屋号確定後、プレースホルダ実値差替を 1 コミットで実施
     - terms / privacy / tokushou の `[会社名]` `[所在地]` `[代表者氏名]` `[電話番号]` `[氏名]`
     - サポートメール `hello@kinda-futari.app` の確定（Phase D ドメイン取得と連動）
   - 顧問弁護士レビュー（任意・営業前リスク低減のため推奨）
   - アカウント登録 UI に **「マーケティングメールを受け取る」チェックボックス（デフォルト OFF）** を追加（terms 第11条・privacy §2 で根拠は明示済）
2. **営業準備**（Phase C 1 周目が main マージされたら着手）
   - 営業資料・台本・メーリングリスト（特定電子メール法対応は terms / privacy で根拠済）
3. Phase D の Resend 契約・ドメイン取得は営業準備と並走可能
