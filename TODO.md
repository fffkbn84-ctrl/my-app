...
- [ ] **リール3本/週が回るか。** `ig-strategy` §10 の「30%超えるまで週1本」を意図的に破っている。
      消耗したら土曜を隔週に
- [ ] **`render-kotosan.js` を作る（次セッションの実作業・9/21 ふうか合意）。**
      吹き出しを焼き込むスクリプトが無く、**9/23(水) の第1回が作れない**。
      版面は `series/kotosan-reel.md` §3（表＝白48px／本音＝薄茶の透け44px／ごくん＝88px／救い＝44px、
      y=1500 より下に置かない）。`render-reel.js` を下敷きにする。
      出力は 1080×1920 の `f1.png`〜`f5.png`、そのまま `build_reel.py` に渡す
      （**`build_reel.py` の `SEGS` は連載用の 3.5/4.0/4.5/4.5/3.0・XF=0.4 なので、
      ことさんの 3.5/3.5/3.5/2.5/3.0・XF=0.35・3→4 だけ 0.15 に合わせて引数化が要る**）
