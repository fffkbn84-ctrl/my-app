...
> サンプル9件のうち、架空の評価が付いていた4件は 2026-09-19 に非公開にした
> （`is_demo = true and review_count > 0`）。評価0の5件はまだ公開中。
...
- [ ] **席タイプのクレイ画像の残り**（テラス／個室／カウンター1列／円卓 ほか）。
      作成済みは4種類：`booth-semi-private`（カーテンで閉じる半個室）／
      `counter-window`（窓向き横並び）／`booth-high-back`（背もたれの高い対面ボックス）／
      `booth-window-green`（窓に面した対面ソファ）。
...
- [ ] **`price_guides` の「食事」の意味が店によって揃っていない。** RIE COFFEE だけ
      「食事もとると 〜2,000円」＝**ドリンク込みの合計**で入れた（飲み物なしで食事だけ
      頼む人は実際にいないため、単価だと誰も払わない額になる）。既存4店の `食事1人` は
      メニュー上の単価。どちらに寄せるか決めて揃える。推測で4店を書き換えるのは避けた
- [ ] **RIE COFFEE 用の席タイプ画像がない。** 既存4種類はすべて仕切り付きで、
      この店（壁沿いベンチ＋通路側いす・仕切りなし・横一列）に合うものがない。
      `photo_url` は未設定のまま
