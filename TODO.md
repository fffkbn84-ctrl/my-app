...
> **main にマージ済み・本番反映済み**（PR #39）。実装メモは WORKLOG 2026-09-09。
> Kinda act に載った初の実在店＝**PISOLA 横浜睦町店**（ふたり検証済み）。
> ランデブーラウンジ（帝国ホテル 東京）も実在店だが、ふたり検証はまだ。
...
- [ ] **席タイプのクレイ画像の残り5〜7種類**（カウンター横並び／窓際2人がけ／
      ソファ対面／テラス／個室）。プロンプトは
      `docs/guides/miniature-image-guide.md` の共通テンプレートの `【SCENE】` 差し替え。
      生成 → WebP → `public/images/seats/<席タイプ>.webp` → `shops.photo_url`
...
- [ ] Notion 取材ログの `公開状態` を PISOLA だけ「候補」のまま。掲載済みに合わせるか
      判断がいる（`取材済み` は語の意味が変わったので選択肢名の見直しも要検討）
