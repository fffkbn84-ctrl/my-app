...
2. **オーナー初期セットアップ運用** — admin から「相談所＋オーナー招待」モーダル (PR #16) で運用開始可能。次の補強 UI：
   - agencies.owner_user_id を admin から手動セットする UI（現状は Supabase Dashboard 直接）
   - counselor 側でオーナーが配下カウンセラーを招待する UI
