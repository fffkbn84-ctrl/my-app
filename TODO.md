...
### 🟢 2026-09-25〜26 story 2本・交際終了コラム・SNS パック・X を10月から1日2〜3本に

- 本番公開済み：story `masashi-30s-owakare-koukai` / `asami-40s-mama-mo-shiawase`、コラム `kousai-shuryo-koukai`
- **IG 9/26(土) 12:00「言いにくい気持ち」は題材決定・制作中**（Notion に文字3カットと `【SCENE】` 入り。正は `docs/sns/packs/2026-09-25-kousai-shuryo-koukai.md` §1）。
  ふうかさんが ChatGPT で3枚生成（同じスレッド・1:1）→ Claude が `iinikui-kimochi.md` §3-b で書き出し
- **X は10月から1日2〜3本**（2026-09-26 ふうか決定）。朝・昼・夜の3枠に分散、同じ日に同じ話題を置かない
  - [x] 10/1〜10/7 の17本を Notion「X投稿カレンダー」に下書き済（`docs/sns/packs/2026-10-w1-x.md`）
  - [ ] **10/8〜10/14 を作る**（反応を見てから。10/11(日) は連載4週ごとの「今月の4つ」まとめ、10/13(火) は連載第5週）
  - [ ] note 記事案（`2026-09-25-kousai-shuryo-koukai.md` §3）の本文を書くか、ふうかさんが決める
...
- [x] **9/25(金) #2 を書き出した**（12.15秒・MP4 納品済み）。新規は場面 `scene-friday-v1` と
      カット2 `glare-turn-v1` の2枚（#1 と3枚同じになるのを避けた）。カット3・4 は #1 と同じ
- [ ] **#4（10/2 社内）までに、デスクのカット3・4 の新ポーズを足す**（#1・#2 で lookdown-tilt／puff-side-v2 を2回使った）。
      候補は `kotosan-poses.md` ⑤ back-window・⑨ puff-badge
- [ ] **ハイライト「気持ちの整理」を左端に動かすか決める。** その枠にストーリーを1枚足すだけ。
      ことさんで来た人が最初に見る棚が Kinda note になる。**「Kindaとは」が押し下がる**（`ig-strategy` §14）
- [x] bio に Kinda note を入れた（4行目「登録なしで、いまの気持ちを天気にできます↓」・2026-09-23 ふうか反映）
...
- [ ] **木曜の1本目「夜の窓」＝ 9/24(木) 20:00 に投稿（確定）。** 曲はふうかさんが選んで報告 → Claude がキット §9 と Notion のメモに曲名を書く。**10/1 に数字を見る。** v1（ものだけ）は綺麗に出たが**投稿しない**。
      v2（顔のない人形）も不採用。**夜・逆光の v3 を採用、MP4 まで書き出し済み**（キット §8）。**9/24 20:00 に投稿**して、送信と「○番」のコメントを見る。曲は**のほほんではない**（ことさん専用）。キット §9 の基準で選び、木曜の曲として固定する。
- [ ] **木曜1枚リールの次の候補：間取り「同じ間取り、4つの暮らし」**（`ig-strategy` §15 ①-2）。夜の窓の数字を見てから作る。**v3 で画像が決まり、MP4 の試作まで済み**（`docs/sns/packs/ig-reel-madori-trial.md` §7）。**10/1 に夜の窓の数字を見て、投稿日とキャプションを決める。**
      **別案「人のいない、くわしい間取り」（2LDK・1軒・問い「この家で、あなたの場所は？」）のプロンプトも用意済み・未生成**（同 §8）。v3 と同じ週には出さない。どちらを先にするかも 10/1 に決める。
      **9/24 17:00 までに1枚が決まらなければ 10/1**。ことさんは**今回は隠さない**（§6-2・10/16 判定後に #2 の候補）。設計は `ig-strategy` §15。**天気の地図は不採用**（note の答えを先に渡すため）。
      型のファイル `series/` を作ったら CLAUDE.md §7-b の木曜の行も差し替える
- [ ] **木曜の Kinda note 紹介リール（10/8 案）。画面収録はやめた。** `public/images/w_*.webp` と実際の質問文から組む（`ig-strategy` §15「2026-09-23 夜の決定」）
