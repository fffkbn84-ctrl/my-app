...
CREATE TABLE IF NOT EXISTS billing_events (
  ...
  grace_until       TIMESTAMPTZ NOT NULL,   -- ここまでのキャンセルは voided
  reservation_at    TIMESTAMPTZ NOT NULL,   -- 面談予定時刻（reservations.start_at の copy）

  void_reason       TEXT,                   -- 'user_cancel_within_grace' | 'agency_cancel' | 'admin_resolved_dispute' 等
  confirmed_at      TIMESTAMPTZ,
  voided_at         TIMESTAMPTZ,

  dispute_note      TEXT,                   -- 相談所オーナーの異議理由
...
BEGIN
  -- agency_id は reservations 自体に denormalize されているのでそれを使う。
  -- なければ counselors から解決。
  ...
  -- 既に billing_event が存在する予約は何もしない（再 INSERT は UNIQUE で弾かれる）
...
BEGIN
  IF NEW.status = 'canceled' AND OLD.status IS DISTINCT FROM 'canceled' THEN
    ...
    -- billing_event が無い予約（旧データ等）は何もしない
...
-- 5-1. 相談所オーナー: 自社の billing_event のみ SELECT 可
...
-- 5-2. カウンセラー本人: 自分に紐づく billing_event を SELECT 可
...
-- 5-3. ふたりへ運営（admin_users.role='admin'）: 全件 SELECT / UPDATE 可
-- admin_users テーブルが未作成の環境では 5-3 / 5-4 を後から追加して構わない
...
-- 注: 直接の UPDATE は不要。状態遷移は専用 RPC（submit_billing_dispute / resolve_billing_dispute）経由で行う。
--     5-4 はあくまでバックアップ手段（pg_cron job が止まった場合の手動修復等）として残す。
