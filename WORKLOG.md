...
**`render-kotosan.js` が未実装で、9/23(水) の第1回が作れない。** ここが次セッションの実作業。
`build_reel.py` の `SEGS` が連載用の尺（3.5/4.0/4.5/4.5/3.0・XF=0.4）で固定されているので、
ことさんの尺（3.5/3.5/3.5/2.5/3.0・XF=0.35・3→4 だけ 0.15）に合わせて**引数化も要る**。
