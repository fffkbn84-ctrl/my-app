...
**モバイルのふたりが小さく、後ろ姿で、木のほうが主役になっている。**
夏はふたりが正面・大きく写っていてファーストビューの主役だった。秋は風景写真寄り。
PC はテキストが主役なので今のままで問題ないが、モバイルは画像そのものがファーストビュー。
差し替えるなら `hero-couple-2026aw.webp` を置き換えるだけで済む（`season.ts` は触らない）。
