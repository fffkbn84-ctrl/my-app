...
## 2026-09-10〜13（Claude Code セッション：登録数の週次レポート・コラム1本・SNS連載とIG戦略）

**詳細は `docs/handoff/handoff-2026-09-13.md` に1枚でまとめてある。** ここは要点のみ。

### 完了

- **登録数の可視化**。お知らせメール登録は `notify_signups`、マイページ登録は `auth.users`。
  集計 SQL を `scripts/weekly-metrics.sql` に固定（運営本人とスキャナ由来ドメインを実ユーザーから分離）。
  9/10 時点でメール登録2件、マイページ実ユーザー4名（＋本人1）
- **スキャナ由来アカウントを削除**。9/04 に `bee@…oastify.com`（Burp Collaborator の外部通信検知用ドメイン）で
  登録があった。誰かが自動スキャンを掛けた痕跡。予約・口コミ・お気に入りは0件で実害の形跡なし。
  削除済み（profiles も連鎖削除・孤児レコード0）。auth.users は5件に
- **週次レポートの Routine を作成**（`trig_016vYiAr8pm8nBrYt3GxijRK`・毎週月曜9:00 JST・プッシュ＋メール）
- **コラム1本追加**。`omiai-kaiwa-wadai`（お見合いの会話）。カテゴリ「お見合い準備」が2本しかなく、
  既存37本に「1時間の中身＝何を話すか」を扱った記事が無かった空きを埋めた
- **SNS連載「ふたりの話題、ひとつずつ」を設計**（`docs/sns/series/kinda-pair-28.md`）。
  Kinda pair の28話題を毎週1つ・28週。層はローテーション（浅い順だと「遅れている」と読めるため）。
  第1〜2週分を Notion の X／IG カレンダーへ下書き投入済み（6件・既存行は未変更）
- **Instagram 発信戦略**（`docs/sns/ig-strategy-2026-09.md`）。入口／理由／深さの3層に役割を分け、
  1投稿に2つ以上の役割を持たせない。週3固定。追う数字はフォロワー増減の1つだけ

### 決めたこと

- **Kinda の中身を SNS で具体的に紹介していく**。隠して守れる資産を Kinda は持っておらず
  （UI・28話題は入会すれば見える／口コミ供給・契約・取材の蓄積は公開しても取られない）、
  28話題は `/kinda-pair/topics` で既に全件公開済みのため新規の開示がない。
  出さない線は 送客料の単価と契約条件／営業リストと契約前の相談所名／口コミ認証コードの仕組みの詳細／ユーザーの個票
- **ポエムと観察は違う**。ポエムは書き手の感情を出し、観察は対象を出す。どちらも静かだが
  スキップ率92%だったのは前者。Kinda はコラムでは現象描写を守れていて SNS では情緒に寄っていた
  （ハピハピの森の投稿が毎回同じ文型で、CLAUDE.md §11 と同じ文法だったことから）

### ハマったこと

- Routine 作成時に「Supabase コネクタが保存されていない」警告。週次セッションが Supabase に
  繋がらない可能性がある。数字を捏造せず「使えませんでした」と報告して終わるようプロンプトに入れた。
  **claude.ai の Routines 画面で Supabase コネクタを付ける必要がある**
- この環境は Instagram・TikTok・prtimes 等へのアクセスが遮断されており、参考アカウントの本体は見られなかった

### 判断待ち

- **繰り返し登場する「顔」を作るか。** SNS用でも繰り返し出る存在はブランドの顔になるため
  CLAUDE.md §4 の範囲と判断。保留でも IG 戦略は全部実行できる

### 注意

- PR #41 は tip コミットが `docs/` のみ。main 反映後に production が `CANCELED` でないか確認する（§10）

---

## 2026-09-09（Claude Code セッション：Kinda act に初の実在店を載せ、詳細ページを作り直す）

### 背景

Kinda act 取材ログ（Notion）に PISOLA 横浜睦町店の取材が入ったので、実際にサイトへ
載せるところまでやった。**Kinda act に載った初の実在店**（これまでは営業デモ用の
ダミーのみ）。載せる過程で、実在店を扱うと壊れる箇所が次々に出てきたので順に直した。

### 完了

**お店の新規掲載が本番に出ない問題（根本原因）**
- `/kinda-act` `/shops` `/kinda-glow` `/places/[id]` はいずれも `revalidate` も
  `dynamic` も指定がなく、Supabase クライアントも cookies を使わないため、
  **ビルド時に静的生成されたまま固定**されていた
- shops テーブルに行を足しても再デプロイするまで本番に出てこない。実際 PISOLA が
  出ずに気づいた
- 4ページに `export const revalidate = 300` を付けて ISR にした。掲載・取り下げが
  データ側の操作だけで完結する

**バッジ「取材済み」→「行って確かめた」**
- 「取材済み」は店に許諾を取って店主に話を聞いた含意がある。実際にやっているのは
  ふたりで客として行って席に座ることなので、語のほうが実態より強かった
- enum の値（`certified`）は変えずラベルだけ変更。18箇所。`/about/transparency` の
  バッジ定義文も同時に直した

**営業時間・定休日は Kinda では持たない**
- 店は管理画面を持たないため Kinda 側に更新の手立てがない。古い値を抱えて誤情報の
  責任を負うより、常に最新が出る Google マップへ渡すほうが結果として正確でいられる
- 値が空のとき「お店にお問合せください」で行き止まりだったのを「Googleマップで
  確認する」導線に。注意ツールチップも、なぜ持たないのかを説明する内容に書き換え
- かわりに Kinda が責任を持てる唯一の日付として「Kinda が行った日」
  （`last_reviewed_at`）を出す。行っていない店では既定値 `now()` が入っていて意味を
  持たないため、`certified` のときだけ表示
- 地図の検索語を「店名＋駅からの徒歩案内」から住所ベースに変更（精度が低かった）

**詳細ページに「着く／話せる／なじむ／終われる」の4章**
- 取材ログに集めた観察の7割が画面に出ておらず、詳細ページは説明文と設備タグだけ
  だった。食べログと同じ土俵に立ってしまっていた
- 4軸は X 投稿で既に使っていた Kinda act の言葉。ふたりが店に着いてから別れるまでの
  時系列そのもので、Notion のプロパティが全部きれいに収まる
- 観察は `shops.act_observations`（jsonb）に持つ。項目がまだ育つ段階なので列を
  増やさず 1 本で受け、埋まっている項目だけを描画する。行っていない店では章ごと出ない
- 長文を四角で囲むと読みにくいので、**1行1事実の配列**で持ち、複数あれば箇条書き。
  1行だけならそのまま出す

**席配置の俯瞰図**
- 取材ログの「席配置ラフ」に添付された実写写真から SVG で起こした
- 初版は図の中にラベルを書き込んでいたため、狭い画面で文字が潰れて横スクロールが
  必要だった。**図には番号だけを置き、説明は下の番号付きリストで読ませる**形に変更。
  図はどの幅にも収まり、説明は本文と同じ大きさで読める
- `public/images/act/seating/` に置き、`act_observations.talk.layoutImage` から参照

**席タイプのクレイ画像**
- 店ごとではなく**席のかたちごと**に用意して使い回す。実在店の写真ではないため
  掲載許諾がいらず、店の外観を偽らない
- 半個室ボックス席の1枚目を `public/images/seats/booth-semi-private.webp` に
  （1,976KB の PNG → 114KB の WebP）
- プロンプトは `docs/guides/miniature-image-guide.md` の共通テンプレートの
  `【SCENE】` 差し替えで作れる

**サンプルバッジを出し分け**
- `DemoBadge` をお店カードとリールに**無条件で**出していた。全件がデモだった頃の
  名残で、実在店に付くと誤解を招く
- `shops.is_demo` を足し、true のときだけ出す

**価格をお見合い／デート別に**
- お見合いはドリンク1杯で1時間、デートは食事をして長くいる。同じ店でも金額が
  まるで違うのに、¥ 記号ひとつではどちらの話か分からなかった
- `shops.price_guides`（jsonb）に使い方ごとの目安を持つ。両方のタグが付く店では
  2行並ぶ

**一覧カードの一行の観察**
- カードに店名とエリアしか出ておらず、どんな場所かが伝わらなかった
- `shops.observation_line` に、行って見てきたことを一文だけ。説明文でも宣伝文句でもない

**星は口コミが5件たまってから（お店・カウンセラー・相談所）**
- 口コミが 1〜2 件のうちは平均が振れすぎる。最初の一人が辛口だっただけの店が
  「★2.0」に見え、一人が満点をつけただけの店が「★5.0」になる。掲載したての実在店に
  とっては不利益
- しきい値は `src/lib/reviewDisplay.ts` の `MIN_REVIEWS_FOR_RATING = 5` 1箇所に置き、
  3系統で揃える。5件未満は平均を出さず件数だけ
- カウンセラー詳細の構造化データ（`AggregateRating`）も同じ条件で外した。画面に
  出さない平均を検索結果にだけ出すのは筋が通らないため

### Supabase スキーマ変更（shops）

| 列 | 型 | 用途 |
|---|---|---|
| `act_observations` | jsonb | 着く／話せる／なじむ／終われる の観察記録 |
| `is_demo` | boolean not null default false | 営業デモ用のダミー店か |
| `price_guides` | jsonb | 使い方ごとの価格の目安 |
| `observation_line` | text | 一覧カードに出す一行の観察 |

`last_reviewed_at` は既存列。手で書いていた `src/types/database.ts` にも追記した。

### Notion「Kinda act 取材ログ」の変更

- 追加：`最初の5分` `沈黙の逃げ場` `先に着いたら待つ場所` `店を出たあと`
  `価格帯 食事1人` `一行の観察`
- 削除：`座標`（緯度経度。住所からジオコーディングで出せるうえ、サイト側に使う場所が
  なく全件空だった）
- 記入ガイドのページを新設：「Kinda act 取材ログ 記入ガイド（席配置）」

### ハマったこと・学び

- **NEXT_PUBLIC_ はビルド時に埋め込まれる。** ローカルで実データを見ようと
  `.env.local` を置いても、再ビルドしないと反映されない
- **この開発環境からは Supabase のホストに出られない**（`Host not in allowlist`）。
  ビルド時は mock にフォールバックする。実データの見え方はプレビューでしか確認できない
- Chromium のヘッドレススクリーンショットは `--window-size` を 375 にしても
  ビューポートが 500 未満にならない。狭い画面の確認は `documentElement.scrollWidth`
  との比較で判断した
- Notion のプロパティ説明欄（description）は MCP から編集できない。DDL は
  `ADD/DROP/RENAME/ALTER COLUMN` のみで `DESCRIPTION` 句を受け付けない
...
**コラムカードを活字サムネにした（/columns・トップページ）**
- 天気コラム以外の16本はサムネが無地のグラデーションで、何の記事か分からないまま
  160px の帯を使っていた。記事ごとのグラデーションを地のまま使い、**タイトルを組む**
- カード下のタイトルは出さない（重複するため）。カテゴリのタグも通常カードでは出さない
  （「すべて」モードはセクション見出し、絞り込み時はピルが既に示しているため）。
  カテゴリが混ざる Editor's Pick とトップの3枚にだけ残す
- メタ行に「読む →」を追加。カードUIの定石「タイトル・メタ1行・ビジュアル1つ・
  **アクション1つ**」のうち、欠けていたのがアクションだけだったため
- PC・モバイルとも Chromium（Playwright）で実描画を確認してからマージ

**戸棚の pair 画像を2回差し替えた**
- 初版が暗い → ふうかが照明を灯した版を生成（v2）→ **並べるとまだ暗く緑寄りだったので
  数値調整（v3）**。明度 99.8→121.8 / 赤 113.6→148.4 で他4枚のレンジに収めた
- 同名で差し替えると Vercel の画像最適化キャッシュに旧版が残るため、v2 / v3 と
  ファイル名を変えている

**SEO：サイトマップの lastmod が壊れていたのを修正 ★今回いちばん実害があった**

### 決定
- **カード下に説明文（抜粋）は置かない。** `description` は検索スニペット用に書かれた
  80〜110字で、カード内で2行に切ると文の途中で切れる。抜粋はカード用に書き下ろす
  場合のみ有効、というのが調査した各所の共通見解だったため
- **Kinda note は戸棚に入れない**（2026-08-13 の判断を維持）

### つまずき・学び

**sitemap の `lastmod` を「常に今日」にしていて、Google に無視されていた**
- Search Console のサイトマップ最終読み込みが **2026-07-17 で約1か月止まり**、
  検出 URL も 81 のままだった（実際は 92）
- 原因：`sitemap.ts` が実際の更新日を持たないページに `new Date()`（ビルド時刻）を
  入れており、**92 URL 中 56 URL の lastmod がデプロイのたびに「今日」**になっていた。
  毎回「全ページ今日更新した」と申告している状態
- Google のガイダンスは「正確な lastmod を出せないなら省略する」。信頼できない
  lastmod はサイト全体で無視される
- 対応：実日付を持つコラム（`updatedAt` / `publishedAt`）にだけ lastmod を残し、
  他は付けない。結果 lastmod 保持は36件・「今日」は0件・範囲 2025-02-10〜2026-08-05
- **ふうかがサイトマップを再送信して検出 URL が 81 → 92 に更新されたことを確認済み**
- 教訓：**sitemap は「出せば良い」ではない。嘘の日付を出すと読まれなくなる。**
  今後ページ単位の更新日を持てるようになるまで lastmod は付けない

**canonical が主要3ページに無かった**
- トップ・`/columns`・`/kinda-story` に自己参照 canonical が無かった（記事や
  kinda-pair 系にはあった）。ドメインプロパティ（`sc-domain:kinda.jp`）は www 有無や
  旧 `my-app-rp9u.vercel.app` も同じ土俵で見るため、入口ページほど明示が要る。追加済み

**インデックス登録リクエストの効き方を誤解しやすい**
- 登録リクエストは「優先クロール待ち行列に入れる」だけで、保証も高速化もされない
- **GSC の「ページ」レポート自体が2〜3日遅れる**ため、登録直後に数字が動かないのは正常。
  「登録したのに増えない」は基本的に見るのが早すぎるだけ

**スクリーンショット検証は Chromium が使える**
- `/opt/pw-browsers/chromium-1194/chrome-linux/chrome` を `playwright-core` から
  `executablePath` で叩けば、本番ビルドを起動して実描画を確認できる。
  デザイン変更を目視せずにマージしなくて済む
- ただし `playwright-core` は検証後に必ず `package.json` から戻すこと（毎回入れて毎回戻す）

### 次
- GSC インデックス登録の続き（#6 以降5本）。TODO 冒頭に転記済み
- ページ単位の更新日を持てるようになったら sitemap の lastmod を復活させる
...
### 本番反映（2026-08-13）
- ブランチを **main にマージして本番反映**（マージコミット `cc6f613`）。`origin/main` は先行して4コミット進んでいたが、追加はバッジ画像と `docs/voices/` のみで競合なし
- ⚠️ **ブランチの tip が docs-only コミットだったため、プレビューは CANCELED になっていた**（§10 の落とし穴の実例）。
  main へは `--no-ff` でマージコミットを作り、その差分に `src/` が含まれる状態にして本番ビルドを走らせた。
  **fast-forward だと tip が docs コミットのままになり本番ビルドがスキップされる**ので、この系統のマージは `--no-ff` を既定にする
- トップの戸棚5枚目用に、`docs/guides/miniature-image-guide.md` に**第3弾（pair カードの生成プロンプト）**を追記。
  第2弾の共通テンプレートをそのまま使い、SCENE だけ差し替える方式。色はセージグリーン

### トップの戸棚を5枚にした（2026-08-13）
- ふうか生成のクレイ画像（1254×1254 PNG）を受領 → 560×560 / 42.7KB の WebP に変換して
  `public/images/section-pair-room.webp` に配置（既存4枚は 560×560・35〜45KB。揃えた）
- `DECIDED_CARDS` に pair を追加（act と glow のあいだ）。配色は `[data-section="pair"]` を
  新設し `--section-bg:#E7EEE1 / --section-tint:#CFDCC4 / --section-deep:#6F8A5E`（パステルセージ）。
  story の緑（`#E8F4E4`/`#5A8050`）とは温度を変えて区別する
- `.pc-grid-2to4` → `.pc-grid-2to5` に置換。**モバイル 2+2+1（最下段は `:last-child:nth-child(odd)` で
  列幅のまま中央寄せ）／PC `repeat(5,1fr)`**。升目が片側だけ空く見え方を避けた。
  5枚とも 1:1 を維持したので**画像は正方形1枚で足りる**（横長版は不要）
- `sizes` を `240px` → `200px` に（PC が4列→5列で1マスが小さくなるため）
- 旧 `.pc-grid-2to4` は参照ゼロになったので削除

#### Kinda note を戸棚に入れなかった理由（判断の記録）
ふうかから「note がカードに無い。入れた方がよいのでは」と指摘があったが、**入れない**と判断した。
- note はトップで最も目立つ位置に既に2箇所ある：**ヒーローの主CTA**（「いまの気持ちを整理する」）と、
  **戸棚の直前の専用セクション**「あなたの気持ちはいま、どんな天気？」
- 戸棚まで下りた人は直前で note のセクションを通過しており、入れると1スクロール以内に同じリンクが3回出る
- 戸棚の見出しは「やりたいことが決まっている方へ」。note は**決まっていない人**が使うもの（Phase 0）で役割が逆
- story / コラムも各々トップに専用セクションを持つため、**戸棚に自然に入る6枚目は存在しない**。
  「行き先が決まっている人の選択肢」は type / talk / act / pair / glow の**ちょうど5つ**＝5が正しい数

### 次
- ~~`/kinda-pair` をトップページの戸棚に5枚目として追加~~ → **2026-08-13 完了**（下記）
...
### 完了
- **SNS投稿スタジオ**（Claudeアーティファクト・`docs/sns/sns-post-studio.html`）を構築。まめきちまめこ／可哀想に!／轟ちゃん／サラ／アートゥーン!の構造調査から「クスッと笑えて第三者が傷つかない」6つの型（ふびん・淡々・ひとこと・じぶんツッコミ・ぜんぶ見せる・翻訳）を設計。型選択で完成形下書き自動セット（23案内蔵）・ボツ機能（理由付き・Claudeに渡すコピー）・リール設計図（秒数×カット×テロップ）・ChatGPT画像生成プロンプト内蔵・型別分析（保存率/プロフ遷移率/CSV出力）・Notionカレンダー書式コピー。URL: https://claude.ai/code/artifact/845b9c73-1a9f-4d65-b218-1775fdb299a4（ホーム画面追加運用）
- **リールトレンド調査→設計図を仮説確定**：冒頭1.5〜2秒テキストフック／3秒1カット／全編テロップ／raw感優勢／静止画スライド式。テロップ標準を固定（横書き・太ゴシック・こげ茶#453A2F＋白角丸帯85%・上20%固定・1行13字・フックは0秒表示・救い行のみフェードイン）。タイトル（フック）7つの型も制定（煽り系・カリギュラ型は不使用）
- **事業計画v1をNotionに作成**（運営ダッシュボード直下）。勝ちの定義・ふびん型主力（40/30/20/10）・達成率目標（勝ち投稿は10本中1〜2本が正常）・認知拡大3シナリオ・90日ロードマップ・撤退基準
- **コラム公開**：「結婚相談所の口コミは信用できる？」（kekkon-soudanjo-kuchikomi-no-mikata・PR#32）。列挙段落の番号小見出し化を口コミ/料金/カウンセラー/入会書類/アプリ比較の5本に適用（PR#33）
- **営業資料レビュー**：デッキ・台本・FAQ・リスト戦略・コールドメールは現行返金ルールと整合＝ほぼ完成。FAQ末尾の旧URL（vercel.app→kinda.jp）のみ修正。営業チャネルはフォーム営業主軸＋voices取材オファーDM＋コールドメール補助へ再設計（返信率調査：フォームはメールの数倍）
- **IG/X投稿カレンダー再編**：IGが7/8・7/9・7/11に2本/日の重複→**1日1本ルール**で7/10〜7/20にリスケ（ふびん型週）。Xは7/10以降が空だったため7/10〜7/13の6本を新規作成（7/3勝ち型の再現・新コラム連動）

### 決定
- IGの主力はふびん型ループリール（7〜15秒・クレイふうか像が代わりに凹む）。テロップ・フックは固定ルールで統一し検証変数を減らす
- 生成AI画像は1日5〜8枚枠・画像の動画化は1本1カットまで（ふびん型3〜4枚＝1日1〜2本分）
- フォーム営業の返信先は hello@kinda.jp（Workspaceエイリアス/グループで追加費用なし・Resend設計のReply-Toと一貫）

### つまずき・学び
- スタジオHTMLのX字数カウント関数にNULバイトが混入しファイルがバイナリ判定に→スペースへ置換で解消
- IGカレンダーに前セッション分と新規分が重複し1日2〜3本状態に。**下書き投入時は必ず既存エントリの日付を確認してから**（今回1日1本に整理）
- Notionのdate型プロパティ更新は `date:投稿日:start` の展開キーで行う

### 次
- hello@kinda.jp の受信設定（Workspaceエイリアス＋kinda.jpのMX/SPF/DKIM/DMARC）→フォーム営業第1波へ
- X/IG自動化Phase 2（まずは公式予約投稿=Xネイティブ＋Meta Business Suiteで週1セット運用、その後API自動化を検討）
- voices取材素材（whisper文字起こし）が届き次第 /kinda-voices で記事化
- じぶんツッコミ型R（7/20）は投稿前に実話へ差し替え

## 2026-07-06（Claude Code セッション：週次SNSレビューW27・コラム公開・アフィリエイト相談）

### 完了
- **`/sns-review` で週次SNSレビュー実施**（`docs/sns/reviews/2026-W27.md`、CSVは `docs/sns/metrics/2026-07.csv` に追記）。X/IGの手動アナリティクス貼り付け→柱・スキップ率・3秒視聴継続率まで構造化。
  - **勝ち型を特定**：X 7/3 22時投稿（冒頭に検索ワード「結婚相談所」＋事実+意見＋リプにコラム2本）がインプ37・エンゲージ8・週内唯一のリンククリック2で突出。
  - IGはリーチ100前後届くが3秒視聴継続率12〜20%と冒頭離脱が課題と判明→「冒頭1秒に具体的フック」「CTAをプロフ誘導→保存・コメント誘導」に方針転換。
  - ふうか承認のうえ、Notionの未投稿分（X 7/6夜〜7/11、IG 7/7〜7/9）に上記方針を反映。IG 7/6分の新規リール（雷雨の夜・冒頭フック実装1本目）も動画生成プロンプト・カット割り・キャプション・選曲込みで作成し下書き登録。
- **コラム公開**：「お見合いの服装、初デートまで。清潔感を軸にした男女別の選び方」（`content/columns/omiai-shodate-fukuso.mdx`、slug: omiai-shodate-fukuso）。男女別構成＋清潔感は男女共通の土台という軸（ふうか決裁）。語感修正2件（「失敗を避けることのほうである」表現・清潔感段落に眉/鼻まわりの毛の処理を追記）を反映後、Vercelプレビューでふうか確認→PR #28作成→mainマージ→本番(kinda.jp)反映確認まで実施。
- **IG保存カルーセル下書き1本追加**：「お見合い・初デートの服装ハウツーカルーセル」（7/16予定）。実写コーデ企画の代替として、クレイ世界観維持・LGBTQ+中立語（「相手」）で作成しNotion投入済み。
- **アフィリエイト相談対応**：カフェ/レストラン/ラウンジの予約リンクをアフィリエイト化する方針を壁打ち→バリューコマース経由でホットペッパーグルメ・一休.comレストランを提携するのが妥当と回答。店舗掲載自体（テキスト）は許可不要、写真は自撮り前提の原則を確認。契約者タイプは法人（AGOGLIFE Inc.名義で運用中のドメイン等と揃える）を提案。
- ふうかが2026-07-06にバリューコマース申込（契約者タイプ=法人）。TODO.md・Notion運営カレンダー（7/9枠・種別「ふうか作業」）に審査待ちを記載。

### 決定
- **服装コラムは男女別構成でOK**（LGBTQ+中立を貫くのは「清潔感」の共通土台部分のみ、というふうか決裁）。Kinda voices（取材レポート）ではなく通常コラムとして作成（取材素材がないため）。
- **アフィリエイトは景品表示法上の開示（PR表記等）さえ守れば問題ない**、将来自社予約システムを持つ計画とも矛盾しない（今は需要検証フェーズ、店舗との直接契約は別途必要になるが手間は増えない）と整理。

### つまずき・学び
- **Notion のデータソースへの `query_data_sources`／`query_database_view` はBusinessプラン以上必須でエラー**。現行プランでは使えないため、投稿の柱・投稿日を確認する際は `notion-search` で候補を洗い出してから `notion-fetch` を個別に叩く必要がある（X投稿カレンダー20件超の照合はこの方法で対応）。
- IGの「3秒視聴継続率」はインサイトのグラフから読み取れる指標で、スキップ率だけでは分からない「フックが効いたか」の判定に使える→今後の定点観測項目に追加。

### 次
- バリューコマース審査結果待ち（7/8〜7/9頃）→承認後、ホットペッパーグルメ・一休.comレストランと提携→Kinda act カフェ記事の予約リンクに反映。
- Kinda act 初投稿＋お見合いカフェ記事のKinda voices展開（ふうか進行中）。
- 次回 `/sns-review` で、今回変更した「冒頭フック」「保存・コメントCTA」の効果を前週比で検証。
...
### 完了
- **Kinda story 第1本目「あつみさん」を本番公開**（`/kinda-story/atsumi-20s-mayoi`）。`/story`(MDX) から `/kinda-story`(mock/stories.ts) へ一本化。stage 自動クレイ画像・相談所名非公開配慮（実名をコードに持たせない）・記事内 FAQ・天気内部リンク（花曇り/薄日）・Article+FAQPage JSON-LD。※詳細は下の「2026-06-27 追補」。
- **返金モデルを現行（予約確定で即時課金・原則返金なし・例外は運営の個別判断）に全文書統一**（CLAUDE.md §12 の要同期リストを消化）：
  - 実装指示書（返金フロー表＋サンプルを admin 限定の例外返金に）／送客契約書 agency-agreement 第6条第5項／B2B特商法 tokushoho-agency-b2b 返金表／営業材料（deck/sales-script/faq/deck-brief）。
  - 実コード（charge/refund エンドポイント）は既に現行モデルだった。ズレていたのは文書と死にコードのみ → 未使用の `isWithinBillingGrace`（旧24h猶予判定）を削除。
- **営業資料の課金訴求を再設計**：当初「空振りが起きない（24h猶予前提）」→事実が変わるため「成果報酬・予約成立時のみ」に。さらにふうか方針で「会ってみたい見込み客の**ご紹介**（連絡先のお渡し）に対する成果報酬／連絡先はご縁を本気で進めたい相談所さまにだけお渡しする」という柔らかい表現に統一。**法務書類は厳密な条文のまま据え置き**（営業=柔らかい／法務=厳密の役割分担）。
- **Vercel デプロイの落とし穴を実体験→記録**：main の tip コミットが docs のみだと Ignored Build Step が本番ビルドをスキップ（CANCELED）。CLAUDE.md §10／本 WORKLOG に記録。復旧＝tip に src 変更を置いて再 push（実施して `kinda.jp/kinda-story/atsumi-20s-mayoi` 本番反映を確認）。

### 決定
- **Stripe 本番審査のコード/文書ブロッカーは解消**。残りはふうかの Stripe ダッシュボード作業（KYC・銀行口座・事業説明・本番キー/Webhook を Vercel env へ）。TODO に「保留」セクションとして残置。
- **Google Workspace**：受信 MX は設定済み（受信可）。`@kinda.jp` 送信のために今契約してよい。月額プランは**日割り＋14日無料トライアル**のため「7/1 まで待つ」コスト上のメリットは無く、むしろ Resend 送信元・Stripe 審査の信頼性に早く効く。
- 営業トーンと法務トーンを分離する原則を確立（営業＝関係性重視の柔らかい言い回し／法務＝中立・厳密な条文）。

### 次
- ふうか：Stripe 本番審査のダッシュボード作業（TODO 参照）。Workspace 契約で `@kinda.jp` 送信を有効化。
- （任意）顧問弁護士レビュー後に契約書/特商法を最終化。

### 学び
- 実コードが現行モデルでも、契約書・特商法・営業資料・実装指示書が旧モデルのまま残ると「サイト・契約・実装の不整合」になり、Stripe 審査の実効ブロッカーになる。コードより文書が遅れる典型。文書統一こそが審査準備の本体だった。
- 同じ事実でも、営業の場とコンプライアンス文書では最適な言い回しが違う。実質（即時課金・原則返金なし）を保ったままトーンだけ分けると、矛盾なく両立できる。
...
### 完了
- 「声」を2層に分けるモデルを確定：ブランドの声（穏やか・笑わせない＝天気/ヒーロー/相性チェック/朗読）と、ふうかの声（あたたかい・たまにクスッ＝X中の人/つくる日記/創業note）。前回の慎重論を更新し、ふうかの"クスッと笑える語彙"を人の層の武器として正式採用。
- ユーモアの線を確定：クスッ採用／ガハハ不採用。笑いの的は自分・天気・ひとり制作の大変さはOK、ユーザー/独身/婚活のつらさ/競合はNG。3秒セルフチェック（自分or天気を笑ってるか/煽ってないか/男女前提・Emma・診断語が無いか）。
- Notion「X投稿カレンダー」に柱F「中の人」（茶）を新設。中の人ツイート10本を下書きで投入（6/30〜7/12・昼12時台中心・日曜のみ19時任意枠）。既存A〜Eは1本も置き換えず追加。実機確認で「昼12時台」枠が既存と判明し新設を回避。
- X運用ルール：総量は増やしてよいが1日2〜3本・朝夜3時間以上は維持（Author Diversity）。リンクは本文でなくリプ。

### 決定
- 認知拡大の主レバーは"創業者個人の声"。世界観を深めるのは朗読、広げるのは中の人。コメディ(ガハハ)・製品レコメンドは核を薄めるので人の層でも不採用。
- 柱Fを独立させ、中の人だけで伸びを計測できる設計に。

### 次
- 2週間後に柱Fをレビュー→伸びた型を厚く次を生成。つくる日記へ声を展開。朗読試作。コラム本番URL確認。

### 学び
- 前回の「面白い系は慎重」は"ブランドがふざける"ことへの正しい警戒だったが、"人があたたかく少しおかしい"こととは別物。後者は初期の認知拡大では最強クラスの資産。混同しないこと。
- 実機確認でNotionの「昼12時台」枠が既存と判明し、不要な新設を回避（ドキュメント鵜呑み禁止が効いた例）。
...
### 完了
- Notion「X投稿カレンダー」DBを運営ダッシュボード直下に新設（IGと同構造・table＋calendar）。database_id 2c2b426bf123473894a98c1adcff2947 / collection 5e7ae4e9-3c21-45fc-9837-d125f9d6357e。2週間分（6/28〜7/12）を実例コピー・下書き済で投入。
- X運用6/28開始。6/28 21時のC問いかけ投稿済。6/27のnote開設告知（固定ツイート）を投稿済記録として追加。IGリール（夏×哲学）のクロス告知を作成（本文＋リプにリール個別リンク）。
- 7/10のnote告知行を、固定ツイートと重複のためE廃止→B「入ったら逃げられない？」に差し替え、状態を下書き済に整理。

### 決定
- X運用ルール：1日2〜3本・朝夜分散・リンクは本文でなくリプ・ハッシュタグ原則なし・会話最優先（リプ返信80%）。
- IGリンク：リール個別リンクは取得可（共有→リンクをコピー）。プロフィールリンク限定は誤認。用途で使い分け。
- 声：朗読系（天気の言葉/季節哲学/つくる日記）を採用。面白い系は穏やかさを壊すため慎重・別枠。
- Kinda Voices製品案は現フェーズ見送り。理由＝核衝突（自己最適化 vs 合う人で選ぶ）・「婚活女子」が包摂性と衝突・薬機法/ステマ/景表法・Voices(実体験証言)の器に不適合。やるなら"安心"フレームの別枠で後回し。

### 次
- コラム6本の本番URL確認→リプリンク修正。X日次運用と計測。声の朗読リール試作。

### 学び
- Kindaの差別化＝"焦らせない/合う人で選ぶ"。製品レコメンドや自己最適化系は、短期の話題性と引き換えにこの核を薄める。新規フェーズは核（ニッチのトピカルオーソリティ＋ユーザー側トラクション）に集中するのが正解。
- 声は強力な差別化資産。世界観を"深める"方向（朗読）に使い、"広げる/崩す"方向（コメディ・製品）には使わない。
...
### 完了
- Xの2026年アルゴリズムを一次〜二次情報で調査（GitHub公開 1月＋5月更新）。要点：①評価は数でなく「次の行動の予測」をAIが判定 ②Author Diversity＝連投は自分の投稿同士で食い合う→1日2〜3本・時間分散 ③加点は会話が本体（自リプ+150/リプ+54/プロフ+24/いいね+1）④外部リンクは本文減点→リプに置く ⑤Xはテキスト主役でKindaの天気の言葉が最適。
- X運用戦略 v1 を確定（5本柱・頻度・時間・減点NG）。成果物 kinda-x-strategy-calendar-v1.md。
- Notion「Kinda 運営ダッシュボード」直下に「X投稿カレンダー」DBを新設（IGカレンダーと同構造）。database_id 2c2b426bf123473894a98c1adcff2947 / collection 5e7ae4e9-3c21-45fc-9837-d125f9d6357e。
- 2週間分25投稿（6/28〜7/12）を実例コピー入り・下書き済で投入。カレンダービュー作成。

### 決定
- 「投稿数が多い方が優勢？」への結論＝量より会話設計。新規アカウントは頻度を上げる価値はあるが上限2〜3本/日、薄い量産は逆効果。
- note/コラムのリンクは本文に貼らず必ずリプ（返信）に置く＝Kinda共通の運用ルールに格上げ。
- XはIGと役割分担：IG＝リール（発見）、X＝テキスト短文（天気の言葉）＋会話。

### 次
- ふうか：コラム5本の本番URL確認（リプリンク欄の推定URL修正）／X運用開始／計測。
- 投稿案1（夏×日記カルーセル）画像生成・7/1投稿（前セッションからの継続）。
- Kinda story 量産。

### 学び
- 2026年のXは「いいねを集める」から「読者の次の行動を設計する」へ。Kindaのように煽らず余白を作るトーンは、問いかけ＋丁寧なリプ返しと相性が良く、会話加点を取りやすい。
- 新規アカウントの罠＝「数で殴る」。Author Diversityがあるため逆効果。質の高い1本を時間分散する方が伸びる。

### GA4（前セッション継続タスクの消化）
- **GA4 `notify_signup` キーイベント登録完了**（ふうか操作）。メール登録の計測が有効に。

---

## 2026-06-27（X初投稿・IG投稿スケジュール確定・3シリーズ設計・Notion連携）

### 完了
- X（@kinda_jp）初投稿＋固定ツイート化。案A（noteリンク付き開設告知）。3媒体（note・IG・X）の看板完了。
- IG 3シリーズ構成を確定：①天気リール（週1・柱A）②季節×哲学リール（週1・柱B+C）③つくる日記カルーセル（隔週〜週1・中の人記録）。リール重視に方針転換。
- 投稿案2（夏×哲学リール）制作完了・下書き保存。3カット動画、テキストは最終カットに1行「急がなくていい夏。」のみ。キャプション確定。
- 夏パレット確定：7月はサイトヒーロー連動で青緑基調。秋に再切り替え。
- リール制作ルール確定：テキストは最終カット1行のみ、詳細はキャプション。音はMeta Sound Collection。
- 投稿時間確定：リール20:00〜21:00、カルーセル10:00〜11:00。
- 動画ツール：CapCut＋Canva併用（無料枠）。
- **Notion連携**：「Kinda 運営ダッシュボード」ページとIG投稿カレンダーを作成。投稿スケジュール・制作状態をNotionで一元管理。
- **投稿案1（夏×日記カルーセル）を軽微修正**：今夏のトレンドカラーを使う投稿へシフト。プロンプト・キャプションは確定済み。

### 決定
- 会員エピソード匿名化投稿は今はやらない（Kindaにまだユーザーがいない＝「盛る」ことになる。Story同意フローと矛盾）。
- ユーザー質問回答集はフォロワー50〜100人超えてから開始。
- ストーリーズ毎日は後回し（フォロワーゼロ＝投稿日告知のみで十分）。

### 次
- 投稿案1（夏×日記カルーセル）の画像生成・制作。トレンドカラー適用。7/1投稿予定。
- GA4 `notify_signup` キーイベント化（ふうか操作）。
- Kinda story 投稿原稿（次セッション主要議題）。

### 学び
- IGの新規アカウントでは発見（リール）が最優先。カルーセルは来た人を深めるツールだが、そもそも来る人を増やさないと深めようがない。
- クレイ動画にテキストを入れる場合、最終カットに1行だけが最適解。映像の空気を壊さず、かつ「何のアカウントか」が伝わる。全カットにテキストを入れるとクレイの触感・静けさが台無しになる。
- ChatGPT提案の「恋をしたくなる場所」はKindaの語彙にない。Kindaは「気持ちを整理したくなる場所」「立ち止まれる場所」。恋愛を煽ると婚活アプリと同じ土俵に立つ。
...
### note・SNS
- 創業者note公開：https://note.com/kinda_jp/n/ndd5a4776cc13 （さき×ふうか インタビュー形式・v3本文）。
- X投稿文・bio差し替え文面は用意済み（note URL 焼き込み・未投稿）。bio から Emma 名削除は note・X で実施予定（IG は既にクリーン）。

### 法人識別情報（Code作業の正式記録・6/20実施分のハッシュ確定）
- プライバシー/特商法/利用規約に AGOGLIFE 実値反映済み（株式会社AGOGLIFE／〒104-0061 東京都中央区銀座1-12-4 N&E BLD.6F／代表 船田剛／hello@kinda.jp）。**コミット `3c7cce6`**（`src/app/privacy` `src/app/tokushou` `src/app/terms`）。特商法の電話番号は非表示＝「請求があれば遅滞なく開示」。
- 引き継ぎでは「ふうか×Code が直接実施・未記録」としていた分。本ログでハッシュ確定して正式記録。

### パスワード方針（訂正含む）
- Leaked Password Protection は **Supabase Pro（$25/月）限定**だったため保留。Stripe本番化/Pro移行時にまとめて ON する方針。
- 代替のパスワード強度は **Supabase Auth のダッシュボード設定**（最小長 6→8 ＋ Password requirements、2026-06-20 設定）で担保。※「アプリコードの複雑性ルール」ではなく Auth 設定である点に注意（引き継ぎの表現を訂正）。

### Instagram 戦略 v1
- `kinda-instagram-strategy-v1.md` を作成（IGの正）。骨子：IGは送客エンジンではなく世界観形成役（送客は note+SEO）。看板＝創業ストーリー7枚カルーセル（ピン留め）、エンジン＝クレイ世界観×Reels（柱A 天気の言葉／B 世界観vignette／C 成婚だけがゴールじゃない／D カウンセラー人間味）。DMシェア重視・ハッシュタグ3〜5・キャプションKW SEO。
- **「クレイ」はAI生成のミニチュア世界観のスタイル名（物理オブジェクトではない）**。ストップモーション撮影案は撤回。動画化は「画像→動画AI」パイプラインの問題として次回整理（CLAUDE.md §4 のクレイ風＝DALL-E生成と整合）。

### 動画パイプライン（次回最優先・保留中）
- image-to-video の有料ツール選定が未決（候補：Morphed 等で Veo/Sora/Kling/Hailuo/Seedance/WAN を秒単位課金）。月予算ヒアリング＋ChatGPT画像プラン確認とセットで次回決める。動画方針が決まるまで IG戦略の頻度/フォーマット優先順位は仮置き。
- Canva 無料：引き継ぎは「生涯5本」で却下としたが、**ふうか再調査では『毎月リセット・無料でも商用利用可・AI生成動画 最大200本/月』**との情報。食い違いがあるため次回 .ai で要再検証（AIの事実情報は両者とも誤りうるので一次情報で確認）。
...
### 2026-06-05（追記②：返金モデルを根本変更＝即時課金・原則返金なし・相談ベース）

人間と協議のうえ返金方針を再確定（二転三転を経た現行）:
- **予約確定で即時 ¥5,000 課金**（旧「予約成立＋24時間無料取消」モデルは廃止）。
- 以後の**キャンセル・当日キャンセル・ユーザー不参加（ドタキャン）は原則返金なし**（送客＝予約成立分の事務手数料という整理。客層はKinda責任外）。
- **やむを得ない事情は運営事務局が個別相談で判断**（初手の方針。運用しながら相談所の声で調整）。
- 反映：請求履歴UI(`futarive-counselor billing/page.tsx`)を新文言へ。CLAUDE.md §12 返金ルールを改定。
- **免責**：相談所⇔ユーザー間トラブル（ドタキャンの交通費・店舗代等）への不関与・免責は agency 利用規約 **第14条（免責事項）4号**で既に担保済み（新規追加不要。必要なら明示の一文追加可）。
- ⚠️ 要同期（顧問弁護士レビュー前提・TODO）：`agency-agreement.md`／`tokushoho-agency-b2b.md`／営業デッキ slide6 返金脚注／`docs/implementation/claude-code-stripe-resend-implementation.md`（Stripe実装：24h返金ロジック→即時課金＋例外返金へ）。

### 2026-06-05（セキュリティ点検：Supabase アドバイザ＋RLS 棚卸し）

ユーザー懸念（「誰でも編集できる口コミだった」）を機に全体点検。`get_advisors(security)` ＋ pg_policies 棚卸し。
- **【重大】`auth_all_*`（ALL/true・authenticated）が7テーブルに残存**：`agencies / counselors / shops / slots / columns / episodes / shop_media`。＝ログインユーザーなら誰でも他人の相談所/カウンセラー/お店/予約枠/コラム等を INSERT/UPDATE/DELETE 可能。reviews と同じ過剰権限（reviews は M2 で対処済み）。**最優先で最小権限化が必要**。各テーブルの正当な書き手（counselor/admin アプリ＝anon/authenticated キー運用）を壊さないよう、所有者(owner)/admin スコープのポリシーへ置換する。
- ERROR：Security Definer View ×3（`audit_suspicious_access` / `audit_counselor_activity` / `audit_auth_failures_recent`）。監査ビューが definer 権限で動くため閲覧権限を要制限。
- WARN：SECURITY DEFINER 関数が anon/public からも実行可（多数）。多くは内部で auth.uid() チェックありだが、anon からの実行は revoke するのが安全（defense in depth）。
- WARN：Storage バケット（agency-media/counselor-media/shop-media）が listing 可。列挙防止のため要確認。
- WARN：Auth の Leaked Password Protection 無効 → 有効化（ダッシュボード）。
- WARN：Function Search Path Mutable ×6（既存関数）。`set search_path` 付与で軽微ハードニング。新規の submit_review / auto_complete_reservations は付与済み。

### 2026-06-05（セキュリティ修正：RLS過剰権限 auth_all_* を7テーブルで是正）

`get_advisors` で発覚した `auth_all_*`(ALL/true) を全廃し最小権限へ（本番適用済み・migration harden_core_tables_rls）。
- `is_admin()`（SECURITY DEFINER）を新設し、各ポリシーの admin 判定に使用（admin_users への RLS 依存を回避）。
- agencies / counselors：SELECT=公開 or 所有者 or admin。INSERT/UPDATE/DELETE=所有者(owner_user_id)・所属相談所オーナー・admin に限定。
- shops / shop_media / columns / episodes：SELECT=公開、書き込み=admin のみ。
- slots：SELECT=公開、INSERT/DELETE=枠所有者(カウンセラー/相談所)・admin。**UPDATE は予約フロー（ユーザーが status を更新）が依存するため当面 authenticated 維持**（要・RPC化で将来 owner/admin に絞る＝TODO）。
- 検証：`pg_policies` で ALL/true(authenticated) が 0 件であることを確認。
- ⚠️ 実アプリ（counselor/admin/ユーザー予約）の動作確認は本人ログインが必要なため、ふうかさんに検証依頼中。万一ブロックされる操作があれば該当ポリシーを微調整する。

### 2026-06-06/07（PC対応・マイページ刷新・キャンペーン期限・DNS反映待ち）

ユーザーサイトPCまわりと運用整合の改善（main 反映：merge e6c269a 系）。
- **予約ページPC**：ヘッダーの左右padding復活・本文720pxカラムに中央寄せ・下ナビ被り解消（`booking-wrap` に下クリアランス）。面談方法ラベル「対面（カウンセラーオフィス）」→「対面」（カフェ面談対応）。
- **MyPage PC**：左サイドバー（sticky ナビ）＋サマリーカード（クイックリンク）＋右メイン。モバイルは従来の単一カラム維持。気になる「担当」リールを `auto-fill,minmax(150px)` で縮小（PCで巨大化しないよう）。
- **一覧の表示量**：気になる（担当/相談所/お店）は各コーナー初期6件＋「もっと見る」。予約は「これから」全件／「過去・キャンセル」初期1件＋「もっと見る」。
- **カウンセラーキャンペーン有効期限**：`isCounselorCampaignActive()`（フリーテキスト期限から日付抽出）で期限切れを判定し、一覧バッジ・カウンセラー詳細で非表示に（相談所と同挙動）。**counselor 管理画面のキャンペーン期限入力を `type=date`（カレンダー）＋「期限を解除」に変更**（既存フリーテキストは読込時 `YYYY-MM-DD` 正規化）。`claude/fix-profile-creation-1clpG`（f1b6202）。
- **DNS（kinda.jp）反映待ち**：whatsmydns で NS は一部ノード（Lille/Rawalpindi）が `vercel-dns` を返し始め＝NS変更は有効、伝播途中（日本未反映）。Vercel上はデプロイReady・ドメイン登録済みだが Invalid Configuration のため `https://kinda.jp` はまだ開けない。当面は `my-app-rp9u.vercel.app` を使用。数時間〜48hで切替見込み。

### 2026-06-07（口コミへのカウンセラー返信表示・未マージ）

- カウンセラーの返信(`reviews.agency_reply`)をユーザーサイトの口コミに表示する実装。`getReviewsByCounselor` の select に agency_reply 追加→`reply` で返却、カウンセラー詳細の口コミカードに「〇〇カウンセラーからの返信」ブロックを表示。
- ブランチ **`claude/review-reply-display`**（プレビュー確認OK）。**main 未マージ**（次セッションでマージするだけ）。
- 関連で未対応：ユーザーが投稿時に選ぶタグ（「アドバイスが具体的」等）は現在 DB 未保存（フォームUI上のみ）。表示には DB列追加＋RPC改修＋フォーム＋表示が必要（TODO化）。

### 2026-06-07（続き・口コミ表示の作り直し＋good_tags＋counselor 返信不具合修正）

ユーザーサイト（`origin/main` 起点 `claude/review-display-tags-reply` → main 反映）とカウンセラー管理画面（`claude/fix-profile-creation-1clpG`）の2系統を対応。

**ユーザーサイト（main 反映済み・23d61c9）**
- 返信表示を作り直し：前ブランチ `claude/review-reply-display` は常時インライン表示だったが、要望に合わせ「返信あり」マーク→タップで展開する UX に変更。新規クライアントコンポーネント `src/components/counselors/ReviewReply.tsx`。前ブランチは破棄可。
- 口コミの選択タグ（good_tags）を保存・表示：
  - DB（migration 034・MCP適用済）：`reviews.good_tags text[]` 追加。`submit_review` に `p_good_tags text[]` 引数を追加し insert。旧6引数版は drop して7引数版へ一本化（PostgREST の named-args オーバーロード曖昧回避）。
  - `ReviewForm`：`selectedTags` を `submit_review` に送信（従来フォームUIのみでDB未保存だった）。
  - `getReviewsByCounselor`：`good_tags` / `agency_reply` を取得し `goodTags` / `reply` で返却。
  - カウンセラー詳細：口コミ本文下に good_tags をピル表示。
- `npm run build` green。ff 確認の上 `claude/review-display-tags-reply:main` へ push。

**カウンセラー管理画面（`claude/fix-profile-creation-1clpG` 反映済み・caff281）**
- 返信が何度でもできてしまう不具合の根本原因を特定：`reviews.agency_replied_at` カラムが**存在しなかった**。返信ページ `handleReply` は `update({ agency_reply, agency_replied_at })` を発行するため UPDATE が毎回失敗→`agency_reply` が保存されず「未返信」のまま→ReviewCard が返信フォームを出し続けていた。
  - 修正：migration 0033（MCP適用済）で `agency_replied_at timestamptz` を追加。保存が成立すると ReviewCard が返信フォームを隠し「1回のみ・編集不可」が機能する。client 側の二重送信ガード（事前チェック＋`.is('agency_reply', null)`）と RLS（owner/admin の UPDATE 許可）は既存で問題なし。
- 返信タブに投稿者の**ニックネーム**（`profiles.nickname`・public read 可）と**相談日時**（`reservations.start_at`・counselor owner read 可）を表示。本名（`reservations.user_name`）ではなくニックネームを見せることで、返信時に本名で返してしまう事故を防止。`Review` 型に `reservation_id`/`user_id`/`reviewer_nickname`/`meeting_at` を追加し、reviews ページ load で別クエリ enrich。
- `npm run build` green。

**注意（次セッションへ）**
- DB 変更2件（reviews.good_tags / reviews.agency_replied_at）は本番 Supabase に MCP で直接適用済み。migration ファイルは user-site 系 `supabase/migrations/034_*`、counselor 系 `supabase/migrations/0033_*` に記録（両系統で migrations 履歴が分岐している点に注意）。
- 視覚確認は Vercel デプロイで（my-app-rp9u=main / futarive-counselor=`claude/fix-profile-creation-1clpG`）。good_tags はフォーム再投稿後の新規口コミから表示される（既存口コミは空配列）。

### 2026-06-07（続き2・リール口コミ数の同期／マイページ返信通知の廃止）

ユーザーサイト（`origin/main` 起点 `claude/reel-count-notif-tweak` → main 反映 `f2372c3`）。

- **リールの口コミ数 0 表示を修正**：リール（`CounselorReelModal`）やカードは `counselor.reviewCount`（=`counselors.review_count` カラム）を表示するが、このカラムが口コミ投稿・公開時に更新されず、カウンセラー詳細（実口コミ件数を集計）と乖離していた。実カウンセラー「小山楓華」は実際の公開口コミ 2 件があるのにリールで 0 表示だった。
  - 対応：`reviews` の INSERT/UPDATE/DELETE で対象カウンセラーの `review_count`/`rating_avg` を実公開行から再計算するトリガー `trg_reviews_recalc_stats`（+ `recalc_counselor_review_stats`）を追加し、既存データをバックフィル（migration 035・MCP適用済）。
  - バックフィルは「実口コミ行を 1 件以上持つカウンセラーのみ」更新。実体 0 件でカラムだけ seed されている純表示用サンプル（中村さくら=14 等）は維持した。検証：小山楓華→2/★4.5、田中美咲（サンプル）→3 に補正、中村さくら→14 のまま。
- **マイページの口コミ返信通知を廃止**：`useUserNotifications`（未読ドット）と `NotificationsSection`（一覧）から `reviews.agency_reply` を通知ソースとして除外。正直な低評価への返信が必ずしも納得のいくものとは限らず、ユーザーに能動的に見せる必要が低いとの判断。返信自体はカウンセラー詳細ページの ReviewReply（マーク→タップ展開）で引き続き確認できる。`NotiKind` から `reply` を削除。
- `npm run build` green。ff 確認の上 `:main` へ push。

**ブランチ整理について（次セッションでの手作業が必要）**
- 削除可：`claude/review-reply-display`（今回の good_tags/返信表示で置き換え済）、`claude/review-display-tags-reply`（merged）、`claude/reel-count-notif-tweak`（merged）。
- ⚠️ この実行環境の git リモート（http プロキシ）は**ブランチ削除 push を受け付けない**（`git push origin :branch` / `--delete` が `Everything up-to-date` で no-op になり、send-pack が hung up する）。GitHub MCP にもブランチ削除ツールが無いため、Claude からは削除不能。**GitHub の Web UI から削除**すること。
- ⚠️ `claude/fix-profile-creation-1clpG` は counselor 本番ブランチなので**削除しない**。

## 2026-06-08

### 完了（Claude.ai セッション成果・本リポジトリへは追記反映）
- Kinda Voices 執筆フォーマット v1 作成（記事の出力側の型・SEO特化）。取材キット（入力側）と対。8セクション固定／Atomic Answer／質問形見出し／引用ブロック＝SNS素材／内部リンク密度／Article・Person・FAQPage JSON-LD／MDXスケルトン（提案）／公開前チェックリスト。
- Kinda Story 執筆フォーマット v1 作成。Voicesと対。同意・プライバシー層を最優先で内蔵（項目別同意・匿名化・透明性・撤回フロー）。画像はクレイ情景のみ／構造化データはArticleのみ（Review不可）／成婚を煽らない。
- SNS立ち上げキット v1 作成。2026年トレンド（バズより信頼・一次情報・盛らない）を反映。note＋Instagram主軸／X軽運用／bio・投稿テンプレ・創業note構成・4週間カレンダー・指標。
- ※3キットの HTML 本体は未受領（別途アップ予定）。本日はドキュメント追記（CLAUDE.md 細則／TODO／WORKLOG）と引き継ぎサマリーの保存（`docs/handoff/handoff-summary-2026-06-08.md`）のみ反映。

### 決定
- SNS媒体：note＋Instagram主軸、X軽運用。Threads/TikTok/BeReal後回し。
- 絵文字：SNSキャプションは可／サイト・記事はなし（CLAUDE.md §3 細則化）。
- 顔出しなし。著者アイコン＝クレイふうか像。実名ふうか使用。声は利用可。
- bio はブランドコピー扱い（婚活語なし）／投稿本文・ハッシュタグは婚活語OK。
- belonging構想：Story は公開投稿型にしない（やらせ排除＝信頼の核を守る）。「ひとりじゃない」は Kinda Note の任意・匿名共有で実現する方針に決定（Phase 3）。今は Note のデータモデルを共有前提で設計しておく前倒しのみ。

### 次
- 創業ストーリー録音→Whisper→note記事化（SNSキットPart C構成）。
- Claude Code：voices/story MDXスキーマ確定・JSON-LD・動的OGP・stageクレイ画像マッピング（TODO 2026-06-08 参照）。

### 学び
- Storyは「実在の他人の人生」が題材。同意層を仕組み化しないと信頼とブランドの土台を崩す。
- Kindaのブランド（煽らない・一次情報・盛らない）は2026年SNSトレンドと構造的に一致。作り込みより切り出し配布。
- belonging（「動きたくなる場所」）の器は「投稿機能」ではない。Kindaの声とモデレーション負荷を壊さない最小形＝感情の抽象（天気）の匿名共有。他人の物語のUGCは同意・モデレーション・やらせの三重リスクがあり、Story では持たない。

### 2026-06-08（追補）

- 再アップされた引き継ぎ資料（handoff-summary / repo-md-additions）は前回反映分と内容同一を確認（差分なし）。CLAUDE.md §5（公開投稿型にしない・belonging は Note 側）／TODO belonging Phase 3／WORKLOG は反映済み。
- **創業エピソードの作り方を変更**：当初の「録音→Whisper→記事化」ではなく、**Claude との対話式**で素材を引き出して文章化する方針に決定。
- **SNS 用アイコン**：現在 Claude.ai 側で各種SNS用のアイコン（クレイふうか像ベース）を選定中。確定したら 3 媒体（note / Instagram / X）共通アイコンに適用。

## 2026-06-09（SEO初動 ＋ メール基盤DNS 完了 → メール配線をコードに反映）

> ふうかさん側で GSC 登録・メール基盤DNS（受信ImprovMX / 送信Resend send.kinda.jp）を完了。
> Claude Code 側でメール送信の配線（タスクA/B/C）を実装。指示書＝`docs/implementation/claude-code-email-wiring-2026-06-09.md`、引き継ぎ＝`docs/handoff/handoff-summary-2026-06-09.md`。

### Track 1 — Google Search Console（SEO初動・ふうかさん）
- GSC をドメインプロパティで `kinda.jp` 登録。DNS TXT（google-site-verification）を **Vercel DNS** に追加 → 所有権OK（罠：お名前.comではなくVercel側に入れる）。
- サイトマップ `https://kinda.jp/sitemap.xml` 送信成功（ドメインプロパティはフルURL指定が正）。主要URLのインデックス登録リクエスト（新規ドメインで「検出-未登録」は正常）。
- 裏取り：`robots.txt` 正常、`sitemap.xml` 全URLが kinda.jp ＝ metadataBase は kinda.jp で正しいと実体確定。
- 不整合発見：sitemap に `/mypage` が含まれるのに robots で Disallow → 除外（タスクC で対応）。

### Track 2 — メール基盤DNS（ふうかさん）
- 設計：受信＝ルート(kinda.jp)にMX（ImprovMX）／送信＝send.kinda.jp（Resend）で分離。`_dmarc` 1本に集約。
- 受信 hello@kinda.jp：ImprovMX 無料・`hello@`→`fffkbn84@gmail.com` 転送。Vercel ルートに MX×2＋SPF。受信テストOK。
- 送信 Resend：`send.kinda.jp`（Region 東京 ap-northeast-1）DKIM/SPF/MX/DMARC 緑。MAIL FROM 用は二重 send の `send.send`。

### Claude Code 実装（タスクA/B/C・本日反映）
- **タスクA（admin・`claude/futarive-admin-dashboard-iKBfw`）**：`futarive-admin/lib/email.ts` の送信元を `onboarding@resend.dev` → `Kinda ふたりへ <noreply@send.kinda.jp>` に差し替え、Reply-To `hello@kinda.jp` を付与。From/Reply-To を定数化。
- **タスクB（user-site `main` / counselor `claude/fix-profile-creation-1clpG`）**：admin と同一仕様の `lib/email.ts` を両プロジェクトへ展開。`resend ^4.8.0` を各 package.json に追加。`RESEND_API_KEY` 未設定なら送信スキップでビルド非破壊。両プロジェクト `npm run build` green。
- **タスクC（user-site）**：`src/app/sitemap.ts` から `/mypage` を除外（robots.txt の Disallow と整合・GSC警告解消）。
- 送信元/返信先の決定：From=`Kinda ふたりへ <noreply@send.kinda.jp>`（送信専用・実箱不要）、Reply-To=`hello@kinda.jp`（ImprovMX→Gmail）。DMARC は当面 p=none。

### 次（依存関係）
- **タスクD（ふうかさん手動）**：Resend で API key 発行 → my-app-rp9u / futarive-counselor の Vercel env に `RESEND_API_KEY`（Production）追加 → 再デプロイ。これで user-site/counselor の実送信が開通。
- **タスクE**：口コミ促進メール（completed 契機・運営名義）。B＋D 完了後に実装。文面ドラフトは着手時に用意。
- **タスクF**：取引メール本文（決済/予約確定/連絡先開示/日程変更/返金）は Stripe 実装とセット（今はやらない）。
- OGP実機検証（X/LINE/opengraph.xyz）・GA4 プロパティURLを kinda.jp に（ふうかさん）。

### 学び
- Resend は登録ドメインが send.kinda.jp のため MAIL FROM 用に `send.send`（二重send）になる。Resend表示Name＝Vercel Name（kinda.jp相対）でそのまま入れる。
- GSC ドメインプロパティの所有権 TXT は NS 委任先（Vercel DNS）に入れる。サイトマップはフルURL指定。

### 2026-06-09（追補・お問い合わせフォーム＝送信者識別 B 実装）

- ふうかさんが Task D（Resend API key 発行 → my-app-rp9u / futarive-counselor の Vercel env に `RESEND_API_KEY` 投入 → 再デプロイ）を完了。
  - 受信テスト（fffkbn84@i.softbank.jp → hello@kinda.jp）は ImprovMX→Gmail で着信確認済み（＝受信側OK）。送信（Resend noreply@send.kinda.jp 差出）テストは、下のお問い合わせフォーム経由で兼ねる。
- **お問い合わせフォーム（B・送信者識別）を実装**（user-site・main 反映）：
  - `/contact` を mailto から**アプリ内フォーム**へ（`src/components/contact/ContactForm.tsx`）。
  - `src/app/api/contact/route.ts`：サーバ側でセッション（auth.uid）を読み、**送信者の種別を判定**（相談所オーナー＞カウンセラー＞会員＞未ログイン）。ニックネーム（profiles）・user_id・メールを問い合わせメールに付与し、`hello@kinda.jp` へ `sendEmail`。**Reply-To＝送信者メール**で Gmail からそのまま返信可。
  - ハニーポット＋最小バリデーション。`RESEND_API_KEY` 未設定時は 502 を返し、フォームは hello@kinda.jp 直連絡へフォールバック誘導。
  - これにより「誰から問い合わせか（会員/カウンセラー）」が運営の受信箱で判別できる。
- **送信テストの方法**：本番 `https://kinda.jp/contact` でログイン状態でフォーム送信 → Gmail（hello@kinda.jp）に **From: Kinda ふたりへ <noreply@send.kinda.jp>**・種別/ニックネーム/user_id 付きで届けば、Resend 送信＋識別の両方が開通確認できる。

### 2026-06-09（追補2・口コミ促進メール=タスクE 実装／問い合わせ返信名義の方針）

- **送信テスト成功（B 経由）**：未ログイン＝「未ログイン」、ログイン＝「相談所オーナー」（user_id・ニックネーム付き）で From=noreply@send.kinda.jp で着信確認。→ Resend 送信＋送信者識別が本番で開通確認できた。
  - 補足：種別判定は「メアド一致」ではなく**ログインアカウント（auth.uid）が agencies/counselors の owner_user_id か**で行う（優先順位 相談所オーナー＞カウンセラー＞会員＞未ログイン）。テストでオーナー判定になったのは正しい挙動。
  - 既知の運用課題：問い合わせメールを Gmail から返信すると差出人が個人 gmail になる（ImprovMX 無料は転送のみ・送信SMTPなし）。hello@kinda.jp 名義で返したい → **Google Workspace 検討**（TODO 化）。Reply-To で返信先が送信者になる点は正常。

- **タスクE 実装（口コミ促進メール・運営名義）main 反映**：
  - `src/app/api/cron/send-review-requests/route.ts`：service role で `status='completed'`・`review_request_sent_at IS NULL`・直近14日・`user_email` ありの予約を抽出、`reviews` に投稿済みの予約は除外し、口コミ投稿リンク（`/reviews/new?reservation=<id>`）付きメールを送信。成功で `review_request_sent_at` を記録（migration 036・DB適用済）。From=noreply@send.kinda.jp / Reply-To=hello@kinda.jp。文面は煽らない（CLAUDE.md 準拠）。
  - `vercel.json` に日次 cron（`0 1 * * *` UTC=10:00 JST）。`CRON_SECRET` で Bearer 認証。
  - **要・手動（ふうかさん）**：my-app-rp9u の Vercel env に `SUPABASE_SERVICE_ROLE_KEY` と `CRON_SECRET` を追加 → 再デプロイ。未設定だと cron は 401（送信されない）。
  - 設計判断：既存 lib/email.ts（タスクB）を再利用するため Supabase Edge Function ではなく Vercel Cron + user-site API ルートで実装。service role が必要なため env を追加。

### 2026-06-09（追補3・Kinda note シェアUI統一＋共有深度の調査）

- **Kinda note 結果シェアを共通 ShareBar に統一**（main 3e6b227）。旧実装は単一ボタンが `navigator.share` を直接呼び、X アイコン表示なのに OS のネイティブ共有シートが開く＝「X専用に見えるのに X じゃない／選択肢が X だけに見える」状態だった。`src/components/share/ShareBar.tsx`（X/LINE/リンク/(対応端末のみ)共有）に差し替え、Kinda type/story と統一。weather パラメータ反映後の URL を shareUrl として渡す。OGP 自体は既に正常で、これは画面表示の修正。
- **共有の「深さ」調査（ふうか相談用）**：感情/診断系アプリのシェア設計を調査。結論＝**生の個人データではなく"抽象（天気・象徴・実績）"を共有し、プライバシーは既定オフ**が定石（Co-Star＝チャート比較の抽象社交、Finch＝感情成長を実績に抽象化、Daylio＝美的ムードカード）。Kinda note の SNS シェア文面は既に「今日の私は『天気名』でした #Kindaふたりへ」＋リンクの**抽象レベル**で適正（詳細回答や YOUR WORDS は文面に含めていない）。検討レバー：`includeFreeText`（YOUR WORDS を画像/保存に含める）が現状**既定ON** → 既定OFF（オプトイン）に倒すと"パーソナルすぎ"懸念を構造的に解消できる。belonging Phase3 の「天気の匿名共有」と同じ思想で一貫。

### 2026-06-09（追補4・UX微修正＋SNS引き継ぎ＋ロードマップ整理）

- Kinda note 自由記述（includeFreeText）を**既定OFF（オプトイン）**に（B採用）。過去にONにした人は localStorage で尊重。
- 予約完了/確認の文言を実態に整合：メール未送信（タスクF前）なのに「確認メールをお送りしました」と出ていたのを撤回し、**一休.com 型の「この画面の表示をもって予約成立」**に変更（BookingFlow / AgencyBookingFlow / Step4Confirm）。メール文言はタスクF実装時に再追加する方針。
- favicon を Next/Vercel デフォルト（`src/app/favicon.ico`）削除 → ブランドの日の出 `src/app/icon.svg`（#F5EEE6/#D4A090）に。Google検索の丸窓は再クロールで反映は数日〜数週間。最終クレイアイコン確定後に差し替え可。
- **SNS発信 引き継ぎ**を `docs/sns/sns-handoff-for-claude-ai.md` に作成（媒体方針・ブランド声・婚活語の線引き・絵文字/著者/ビジュアル・素材源・Story/belonging 禁止事項・役割分担・最初の一手）。Claude.ai に渡す用。
- ロードマップ確認：前進2大トラック＝Stripe / SNS。課金前に法務同期＋セキュリティ残を並走、が現在地（TODO 追補2に明記）。

### 2026-06-09（追補5・Bの差し戻し／favicon方針修正／現在地確認）

- **B の取り違えを是正**：A/B 提示時「B＝現状維持（自由記述トグル既定ON）」だったが、前 commit で誤って A（既定OFF）を実装していた。指示どおり**既定ON に差し戻し**（bb19b5e）。
- **favicon**：暫定の日の出 `icon.svg` のまま。最終は**クレイ『Kinda』ロゴ画像**（SNSと統一）にする方針。貼り付け画像はファイルとして読めないため、ロゴPNGのファイルアップロード待ち → 受領後 `src/app/icon.png` / `apple-icon.png` に差し替える。
- **タスクE**：ふうかさんが Vercel env（SUPABASE_SERVICE_ROLE_KEY / CRON_SECRET）を投入済み＋その後のコードデプロイで有効化済み＝**稼働中**。残るは「面談完了→口コミ促進メール」の実地テストのみ（completed 予約での着信確認 or `curl` 手動トリガー）。
- CLAUDE.md：変更なし（憲法レベルの変更なし）。

### 2026-06-09（追補6・favicon=クレイKindaロゴ採用）
- ふうかさんが GitHub Web からロゴPNGを `public/images/` にアップ → `public/images/kinda-icon.png` にリネーム。`layout.tsx` の metadata.icons（icon/shortcut/apple）に設定し SNS アイコンと統一。暫定の日の出 `src/app/icon.svg` は撤去。build green・main 反映（9a7897c）。
- 注：ロゴは 1.1MB（約1242px）。ファビコン用途には大きめだが機能上は問題なし。気になれば将来 256/512px に縮小最適化（このenv に画像変換ツールが無いため今回は未実施）。Google 丸窓は再クロール待ち。

### 2026-06-09（追補7・Stripe 実装 Stage 1：決済バックボーン）

- ふうかさんが Stripe アカウント作成（AGOGLIFE株式会社・法人）→ サンドボックス到達、テストキー取得済み。
- Stripe 実装 Stage 1（テストモード対応・main 2e04613）：
  - **TASK7 スキーマ**：`agencies.stripe_customer_id` ＋ `reservations`（stripe_payment_intent_id / stripe_refund_id / paid_at / refunded_at / user_info_visible）。migration 037・DB適用済（突合で既存0件＝クリーン追加）。
  - `src/lib/stripe.ts`：`STRIPE_SECRET_KEY` 未設定なら null（503ガード・ビルド非破壊）。送客料 ¥5,000 定数。
  - `/api/stripe/charge`：予約成立で相談所カードに即時 ¥5,000 を off_session 課金（予約本人検証・二重課金防止・カード未登録は402）。確定更新は webhook。
  - `/api/stripe/webhook`：`payment_intent.succeeded`→`paid_at`/`user_info_visible=true`（連絡先開示）、`charge.refunded`→`refunded_at`。raw body 署名検証・service role 更新。
  - `/api/stripe/refund`：例外返金（admin 限定）。
  - `stripe ^18`（18.5.0）追加。build green。
  - 設計メモ：reservations.status は active/canceled/completed のままで「confirmed」は作らない。決済成立の真の signal は `paid_at` + `user_info_visible`。RESEND_FROM は `noreply@send.kinda.jp`（指示書の noreply@kinda.jp から読み替え）。

#### 次段（Stripe Stage 2/3）
- Stage 2：**counselor 管理画面にカード登録 UI**（Customers＋SetupIntent＋Stripe Elements）。相談所がテストカード(4242…)を登録 → 最初に動作確認できる単位。
- Stage 3：**予約確定フローに /api/stripe/charge を配線**（予約作成後に課金 → 失敗/カード未登録時の出し分け）＋ webhook で連絡先開示を相談所画面に反映。

#### ふうかさんの手動（準備でき次第）
- my-app-rp9u の Vercel env：`STRIPE_SECRET_KEY=sk_test_…` / `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_…`（テスト）。
- Stripe サンドボックスで Webhook エンドポイント作成：URL `https://kinda.jp/api/stripe/webhook`、イベント `payment_intent.succeeded`・`charge.refunded` → 発行される `whsec_…` を Vercel env `STRIPE_WEBHOOK_SECRET` に設定。
- counselor（futarive-counselor）の Vercel にも `STRIPE_SECRET_KEY` / `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`（Stage 2 のカード登録用）。

### 2026-06-09（追補8・Stripe Stage 2：相談所カード登録UI）

- counselor 本番ブランチ（`claude/fix-profile-creation-1clpG`・4ea7e36）に実装：
  - `lib/stripe.ts`（503ガード）／`/api/stripe/setup-intent`（owner の agency に Stripe Customer 作成→`agencies.stripe_customer_id` 保存→SetupIntent 発行＋既存カードのブランド/末尾4桁返却）／`/api/stripe/set-default-pm`（登録カードを Customer のデフォルトに）。
  - `components/billing/CardRegistration.tsx`：Stripe Elements（PaymentElement）でカード登録/変更。登録済みは「✓登録済み VISA ••••4242」表示。`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` 未設定時は「準備中」表示でビルド非破壊。
  - 請求ページ `/billing` 上部に設置。`stripe` ＋ `@stripe/stripe-js` ＋ `@stripe/react-stripe-js` 追加。build green。
  - RLS 確認：`agencies_update`（owner/admin）でオーナーが自相談所の `stripe_customer_id` を保存可能。
  - 注：counselor app の supabase client は型未指定（untyped）なので新カラムの型キャスト不要。

#### テスト手順（Stage 2）
1. futarive-counselor の Vercel env に `STRIPE_SECRET_KEY=sk_test_…` / `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_…` を設定 → 再デプロイ。
2. 相談所オーナーで counselor 管理画面 → 請求（/billing）→「お支払い方法」でテストカード `4242 4242 4242 4242`／任意の将来日／任意CVC を登録 → 「✓登録済み」表示を確認。
3. これで Stage 3（予約成立→課金）の前提（相談所カード）が満たせる。

### 2026-06-09（追補9・Stripe Stage 3：予約成立→課金配線）

- Stage 2 動作確認OK（相談所がテストカード 4242 を登録 → 「✓登録済み VISA ••••4242」）。
- Stage 3（user-site・main 583bec7）：`Step4Confirm` で `createReservation` 成功後に `/api/stripe/charge` を呼ぶ配線。
  - 課金の成否はユーザーの予約完了を妨げない（カード未登録/失敗は運営フォロー・既存デモ相談所でも壊れない設計）。「カード未登録の相談所は予約不可」の厳格化は将来、相談所一覧/枠表示レベルで実施する方針（今は破壊回避を優先）。
  - 課金成功時：webhook（payment_intent.succeeded）が `reservations.paid_at` / `user_info_visible=true` を更新 → 相談所に連絡先開示。
  - BookingFlow / AgencyBookingFlow とも Step4Confirm 経由なので両導線カバー。build green。

#### end-to-end テストの前提（ふうかさん手動）
- **my-app-rp9u の Vercel env**：`STRIPE_SECRET_KEY=sk_test_…`（charge/webhook に必須）、`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_…`（任意・将来用）。`SUPABASE_SERVICE_ROLE_KEY`/`RESEND_API_KEY` は設定済み。
- **Stripe サンドボックスで Webhook 作成**：URL `https://kinda.jp/api/stripe/webhook`、イベント `payment_intent.succeeded` ＋ `charge.refunded` → 発行 `whsec_…` を Vercel env `STRIPE_WEBHOOK_SECRET` に設定。
- env 投入後、Claude が user-site を再デプロイ（マーカーpush）→ テスト。
- **テスト導線**：カード登録済みの相談所に所属するカウンセラーを、ログインuser で予約 → Stripe ダッシュボード(サンドボックス)に ¥5,000 の支払い → 相談所の請求ページ/予約に反映（paid_at・連絡先開示）。

#### 残（Stripe 周辺の今後）
- 連絡先開示 UI：counselor 予約詳細で `user_info_visible` を尊重して本名/連絡先を出し分け（未確認・要点検）。
- カード未登録相談所の予約不可ガード（一覧/枠レベル）。
- 取引メール（TASK8：予約確定/キャンセル/日程変更）— Stripe webhook 起点で送信（lib/email 流用）。
- 本番有効化前に：特商法(AGOGLIFE実値)・返金ポリシー公開、live キー切替、Webhook を本番モードで再作成。

### 2026-06-14（Stripe 通しテスト成功 ＋ 請求台帳同期）

- **end-to-end テスト成功**（サンドボックス）：カード登録済み相談所(Emma/小山楓華)のカウンセラーを予約 → `/api/stripe/charge` → Stripe で ¥5,000 課金成功（payment_intent.succeeded）→ webhook `https://kinda.jp/api/stripe/webhook` が **200 OK** → DB の `reservations.paid_at` / `user_info_visible=true` / `stripe_payment_intent_id` 更新を確認。決済フロー全段が通った。
- **発見＆修正**：counselor 請求ページは別系統の `billing_events` 台帳を表示しており、Stripe 課金と未連動で「保留中」のままだった。webhook を拡張し、`payment_intent.succeeded`→該当予約の `billing_events` を `confirmed`＋`paid_at`（voided 除く）、`charge.refunded`→`voided` に同期（main 9ea3023）。これで請求ページが Stripe の実課金状態（支払い済み/無効）を反映する。
- 既存のテスト行（webhook 拡張前に pending のまま）は、Stripe の該当 `payment_intent.succeeded` イベントを「再送」すれば新コードで confirmed+paid に更新される。
- 注：テストデータはサンドボックス側で本番と隔離。掃除は任意（残しても本番に影響なし）。

#### Stripe 残（次セッション候補）
- 取引メール（TASK8）：予約確定/キャンセル/日程変更を webhook 起点で送信（lib/email 流用）。
- カード未登録相談所の予約不可ガード（一覧/枠レベル）。
- 連絡先開示 UI：counselor 予約詳細が `user_info_visible` を尊重しているか点検。
- 本番有効化：特商法(AGOGLIFE実値)・返金ポリシー公開 → live キー＋本番 Webhook 再作成。

### 2026-06-14（続き・連絡先の二層開示／GSC重複通知）

- **発見**：counselor 予約詳細(`ReservationDetailBody`)は本名/メール/電話を**支払い状況に関係なく無条件表示**していた（user_info_visible 未参照）＝§12 の「入金後に連絡先開示」が未実装だった。「未払いだと何が見えるか」を確認できなかったのはこのため（実際は全部見えていた）。
- **修正（counselor cf4bce8）**：予約詳細で連絡先（本名/メール/電話）を **user_info_visible=true（Stripe決済成功）時のみ開示**。未決済はニックネーム（profiles.nickname）＋「決済確定後に開示」案内に切替。Stripe 導入(2026-06-14)前の旧予約は grandfather で従来どおり開示。
  - follow-up：inbox(LeadCard)/dashboard の一覧でも user_name(本名)を出しているので、同様にニックネーム化/ゲーティングする（プライバシー整合）。
- **GSC「重複/リダイレクト」通知**：新規ドメインでよくある通知。「ページにリダイレクトがあります」=www→kinda.jp 等の正常な挙動（対応不要）。「重複・正規ページ未選択」=クエリ付きURL(?weather=/?from=/?route=)や canonical の都合。重要ページ(カウンセラー詳細は canonical 設定済)に正規URLがあれば実害なし。今すぐ対応不要、GSC のインデックスレポートで該当URLだけ確認。

### 2026-06-14（続き2・② 一覧の本名ゲーティング ／ ③ 決済確定メール）

- **②（counselor b3820ed）**：inbox・dashboard の一覧でも、未決済（連絡先未開示）の予約は本名を出さず「お客様（決済前）」表示に。共有ヘルパー `lib/disclosure.ts`（`isContactDisclosed`）で詳細ページと判定統一。対象：inbox/page・LeadCard・UpcomingReservationsSection。※一覧での「ニックネーム表示」は今後の磨き込み（今は本名漏れ防止を優先）。
- **③（user-site 99c4777）**：Stripe webhook(payment_intent.succeeded) の**初回確定時のみ**、ユーザーへ「予約確定」メール、相談所へ「新規予約＋連絡先開示」メールを送信（From=noreply@send.kinda.jp / Reply-To=hello@kinda.jp）。送信失敗は webhook を落とさない（.catch）。再送・重複イベントは paid_at 既設で二重送信しない。
  - ③残（follow-up）：キャンセル確定／日程変更 申請・了承 のメールは、各 RPC/更新フロー（user-site・counselor 両方）に hook が必要なため別途。

#### テスト
- ②：カード未登録相談所の予約を作る → inbox/dashboard の一覧で「お客様（決済前）」、予約詳細でもマスク表示。決済済み（小山楓華の例）は本名表示。
- ③：決済が確定する新規予約 → ユーザーのメール（予約確定）と相談所のメール（新規予約・連絡先）が届く。※相談所メールは agencies.email 宛。テスト相談所に email が入っていないと届かない（その場合はユーザー宛のみ）。

### 2026-06-14（まとめ・Stripe一式＋連絡先二層開示＋取引メール（一部）／返金・GSC確認）

- このセッションで Stripe をテストモードで一通り実装し end-to-end 成功（予約→¥5,000課金→webhook 200→reservations.paid_at/user_info_visible→billing_events 確定同期）。詳細は本日の各追補参照。
- **返金の状態**：機能完成。Stripe ダッシュボードで返金すれば charge.refunded webhook で `reservations.refunded_at` ＋ `billing_events=voided` に同期。`/api/stripe/refund`(admin限定) もあり。admin の返金ボタンUIのみ未作成（ダッシュボードで代替可）。
- **連絡先二層開示**：予約詳細＋一覧とも、未決済は本名/連絡先を出さない（決済後に開示）。`futarive-counselor/lib/disclosure.ts` で統一。旧予約は grandfather。
- **取引メール**：決済確定の初回のみ「予約確定(ユーザー)」「新規予約＋連絡先開示(相談所)」を webhook 起点で送信。残（キャンセル/日程変更メール）は次セッション。
- **GSC**：「クロール済み‑インデックス未登録(7)」＋重複1＋リダイレクト1。新規ドメインの正常状態でコード対応不要。待ち or URL検査でリクエスト。
- 全コミット push 済み・未コミットなし。詳細な実装ステータスと残タスクは TODO.md「🆕 2026-06-14 Stripe決済…」セクションに集約。

#### 次セッションの着手ポイント
1. ③残り：キャンセル確定／日程変更（申請・了承）メールを各フローに hook。
2. カード未登録相談所の予約不可ガード（一覧/枠）。
3. 本番化：特商法 AGOGLIFE 実値公開 → Stripe live 有効化 → 本番 Webhook 再作成 → env を live へ。

### 2026-06-27（Kinda story 第1本目：あつみさん公開実装）

#### やったこと
- **Kinda story 第1本目（あつみさん）** の公開インフラ＋原稿を一括実装してプッシュ。
- ブランチ：`claude/todo-worklog-n3od3c`（origin/main 起点）

#### 実装内容
- `content/story/story-atsumi-20s-mayoi.mdx` 追加
  - publishedAt / updatedAt / consentDate はすべて 2026-06-27（ふうか確認済み）
  - frontmatter に FAQ 2問（恋愛経験少ない人の変化 / みんな優しくて選べない迷い）
  - consentAgency=false（相談所名 Emma は非公開）、consentPhoto=false（クレイ情景のみ）
  - 末尾に「この物語に登場する相談所はKinda運営者が関わっています」透明性明記
- `src/lib/mdx-stories.ts`：columns.ts と同構成の MDX 読み込み util 新設
- `src/app/story/page.tsx`：一覧ページ（グリッド、カテゴリバッジ、プルクォート表示）
- `src/app/story/[slug]/page.tsx`：個別ページ
  - Article + FAQPage 構造化データ（Review/Rating/AggregateRating は付けない）
  - canonical: `https://kinda.jp/story/story-atsumi-20s-mayoi`
  - heroImage `/story/clay/marriage.webp` が未配置の場合はグラデーションフォールバック
  - OGP fallback: `/images/OGP-hero.webp`
- `src/components/story/LeadAnswer.tsx`：冒頭リード強調ブロック
- `src/components/story/PullQuote.tsx`：引用ブロック（SNS切り取り用、`--accent` ボーダー）

#### 画像について（TODO）
- heroImage `/story/clay/marriage.webp` → `public/story/clay/` に WebP を配置すれば自動で反映
- ogImage `/story/atsumi-20s-mayoi/og.webp` → `public/story/atsumi-20s-mayoi/` に 1200×630 WebP を配置

#### URL
- 個別ページ: `https://kinda.jp/story/story-atsumi-20s-mayoi`
- 一覧ページ: `https://kinda.jp/story`
- ヘッダーメニューは既存 `/kinda-story`（モック）のまま。将来 MDX ベースに統合を検討。

#### 残作業
- クレイ画像（marriage.webp）と OG 画像（og.webp）を Canva/ChatGPT で生成して配置
- Vercel デプロイ確認後、GSC サイトマップ再送信
- ヘッダーメニューに `/story` を追加するかどうかふうかと確認

---

### 2026-06-27（追補・方針転換）Kinda story を `/kinda-story` に一本化して公開

> 上の `/story`（MDX）実装はプレビューで「ヘッダーの Kinda story に出てこない」問題が判明。
> 原因：ヘッダー/フッターの「Kinda story」は**既存ライブの `/kinda-story`**（`src/lib/mock/stories.ts` のリスト方式）を指しており、新規作成した `/story`(MDX) はどこからもリンクされていなかった。
> 執筆フォーマット仕様（PART C 注記「実態スキーマに合わせよ」/ frontmatter は提案）に従い、**ライブの `/kinda-story` 側に一本化**。

#### 最終的にやったこと
- **`src/lib/mock/stories.ts` に `id="atsumi-20s-mayoi"` を追加**（一覧の先頭＝最新）。これが本番に出る実体。
  - `stage:"成婚"` → 既存クレイ `/images/story-seikon.webp`（夕景のふたカップ）を `getStoryThumbnail` が自動適用。**画像の新規生成は不要だった**（既存3種 seikon/kosai/katsudo を stage で出し分け）。
  - 相談所名 非公開（`consent.agency=false`）→ `counselorId/agencyId=0` で著者カードを非表示。**相談所名(Emma)はコードにも一切持たせない**（mock はクライアント配信されるため）。
  - 透明性の一文を本文末尾に明記。仮名・年代ぼかし（20代/宮城）・実写なし。
  - `faq`（質問形2問）・`relatedWeather`（花曇り/薄日）・`consent`（撤回対応の記録）を任意フィールドで保持。
- **Story 型に任意フィールド追加**：`faq` / `relatedWeather` / `consent`（既存4件は影響なし）。
- **`src/app/kinda-story/[id]/page.tsx`**：
  - 相談所カードが無い物語は「支えた人たち」ブロックごと非表示。
  - Article + FAQPage の JSON-LD を追加（Review/Rating なし）。
  - 「この気持ちに近い天気」内部リンク欄（`relatedWeather` ＋ `/note` 診断入口）。
  - 記事内 FAQ アコーディオン（`FAQAccordion` 流用）。回答は左揃え。
- **MDX 一式を削除**：`/story` ルート・`content/story`・`src/lib/mdx-stories.ts`・`src/components/story/*`（LeadAnswer/PullQuote）。
- 作業ミス：クリーンアップ時に誤って `src/app/columns` を一度削除 → 即 `git checkout` で完全復元・ビルド確認済み（最終成果物に影響なし）。

#### 確定 URL
- 本番（一本化後）: `https://kinda.jp/kinda-story/atsumi-20s-mayoi`
- `/story`（旧 MDX ルート）は**廃止**。

#### 学び（重要）
- **Kinda story の実体は `/kinda-story`（mock/stories.ts のリスト方式）**。今後の投稿はここに 1 オブジェクト追加するのが正。MDX 方式(`/story`)は作らない。
- クレイ画像は **stage から自動**（seikon=成婚/kosai=交際中/katsudo=活動中）。記事ごとの画像生成は不要。
- 相談所名非公開の Story は、mock がクライアント配信される前提で**コードにも実名を残さない**こと。

#### ⚠️ デプロイの落とし穴（今回ハマった・CLAUDE.md §10 にも反映）
- main に FF マージした直後、Vercel 本番デプロイが `CANCELED` になり「あつみさんが本番に出ない」事象が発生。
- **原因**：このプロジェクトは Ignored Build Step で「**tip コミットがドキュメントのみ（`.md`）の push は本番ビルドをスキップ**」する設定（仕様）。今回 main の tip が docs だけの `3d23ffa` だったため、コード（あつみさん `ab23f5b` 等）は main に入っていたのに本番ビルドだけ走らなかった。
