...
## 2026-09-14（Claude Code セッション：期限超過TODOの裁定・計測の一本化・勝ち型の復元）

> 発端は「運営カレンダーで予定のまま日付超過が10件ある。SNSをサボっていたので破棄したい」という相談。
> 調べたら前提が違っていたので、そこから整理した。

### 分かったこと（前提の訂正）

**「発信をサボっていた」は半分だけ正しい。止まっていたのは発信ではなく計測だった。**

Notion の X 投稿カレンダー全97行を見た結果、`投稿済` は 6/27〜7/6 の約25件に加えて
**7/7〜7/18 に約30件、8/17〜8/20 に4件、9/12〜9/14 に3件**あった。IG も同様。
一方で **`計測日` が入っている行は X 97件・IG 25件ともにゼロ**。
9/12 に追加した計測列は一度も使われていなかった。

つまり「新しく計測の仕組みを作るべきか」という問いに対する答えは **作る必要はない**。
仕組みは既にあり、一度もデータが入っていないだけだった。

### 判断

**過去分の計測はほぼ破棄でよい。ただし 9/12〜9/14 の5件だけは別。**

- 7/6 までの分は W27 レビュー済みで、結論（1秒目ルール・保存誘導CTA・ハッシュタグ5枠）は9月戦略に反映済み
- 7/7〜7/18 の約30件は未計測だが、フォロワー0・インプ1桁〜30台では2か月前の週次差は判断材料にならない
- **例外**：9/12 IGリール `l1-holiday`（1秒目を `topics.ts` の `ask` に変えた効果）と
  9/14 IG【型検証】（単色背景・1枚1文＝キーウィ型の検証1本目）は、いま一番知りたい2つの仮説の答えそのもの

### 二段計測ルールを決めた（今回の一番の学び）

「2日しか経っていない数字でいいのか」という問いから、**指標ごとに確定の速さが違う**ことを整理した。

| タイミング | 取る指標 |
|---|---|
| 翌日〜2日後（暫定） | X のインプ・エンゲージ／IG リールの**比率**（3秒視聴継続率・スキップ率） |
| 1週間後（確定） | IG のリーチ・閲覧／**保存数・DMシェア数**／フォロー |

X は大半が最初の24〜48時間に出る。IG リールの**比率**は初日で安定するが、
**リーチはリールタブ配信で数週間伸び続ける**ので絶対値は確定しない。
→ **暫定値で方針を変えてよいのは比率だけ。**

### やったこと

**1. Notion 運営カレンダー：超過10件を裁定**
- 破棄7件（7月時点の設計に紐づく SNS タスク：隔週レビュー×2・`/sns-pack`×2・週次計測×2・月次計測×1）
- 残す3件：GSC手動インデックス＋GA4プロパティURL更新／Stripe本番審査／`/repo-tidy` 月次棚卸し
- GSC/GA4 は SNS より優先度が高い。**GA4 が kinda.jp を向いていないと、SNS を再開しても流入が測れない**

**2. 勝ち型の復元（Notion X投稿カレンダー）**
- W27 で唯一機能したのは「B柱＋冒頭に顕在ワード＋リプにコラム2本」で、週内唯一のリンククリック2はここから出た
- 7月のB柱は全件リプリンク有だったのに、**9/15〜9/30 のB柱7件のうち6件からリプ2本が落ちていた**
- 下書き段階だったので6件すべてにコラム2本を割り当てた（9/16・9/20・9/22・9/24・9/28・9/29）

**3. 計測の二重管理を解消**
- repo の `docs/sns/metrics/*.csv` と Notion の計測列が並立し、`/sns-review` は CSV を読む前提のままだった
- **正を Notion の計測列に一本化**。CSV は 2026-07 で凍結（W27 の根拠としてのみ残す）

**4. 週次レビューを廃止して月次に**
- 週5〜7件・インプ2桁の規模では週次で差が読めず、レビューを書くこと自体が作業になる
- 実際、今回超過した10件のうち「週次SNS計測」が2件積んでいた。**続かない頻度を設定したことが原因**
- 毎朝の Routine が「数字の入力」を催促するので、それが継続装置として機能する

### 変更ファイル

- `docs/sns/metrics-operations.md`：二段計測ルール追加／レビューを月次に／計測データの正を明記
- `.claude/skills/sns-review/SKILL.md`：入力を Notion に／出力を `YYYY-MM.md` に／勝ち型の引き継ぎ確認を追加
- `docs/sns/metrics/README.md`：凍結を冒頭に明記
- `TODO.md`：定常運用セクションを改定／コラムURL確認を済に（運営カレンダーでは 7/3 に完了済みだった）

### 学び

- **「やっていない」と「記録していない」は別物。** 状態を見てから裁定する。今回は発信は続いていた
- **続かない頻度を設定すると、未消化タスクが積んで全体の信用が落ちる。** 頻度は規模に合わせて下げてよい
- **勝ち型は理由ごと引き継がないと落ちる。** 戦略を更新するとき、実証済みの型が新しいカレンダーから
  消えていないかを確認する工程が要る。`/sns-review` の判断基準に明文化した
- ドキュメントを更新したら、**先に投入済みの Notion 行も同時に直す**（9/12 の教訓と同じ構図だった）
