...
## 2026-09-25（Kinda story 2本・コラム1本）

- **story 追加**：`masashi-30s-owakare-koukai`（35歳・東京・活動3ヶ月・自分から交際終了して後悔）／`asami-40s-mama-mo-shiawase`（40代・地方・活動1ヶ月未満・子どもの「ママも幸せになってね」）。
  いずれも全媒体掲載OK・相談所名なし・写真なし（consent 2026-09-25）。ネガティブな感情は前向きに書き換えない
- **story の書き方（ふうかさん指摘で修正）**：名前・年齢・ステージ・期間は詳細ページの見出しに自動で出る。**本文は自己紹介（「35歳です」「〇〇に住んでいます」）で始めず、本人の状況から始める**。読点は原文より増やさない
- **コラム追加**：`kousai-shuryo-koukai`「自分から交際終了を伝えたのに後悔しているとき」（お見合いと交際のこと・主KW「交際終了 後悔」）。
  まさしさんの story を引用・リンク。手続きは既存 `kekkon-soudanjo-fukuen` に任せて気持ちの整理に絞った
- 「Kinda voices」はサイト上 `/columns` 全体の名前で、取材レポートだけではない（最初に取材記事と取り違えた）

## 2026-09-25（ことさん#2 を書き出し）

- **#2「金曜17:55のちょっといい？」を MP4 で納品**（`kotosan4`・12.15秒）。文字は Notion のまま（毒の矛先は職場の理不尽で §5 内・直しなし）
- 新規生成2枚を `docs/sns/assets/kotosan/` に保存：
  - `kotosan-scene-friday-v1`：帰り支度のトート＋閉じたノートPC、背後に書類の束を抱えた顔のない上司。
    #1 で効いた「相手を絵の中に置く」を継続。小物で「帰る直前」「ちょっとで済まない」を文字なしで見せる
  - `kotosan-pose-glare-turn-v1`：椅子ごと振り返る glare。9/22 は正面で出た ① を、**カメラを背後に置く書き方**に直したら出た（`kotosan-poses.md` ① を差し替え）
- **カット2だけ新規にした理由**：TODO は「#1 のポーズをそのまま使える」だったが、2日後に3枚同じだと12秒中約9秒が #1 と同一になり近似重複に当たる
- 既知の小さなズレ：カット1・2 はノートPCが閉じていて、カット3・4（#1 のもの）は開いている。クロスディゾルブで気になるほどではないので採用
- カット4 の「（ごくん）」は天地中央で、puff-side-v2 の顔に一部かかる（#1 と同じ見え方）

---

## 2026-09-23 夜（続き：間取りの別案）

- 予定を確定：**夜の窓は 9/24 20:00 に投稿**（曲はふうかさんが選んで報告 → キット §9 と Notion に記入）。**10/1 に数字を見て、間取り v3 の投稿日とキャプションを決める**
- ふうかさん提案「人を無くして、もっと詳しい間取りにフォーカスした版」に賛成し、**v3 とは別の1本**としてプロンプトを用意（`ig-reel-madori-trial.md` §8・未生成）。
  1軒の縦長 2LDK を画面いっぱいに。人の代わりに「いま出かけたところ」の気配を置き、人数は読ませない。
  リビング1枚案（C）が弱かった「選ぶものがない」を避けるため、居場所を 6〜8 か所置いて問いを「この家で、あなたの場所は？」にした

## 2026-09-23 夜（Claude Code セッション：1枚リール・mekuru・夜の窓）

- ふうかさんの提案「1枚にまとめた図」を調べた。参考実物5つ（犬の座標・ヘッドホンの座標・perfectpiggy・窓の集合・ユニクロ）から、
  **送られるのは「たくさん並んだ中から自分と誰かを探せる1枚」**、キャラの問いかけは送られない（perfectpiggy 送信/いいね 0.25%）と整理
- **木曜を「1枚リール／Kinda 紹介」の隔週交互で再開**（ふうか決裁）。1枚の図もリールで出す
- **天気の地図は不採用。** ふうかさんの指摘：自分で天気を選べると note の価値を先に渡す。
  原則「1枚リールで Kinda の答えを先に渡さない」を §15 に置いた
- 1本目は**夜の窓**（人を描かずものだけ・評価しない）。キット `docs/sns/packs/2026-09-24-ig-reel-yoru-no-mado.md`
- mekuru（静かな読書ルーム）を分析。伸びたのは作り手個人の X で IG ではない。取り入れたのは「しないことで定義する言葉」と
  「登録なしでどうぞ↓」。**bio に4行目を追加**（ふうか反映済み）
- ことさん#1 の途中経過：**3秒残存 37%**（過去最高）・スキップ率 64.5%。シェア・フォローは0（`kotosan-reel.md` §7）
- Kinda note 紹介は**画面収録をやめ、`public/images/w_*.webp` から組む**方針に変えた
- 夜の窓 v1（ものだけ）を生成。12窓とも指示どおりで絵は成功したが、ふうかさん「人がいるから覗きたくなる」に賛成して**不採用**。
  リアルな人ではなく**ことさん#1 と同じ顔のないクレイ人形**で v2 を作る（キット §5）。v1 は `docs/sns/assets/yoru-no-mado/` に保存

- （続き）夜の窓 **v2 のプロンプトを確定**（キット §6-1）。12場面を人のいる場面に書き直し、2人／1人の窓を市松に置いた。
  各窓に「分からない」を残す（贈り物は渡す前か後か、傘はもう1本誰のか）。v1 は左右の窓が端から約6%で、9:16 に切ると枠が切れるため余白を1/8に強めた
- **ことさんは夜の窓に隠さない**（§6-2）。この回で測りたい「自分の窓を探す型」が濁る／問いが2つになる／ことさんを知らない人が大半／
  ループ生地は窓の中で浮く。10/16 の判定後に、既存ポーズをスクリプトで合成する形で #2 の候補にする
- 投稿は 9/24 20:00 狙い、**9/24 17:00 までに1枚が決まらなければ 10/1**

- 夜の窓 v2（顔のない人形）を生成。指示どおりに出たが**不採用**。ふうかさん「表情がほしい／細い線の人が気持ちいい／逆光なら表情なしで成り立つ・夜にしたい」に賛成し、
  **夜・逆光のシルエット・細い人形の v3** に切り替えた（キット §7）。のっぺらぼうは明るい部屋だと「空白」に見える、逆光なら顔は最初から見えない。
  v2 で「性別の手がかりを全部消す」にしたのは行き過ぎだったので、**髪型・体つきを混ぜ、組み合わせを性別で決めない**に直した。友達か恋人かも決めない

- 夜の窓 **v3（夜・逆光）を採用**。姿勢で気持ちが読め、顔は影の中にうっすら（表情の気配だけ）。9:16 に切ると左右の窓枠が端に付くので、
  **切らずに上へ空を足す** `prep-sky.py` を作った（空は引き伸ばすと縦の筋になる → 帯を反転しながら積む）。`build_reel.py` に `one7` を追加し、
  7秒の MP4 まで書き出し済み（キット §8）

- 音源の訂正：のほほんは**ことさん専用**だった（Claude が「全リール」と広げて記録していた）。夜の窓の曲は基準を渡して、ふうかさんが選ぶ（キット §9）。
  シリーズごとに1曲決めて固定する
- 1枚リールの次の候補に**間取り**（ふうか提案）。「同じ間取り、4つの暮らし」を推した（`ig-strategy` §15 ①-2）

#### 訂正の経緯
- 最初に「天気の地図」を1番に推したが、ふうかさんの指摘で取り下げた（note の答えの先渡しになる）
- 夜の窓を 10/1 案としていたが、ふうかさんが今すぐやりたいとのことで 9/24(木) に前倒し。9/23 当日に出さない理由は
  「邪魔になる」根拠ではなく**計測を混ぜないため**と明記した

## 2026-09-22（季節ビジュアルの仕組み化：夏 → 秋 → ハロウィーン）

「サイトのヘッダーと X のヘッダーが夏のままだ」から入ったが、調べると **Header.tsx には季節の要素がなかった**。
サイトの「夏仕様」の実体は**トップのヒーロー画像**（`hero-couple-2026ss.webp` / `-pc.webp`）で、
`Summer` シールも季節の情景も**画像側に焼き込まれている**。コードで描いている文言はない。

### 直したこと

- **`src/lib/season.ts` を新設（季節ビジュアルの単一の正）**
  - `CURRENT_SEASON` を 1 行変えるだけで、ヒーロー（モバイル／PC）と LCP preload が同時に追従する
  - `summer` / `autumn` / `halloween` の 3 つを定義済み。alt も季節ごとに持つ
  - 画像パスをページに直書きしない。**直書きしていたせいで preload とズレていた**（下記）
- **`layout.tsx` の LCP preload のバグを修正**
  - preload が **`/images/hero-couple-new.webp`（もう使っていない旧画像）** を指していた。
    実際のヒーローは `hero-couple-2026ss.webp` なので、**毎回まるごと無駄な1枚を先読みしたうえに
    LCP の preload は効いていなかった**
  - さらに PC とモバイルで別画像を出しているのに preload は 1 本だけだった。
    `<picture>` と同じ 1024px の分岐を `media` で張って 2 本に分けた
  - ビルド後の HTML で、`media="(max-width: 1023.98px)"` → モバイル用、`media="(min-width: 1024px)"` → PC 用が
    正しく出ることを確認済み
- **`page.tsx` の alt を季節連動に。** これまで「ふたりが歩く村」だったが、実画像はビーチで**内容が合っていなかった**
- **`docs/sns/seasonal-visuals.md` を新設。** サイズ規格・年間の切り替え予定・構図の制約・
  ChatGPT 生成プロンプト（ヒーロー モバイル／PC／ハロウィーン差分／X ヘッダー）・差し替え手順

### 決めたこと

- **9月下旬は「秋」。ハロウィーンは 10 月に入ってから。** 9/22 にハロウィーンは早く、
  「急かしている」印象になる。11 月に `autumn` へ戻す
- ハロウィーンは**かぼちゃの灯りと落ち葉まで**。おばけ・コウモリ・紫・ホラー調は使わない
- プロンプトで**ふたりの性別を指定しない**（`two small clay figures`）。CLAUDE.md §3 の LGBTQ+ 包摂
- **X ヘッダーは 1500×500。文字を入れない**（プロフィール欄と二重になる）。
  左下はアイコンが丸く重なるので主役を置かない。生成物は `docs/sns/assets/x-header-<season>.png` に残す

### 秋の画像を入れて切り替えた（同日・後半）

ふうかさんが3枚生成。`CURRENT_SEASON` を `autumn` に切り替えて公開。

**PC 画像が 3:2 で生成されていて、そのまま置くとシールの「Autumn」の行とふたりの下半身が切れていた。**
PC のヒーローは `min-height: 86vh` + `object-fit: cover` なので、1440×800 の画面では
枠が約 2.09:1 になり、**3:2 の画像は上下 14% ずつ切られる**（16:9 なら 7.5% ずつ）。

判断を目視でやると外すので、**CSS の値（`max-height: 64svh`／`object-position: center 30%`／
右 62% オーバーレイ／下端 14% フェード）をそのまま再現するスクリプトを書いて、
実際の表示枠にレンダリングして確認した。** 現行の夏画像で同じ処理をかけて
「夏は切れない・秋は切れる」を先に確かめてから直した（シミュレーション自体の検算）。

- **PC**：上下を切らず、**上にベージュ空 110px・右に余白**を足して 16:9 にした。
  右に足した分はオーバーレイの下に完全に隠れるので見えない。切ると主役が消える
- **モバイル**：1024×1536 ちょうどだったので変換のみ。LCP 画像なので quality 80（85KB）
- **X ヘッダー**：2048×768 → 1500×500 に上下均等トリム。`docs/sns/assets/x-header-autumn.png`
- 安全域（シール上端は 12% より下／ふたり下端は 88%・モバイルは 85% より上）を
  `docs/sns/seasonal-visuals.md` §4-2 に明文化した

### モバイルを作り直した（同日・3回目）

初稿はふたりが小さく後ろ姿で、木のほうが主役になっていた。夏はふたりが正面・大きく
写っていてファーストビューの主役だったので、**木を4本に絞り、ふたりの背丈と下端位置を
数字で指定したプロンプト**で作り直してもらった。ふたりの存在感が夏と同等になり、
容量も 85KB → 62KB に下がった。

**作り直した版にはシールが入っていなかったので、旧版からシールを抜き出して移植した。**

- シールの最高解像度は**旧・秋モバイルの 221px**だった（PC 原版は 194px で、実は PC のほうが小さい）。
  紅葉が同じ色域（高彩度・R>B）に入って誤検出するので、**木より上の行だけを走査して円を確定**させた
- 元画像と新画像でベージュが僅かに違う（delta 約 [+1.6, -0.2, -2.1]）ため、
  **背景色の差を補正してから**円形マスク（外周6pxぼかし）で合成。継ぎ目は出ていない
- 配置は §4-2 の安全域どおり **上から 12%**（旧版は 3.1% で、iPhone SE の 375×667 だと
  上が 7.2% 切られるため危なかった）

### 覚えておくこと

**季節画像の判断は、CSS の値を再現して実際の表示枠にレンダリングしてから行う。**
今回、目視では分からない欠け（PC のシールとふたり）と、端末差で切れる危険（モバイルのシール位置）の
両方がこの方法で先に見つかった。シミュレーションは現行画像で検算してから使う。

---

## 2026-09-22（PC ヒーローの横切れを直す・シールを中央寄りへ）

公開後のスクリーンショットで、**PC のシールが左で切れて「Autumn」が「utumn」になっていた。**
前日に上下の安全域だけ確認して**横を見落としていた**のが原因。

### 何が起きていたか

PC の枠は `幅 / (0.86 × 画面高)`。これが画像の比率 1.778 を**下回ると左右が切られる**
（上回るときだけ上下が切られる。両方は起きない）。
...
シールは **x 0%〜11.9%** にあり、枠 1.50 で左 7.8% が切られて左半分が消えていた。
しかも前回**右に 480px 足したことで、シールの左端が 3.1% → 2.3% とさらに端に寄っていた。**

### 直し方（ふうかさんの「もう少し真ん中に寄せていい」に合わせた）

**シールは画像の中で自由に動かせる。** 背後が平坦なベージュ空（標準偏差 0.8）で、
円内の紅葉は 19 画素しかなかったため、消して貼り直せると先に確認した。

- 元位置を、**同じ行の平坦な帯（x 560..760）の中央値**で埋めて消した。
  空には僅かな縦の階調があるので単色で塗らない。跡の標準偏差は 0.32〜0.74 で、
  素の空（0.71〜1.0）より平坦。差は 255 階調中 2 以内で視認できない
- 貼り先（x 273..467, y 133..327）は紅葉が y=404 から始まるので重ならないことを確認してから合成
- **上パディングをやめた。** 上を足すほど右パディングも増える（`H×16/9 − W`）ため、
  480px → **284px** に減り、絵の占有率が **76.2% → 84.4%** に上がった。
  結果、木もふたりも右に寄って「中央が空いている」感じが解消した
- 縦の安全域は、パディングで稼ぐのではなく**シールを下に移して**確保した（上端 13.0%）

最終: シール **x 15.0%..25.7% / y 13.0%..31.9%**。枠 2.07 / 1.86 / 1.55 / 1.50 の4つで
レンダリングして、どれでも切れないことを確認した。

### SNS（同日・決裁と実施）

- **X に1本投稿した（A案・119字）。** 画像は PC 版 16:9 を添付。
  文言と採用理由は `docs/sns/seasonal-visuals.md` §7 に実例として残した。
  3案出したうち、**「なぜ絵を変えるのか」まで書いている**案を採った。
  「秋にしました」＋シールの言葉だけだと読む側に「で？」が残るため
- **IG には出さないと決めた（ふうかさん了承）。** 理由は実測。
  フィードは 9/14・9/15・9/17 の3本がそろってリーチ0（フォロワー0だと配信先が存在しない）。
  リールにしても「サイトの絵が変わった」は中の人の報告で、知らない人が見る理由がない。
  3秒継続率15%（目標30%）の改善局面で、いちばん引きの弱いネタに枠を使うのは逆行する。
  中の人の報告枠である「つくる日記」は休止中
  - 判断が再燃しないよう **`docs/sns/ig-week-2026-09.md` §1 に「ここに入れないもの」として明文化**した
  - IG で季節を見せたいときは、報告ではなく**土曜「言いにくい気持ち」で季節そのものを扱う回**にする

### 覚えておくこと

**右パディングは「絵を左に押す」。** 16:9 に合わせるための余白は、切れを防ぐ代わりに構図を犠牲にする。
足す量は最小にし、縦位置はシールを動かして合わせる。

**季節の更新は「サイト＋X」で完結させる。IG は使わない。**

---


## 2026-09-23（Claude Code セッション：ことさん#1 を出し、IG の目的を決めた）

### ことさん#1 を投稿した

**内容を差し替えた。** 元は「社内・自己紹介（そのキャラ、私ですよね）」だったが、
**#4 の「それ、さっき私が言いました」と入れ替えた**（ふうか決裁）。

決め手は **3秒視聴継続率が実測 10〜15%** だったこと。届いた人の85〜90%が3秒で離脱している。
1カット目で勝負がつくのに、「インスタにキャラを立てよう」は作り手側の内輪の話で、
初見の人に共感の対象がない。オチもメタでシェアの動機がない。

空いた 10/2 は、元の社内案ではなく**口コミを読む回**にした。
「そのキャラ、私ですよね」は Kinda が何の会社かを1ミリも伝えないため。

配分は 仕事2・日常1・社内1・人間関係1・恋愛1 で設定書§12 のまま。

### `render-kotosan.js` を実装した（9/21 から持ち越し）

5つの版面（scene / omote / honne / gokun / sukui）。**3つの条件で落とす**：
字数上限超え／吹き出しの下端が y=1500 超え／`gokun` が空。
localhost で確認できない環境なので、目視に頼らない作りにした。

`build_reel.py` をプリセット制に（`pair` / `kotosan4` / `kotosan5`）。
「飲み込む間」の 0.5 秒静止も入れ、実測で確認した（静止区間の前後フレームの平均差 0.004）。

### 画像の穴を3つ潰した

1. **眼鏡が銀色で出ていた。** 基準絵はゴールド。ロックが `silver-grey` で、さらに他の枠のルール
   「No gold, no brass」まで当たっていた。**設定書の文章だけでは色が分からなかった**
2. **机の色が回ごとに変わった。** 場面セットに色を書いていなかった。
   1本のリールで机の色が変わると、12秒のクロスディゾルブの中で気づかれる
3. **場面画像が 360×540 で届いた。** 9:16 にすると 356% 拡大で生地が潰れる。原寸で受け取る

**教訓：文章の設定書と基準絵が食い違ったら基準絵が正。** 基準画像2枚を
`docs/sns/assets/kotosan/` に入れたので、次からふうかさんが貼り直さなくていい。

### 調査：なぜシェアされるのか

**2026年の IG は Sends per Reach（リーチあたりのDM送信率）が最重要シグナル。**
保存は主役から降りた。詳細と出典の限界は `ig-strategy` §13。

仮説：**「これ私じゃん」（いいね）と「これ○○さんじゃん」（送信）は別物。**
DM で送るとき、人は自分の話ではなく**相手に見せたい話**を送る。
テスト6本を「誰かを思い出させるか」で分類して、10/16 に送信数との相関を見る。

ハピハピの森・ちいかわ・おぱんちゅうさぎ・んぽちゃむを調べた。
**全部「かわいい絵 × シビアな中身」のギャップで、ことさんも同じ系譜にいる。**
足りないのは**報われなさ**。ふくらみが元に戻らず蓄積する案を記録（判定後に決裁）。

**調べ方で2回間違えた。** 静止画5枚から「ハピハピは文字が3回」と推測したが実際は違い、
「事件が起きない」も違った（転ぶし夜になる）。**材料不足を推測で埋めると外す。**
訂正は `kotosan-reel.md` §8 に経緯ごと残した。

### いちばん大きい決定：IG の目的（`ig-strategy` §14）

ふうかさんから「認知を取るみたいな思考に固執していて迷走感がある」という話があった。
調べた結果、**方向（CLAUDE.md のビジョン）はブレていない。
SNS と送客 ¥5,000 の間に線が引かれていなかった**のが原因だった。

**決まったこと。**

- **IG から来た人にやってほしいのは Kinda note**（最優先）→ act → コラム
- **送客は IG の直接の目的にしない**（相談所を探す人はリールタブでなく Google で探す）
- 方針 **A（週3本維持）**。掲載営業は連休明けから並行して開始
- 音源は**モッフー「のほほん」に固定**

**実装を確認して自分の発言を訂正した。** Kinda note は会員登録不要で結果は localStorage のみ。
やってもらっても Kinda 側に何も残らない。「ストックとして貯まる」は誤りだった。
会員登録なしは意図的な設計なので変えない。価値は体験・回遊・想起のほう。

**導線の穴：** bio の1行目に note が無い（pair と act だけ）。コラムの棚が無い。
リールのキャプションにリンクは貼れないので、**勝負は bio とハイライトだけ**。

### セッションの長さについて（CLAUDE.md §7-7 に追加）

会話が伸びると古い部分は要約され、細部が失われる。調べ物の精度も落ちる。
**ふうかさんに言われる前に Claude から申告してまとめに入る**ルールを足した。
結論は会話に置かず、ファイルに置く。

---

## 2026-09-22（Claude Code セッション：ポーズ2枚と、セットがズレる穴）

### 眼鏡はゴールドで確定

2枚とも基準絵どおりゴールドで出た。ふうか判断でゴールドに統一する。
**銀色の `puff-side-v1` は作り直し。**

### 場面セットに色を書いていなかった（実際にズレた）

同じ §3-A の場面プロンプトから、**テラコッタの机とクリームの机が出た**。
`lookdown-tilt` と ⑦ はテラコッタ、`glare-desk` はクリーム。

1本のリールはカット2〜4 が 12 秒の中でクロスディゾルブしながら切り替わる。
**机の色が変わると気づかれる。** 原因はプロンプトが「A clay desk and clay office chair」としか
書いておらず、色が毎回運任せだったこと。

**§3-A で机・椅子・ノートパソコンをテラコッタ（#D4A090）に固定した。**
マグも同じテラコッタ、鉢だけクリーム。壁に影を落とさない指定も足した
（`lookdown-tilt` の壁に斜めの影が出ていたため）。

§7 のチェックに「同じ回で使う他のカットと机・椅子の色がそろっているか」を足した。

### 出たもの

| ファイル | 状態 |
|---|---|
| `kotosan-pose-lookdown-tilt-v1` | 採用。机テラコッタ |
| `kotosan-pose-glare-desk-v1` | 採用だが**机がクリーム**なので #2 では使えない。要再生成 |
| `kotosan-pose-puff-side-v1` | 眼鏡が銀色。要再生成 |

① は「椅子ごと半身で振り返る」と書いたが**正面で出た**。振り返りは取れていないので、
出たものは `glare-desk-v1`（机ごしの正面）として別名で登録し、`glare-turn-v1` は未生成のまま残した。
**プロンプトの名前と出た絵が違うときは、出た絵に名前を合わせる**（あとで探せなくなるため）。

---

## 2026-09-21（Claude Code セッション：ことさんのポーズ生成プロンプト集）

`docs/sns/series/kotosan-poses.md` を新設。**4ポーズだけで回さない**ための資産。

### なぜ作ったか

IG のオリジナリティ規定（2026-04-30 改定・70%以上一致で近似重複、24時間〜30日リーチ低下）に、
毎回同じ4フレームは正面から当たる。リール化で「5枚中4枚が同じ」は解消したが、
**カット2〜4の中身は依然として使い回し**なので、そこに厚みを持たせた。

### 作ったもの

- **共通ロック＋場面セット＋ポーズ差分＋共通フッタ**の組み立て式。毎回考えるのは差分1ブロックだけ
- 新規11ポーズ（glare 3・lookdown 3・puff 3・mug 2）。`kotosan-reel.md` §4 にあった
  候補（窓の外の後ろ姿・ひもがゆるむ・角度違いの lookdown・無地の puff）を実文に起こした
- 出力チェック10項目と、#1〜#6 でどれを当てるかの表

### 比率を 4:5 → 2:3 に変えた（ふうか決裁）

**4:5（1080×1350）から 9:16 を切ると 759×1350 しか残らず、1080×1920 に伸ばすと 142%。**
2:3（1024×1536）なら 864×1536 が残って **125%** で済む。実際に ⑦ を 2:3 で生成して確認した
（中央で左右80pxずつ落とすだけで、机も椅子も切れずに収まる）。

他の枠の「生成は必ず 1:1」は `prep.py` でモチーフを切り出す型のためのルール。
**ことさんは背景ごと写るので `prep.py` を通さない**。TODO.md と `ig-week-2026-09.md` §7 の
両方に例外として書き足した（片方だけ読んだセッションが取り違えるため）。

### ⑦（真横からのふくらみ）を1枚生成した

`docs/sns/assets/kotosan/kotosan-pose-puff-side-v1.webp`。**SNS の作業素材の置き場をここに決めた。**
`public/` に置くと kinda.jp から配信されてしまう。WebP・quality 95 で 1.9MB → 178KB。

作業ディレクトリは毎回リセットされるので、これまでは毎回ふうかさんが貼り直していた。
**リポジトリに置けば次のセッションが自分で取れる。**

### ハマったこと

社員証に**青い四角**が出た。共通ロックは `blank cream felt staff ID badge` と書いてあり、
フッタにも `No text, no letters ... including on the ID badge` があるが、
**「文字を入れるな」は図形を止めない**。`no shape, no square, no icon, no colored patch` を明示して直した。
今回のぶんは文字ではないので採用した。

### 直した古い記述

- TODO.md の週の型が「金 18:00・週1本・判定 11/6」のまま。9/20 決裁の**水・金・週2本・判定 10/16**が正
- README の型の表に、ことさんの画像の正（`kotosan-poses.md`）を足した

### 基準絵を見て、共通ロックの誤りが2つ見つかった

ふうかさんが4面キャラシートを貼ってくれて判明。**設定書§9 の文章だけでは足りなかった。**

- **眼鏡は基準絵ではゴールド。** ロックに `silver-grey` と書き、さらにフッタの
  `No gold, no brass` まで当たっていたので、⑦ が銀色で出た。
  あの指定は `prep.py` を使うモチーフの枠のルールで、**ことさんに当ててはいけなかった**
- **社員証の中央の青い四角は基準絵に元からある。** 「無地」と読んで no square を足したのは誤り。撤回した

教訓：**文章の設定書と基準絵が食い違ったら基準絵が正。** 色のような「絵を見れば分かること」は
設定書に書かれていないことがある。

### `render-kotosan.js` を実装した

5つの版面：`scene`（場面＋シリーズ名＋状況44px）／`omote`（白48px）／`honne`（薄茶の透け44px）／
`gokun`（88px・吹き出しなし・天地中央）／`sukui`（白44px）。`render-reel.js` と同じ Playwright 方式。

**黙って壊れたフレームを出さないよう、3つで落とす。**
localhost で確認できない環境なので、目視に頼る設計にしない。

1. 1行の字数上限超え（48px=16字／44px=17字／88px=8字）
2. 吹き出しの下端が y=1500 を超える（IG の UI に隠れる）
3. `gokun` が空（シリーズの目印を省かせない）

`kotosan-reel.md` §3 に**カット1の版面が無かった**ので足した（シリーズ名 y=268・28px・55%／状況44px）。
実装で決めた値を型のファイルに戻している。

### `build_reel.py` をプリセット制にした

`SEGS` が連載用の尺でハードコードされていたので、`pair`／`kotosan4`／`kotosan5` を持たせた。
**引数なしの既定は `pair` で従来どおり**（連載の書き出しは変わらない）。
出力ファイル名も引数で受けるようにした（従来は `kinda-ig-0919-reel.mp4` 固定だった）。

`SEGS` に**静止**の値を足した。ことさんは3カット目の末尾 0.5 秒だけズームを止める＝飲み込む間。
`zoompan` の式を `min(z0+Δ*on/(m-1), z1)` にして、m フレームで到達したあとは z1 のまま保つ。
実測で確認した（静止区間の前後フレームの平均差 0.004、ズーム中は 4.339）。

クロスディゾルブは1つの `XF` から**カット間ごとの `XFS`** に変えた。ことさんは 3→4 だけ 0.15。

### 尺の読み方を直した

`kotosan-reel.md` §2 の表の「秒」はカットの境目で、**クロスディゾルブの重なりを引く前の値**。
実尺は `kotosan4` が **12.15秒**、`kotosan5` が **14.80秒**（表は約13秒・約16秒と書いてある）。
型のファイルに但し書きを足した。

### 次

**9/23(水) の第1回。** Notion「ことさん#1」から状況・表・本音を取り、場面画像（社内の会議）を
生成して `render-kotosan.js` → `build_reel.py kotosan4`。
カット2〜4 のポーズは `assets/kotosan/` に置いたものを使う（眼鏡が銀色のものは作り直し）。

---

## 2026-09-21（Claude Code セッション：RIE COFFEE の手直しと、ランデブーラウンジの整備）

PR #53・#54。これで **Kinda act の実在6店すべてにリール画像が付き、席タイプ画像も6種類そろった**。

| 席タイプ | 店 |
|---|---|
| `booth-semi-private` | PISOLA 横浜睦町 |
| `counter-window` | 銀座伴助 新宿タカシマヤ |
| `booth-high-back` | おだしもん トレッサ横浜 |
| `booth-window-green` | むさしの森珈琲 久が原 |
| `banquette-open-row` | RIE COFFEE 本店 |
| `lounge-armchairs` | ランデブーラウンジ |

### RIE COFFEE の手直し（ふうかさんの指摘）

- **壁の上の本棚を図から削除。** 飾りで、席の使い勝手と関係がない。図の5番と説明1行を落とし
  番号を1〜4に詰め、空いた分だけ viewBox の高さも縮めた。
  **写真に写っていても、意味のないものは図に描かない**（トレッサで間仕切りを描きすぎたのと同じ話）
- **服装の「両方いける」→「きれいめでもカジュアルでも浮かない」。**
  選択肢の値は**サイトにそのまま出る**ので、「両方」が何と何を指すのかページ上で分からなかった。
  Notion の選択肢名も同じ文言に変え、入力と出力で語を一致させた。
  片方だけ直すと翻訳の手間が残り、そのうちズレる
- **一行の観察を差し替え**：「仕切りがないので、隣の声も自分たちの声も届く。」→
  「先に注文して支払うので、座るまでの数分は並んで過ごす。」
  旧案は `隣席との距離＝ほぼ聞こえる` が詳細ページに出るので一覧カードと重複していた。
  新案は他の5店にない、この店だけの現象（向かい合う前にカウンターで横に並ぶ時間が入る）

### ランデブーラウンジの整備

公式情報を入れた。1人1,900円〜／17:30〜21:00は予約可／プラン予約の無断キャンセルは料金100%。
**無断キャンセルの条件は利用者の不利益になるので、特徴タグではなく本文（description）に書いた。**

**不変則の取り残しを1件見つけて直した。** この店だけ `hours` / `holiday` を持っていた
（「9:00〜22:30ごろ・2時間制」）。「営業時間・定休日は Kinda では持たない。古い値を抱えるより
Google マップへ渡す」は 2026-09-09（PR #39）に決めた不変則で、この店はそれ以前に入っていた。
削除したところ、既存のしくみが働いて「Googleマップで確認する」に置き換わった。
**不変則を決める前に入ったデータは、後から点検しないと残る。**

「2時間制」も特徴から削除。公式サイトに記載がなく、一休.com のプラン予約に付く条件と
混ざっていた可能性があるため。**出どころが特定できない条件はサイトに載せない。**

### 判断の記録

- **リール画像はブランド色に寄せなくてよい**（ふうかさん判断）。ランデブーラウンジは青い絨毯と
  色帯の壁をそのまま残した。この用途では、店の見分けがつく要素のほうが効く。
  クレイ風ベージュの縛りは**サイトのUI・ヒーローの話**であって、席タイプ画像には及ばない
- 一行の観察について「女性を座らせて男性が注文する、と読まれないか」という指摘があった。
  文そのものは役割を振っていないが、**ふたりが並ぶと断定している点で事実としてやや踏み込んでいる**。
  今回はそのまま採用。引っかかるようなら「注文と支払いを済ませてから、席に着く。」に差し替える
  （役割にもふたりが並ぶかにも触れない形。代わりに「最初の数分が正面からでない」含みは消える）

---

## 2026-09-19〜20（Claude Code セッション：RIE COFFEE 掲載、シーン語彙の許可リスト、サンプル全件撤去）

PR #45〜#50。Kinda act の実在店が6件になった。

### RIE COFFEE 本店を掲載（初のお見合い向き）

ヒルトン東京 B1F。`8f7b4b88-b00a-44da-9188-a81f37adcdca`。
これまでの4店と違い**席と席のあいだに仕切りがない**（壁沿いに一続きのベンチソファ、
通路側にいす、テーブルが横一列）。`隣席との距離＝ほぼ聞こえる`。
お見合いで使われている店なのに会話は聞こえる、という組み合わせがこの店の性格なので、
俯瞰図でも隣のテーブルとの間隔を見せた。席タイプ画像 `banquette-open-row.webp` を新規作成。

取材ログから直したもの:
- `沈黙の逃げ場` に「話題はKinda pairで探しておいて」という**自社導線が入っていた**。
  観察欄に宣伝が混ざる形なので、事実（セルフサービスで席を立つ場面がある／目を移せる
  ものは少ない）に書き換えた。導線は UI 側の仕事
- `店の空気` の「マダム（言い方考えて）」→ 年齢や属性のラベルを貼らず起きていたことだけ
  書く形に。ふうかさんから「『長く』話していたは場合によっては攻撃的」と指摘があり削除。
  席を占有していたというニュアンスが出るため
- 「穴場と呼ばれている」（伝聞・宣伝寄り）は現地メモへ退避
- `公式URL`（url型）に3つのURLとコメントが入っていてリンクが壊れる状態だった

### シーン語彙を許可リストで固定した

`何度か会ってから` を足したあと、`scenes` が `text[]` で enum 制約もコード側の許可リストも
なく、「何回か会ってから」のような表記ゆれを止める仕組みがないことをふうかさんに説明したら
「あなたが触れるようにして」と。二重に固めた。

- `src/lib/actScenes.ts` の `ACT_SCENES` が正
- DB の CHECK 制約 `shops_real_scenes_vocabulary`（**実在店のみ対象**。サンプルは旧語彙を
  持ったままなので除外し、実在店に切り替えた時点で語彙が必須になる設計）
- 本番DBで `何回か会ってから` を実際に書き込んでみて `check_violation` を確認（ロールバック済み）
- 表示順も `sortActScenes` で関係が進む順に固定。DB の配列順のままだと店ごとに並びが変わる

`顔合わせ・待ち合わせ` は廃止（ランデブーラウンジ1件のみ保持していた）。

### サンプル9件をすべて非公開に

きっかけは、詳細ページに「サンプル」バッジが**実装されていない**と気づいたこと
（`DemoBadge` はリールのカードとモーダルだけ）。架空店の `/places/[id]` が
★4.6・12件の口コミを本物の顔で出していた。構造化データには出ていなかった
（`aggregateRating` なし）ので検索結果への影響はなかった。

まず評価付き4件、ふうかさんの指示で残り5件も下げ、`is_demo = true` の9件すべてが非公開。

### 失敗：一覧の断り書きが事実と逆になった

`/kinda-act` と `/kinda-glow` に「現在掲載中のお店はすべてサンプル表示です」が**ベタ書き**
されていた。実在店が入ったあとも残り、**行って確かめた6店を指して「サンプル」と言う状態**。
「行って確かめた」バッジと正面から矛盾していた。

さらにサンプルを全件下げたことで `/kinda-glow` は掲載ゼロになった（あのページは美容系4件
だけで全部デモだった）。**お店がないページに「掲載中のお店はすべてサンプル」**と出ていた。

`PlacesDataNotice` に切り出してデータで出し分ける形に。0件／全部デモ／一部デモ／全部実在の
4通り。見た目は変えていない。

**教訓**：掲載データの状態を前提にした文言をページにベタ書きしない。データが変われば
必ず嘘になる。Kinda act の不変則「ページに文言をハードコードしない」（Kinda pair で
決めたもの）は、この手の注記にも当てはまる。

### 訂正した自分の判断

- **価格**：「`食事1人` の意味が店によってメニュー単価と合計で揃っていない」と TODO に
  起票したが誤り。全店もともとドリンク込みで揃っていた。**飲み物なしで食事だけ頼む人は
  実際にいないので、単価で書くと誰も払わない額が並ぶ**。取り決めとして不変則に格上げした
- **客層の「年齢層高め」**：代案「落ち着いた年代が多い」を出したあと、先日 `客層` から
  評価語として消した「落ち着いた」を戻すことになると気づき、ふうかさんの原文のままにした

### ハマったこと

- `shops.scenes` は `jsonb` ではなく `text[]`。`'[...]'::jsonb` は 42804 で落ちる
- リールカードは `aspect-ratio: 9/16` に `objectFit: cover`。正方形画像は**左右44%が切れる**。
  画像生成プロンプトに「左右に余白を残す」と明記して回避
- この環境に `playwright` npm パッケージはないが、`/opt/pw-browsers/chromium-*/chrome-linux/chrome`
  を直接 `--headless --screenshot` で叩けば SVG の見た目を確認できる
- `Kinda ふたりへ` はヘッダー・フッター・ロゴで使われている正式名称。古い表記ではないので
  文言整理のときに消さないこと

---

## 2026-09-20（Claude Code セッション：IG を全枠リールに切り替えた）

> **この週最大の発見。** カルーセルは内容が悪かったのではなく、**届く面がなかった**。
...
ふうかさんが IG インサイトで直接確認。**入力ミスではなく実際に0**。

フォロワー0だとフィード投稿の配信先が存在しない。リールはリールタブ（65〜68%）と
発見（30〜35%）という非フォロワー向けの面を持つ。**構造の差**であって、内容の差ではない。

`ig-strategy` §2 はカルーセルに「フォローされる」「プロフィールに来てもらう」を割り当てていたが、
**見られていない投稿はどちらも実行できない**。週2本ぶんの制作が届かないところに置かれていた。

### リール自体は良くなっている

9/12 → 9/19 で 3秒継続 10%→15%、スキップ 92.7%→88.8%、プロフ遷移 0→2
（W27 から4本連続0だったのが初めて動いた）。**尺は14→17秒と伸びたのに改善**。
単色スタイル＋モチーフ3枚が、ジオラマ5カットより効いている。

> **訂正**：WORKLOG 9/12 の「公開40分で3秒継続率100%」は閲覧72時点の暫定値で、**確定値は10%**だった。
...
| | |
|---|---|
| 火 20:00 | 連載「ふたりの話題」28週 → **リール** |
| 金 18:00 | ことさん → **リール**（火曜が連載で埋まるため週1本・判定を11/6へ） |
| 土 12:00 | 「言いにくい気持ち」 → **リール**。外枠から本枠に昇格 |
| 木 | つくる日記 → **休止**（可能性のある枠に集中する・ふうか判断） |

旧「入口リール」は役目を終えて統合。**火曜の連載と同じ話題を土曜に出す設計**だったため、
連載のリール化で近似重複になる。土曜は別の内容にした。

### IG のオリジナリティ規定を調べた（2026-04-30 改定）

- **投稿頻度そのものにペナルティはない。** 1日1〜2本は推奨範囲。カルーセルとリールは配信面が違う
- 効くのは**オリジナリティ**。従来リールだけだった規定が**写真・カルーセルにも適用**された。
  70%以上が既出と一致すると近似重複と判定され、**24時間〜30日間、非フォロワーへのリーチが落ちる**
- 「新しいナレーション・グラフィック・コメンタリーの追加」は material edit としてオリジナル扱い

これが2か所に刺さった。**①同じ話題を週2回出す設計**（やめた）。
**②ことさんの「4ポーズ使い回し」**（カルーセルだと5枚中4枚が毎回同じ。リール化で解決。
それでもポーズのバリエーションは足していく）。

### ことさんをリールにした理由（3つ）

1. **カルーセルのままだとテストが判定できない。** 設定書§12が「リーチあたりで比較」なので、0では割れない
2. **オチがスワイプ頼みでなくなる。**「（ごくん）」は途中離脱で死ぬ。リールなら最後まで見せられ、間も作れる
3. **Canva 作業がなくなる。** 吹き出しはスクリプトで焼き込める

### 仕組みの穴を1つ塞いだ

`docs/sns/` に型を書いても、**新しいセッションは自動では読まない**。毎回必ず読み込まれるのは
`CLAUDE.md` だけ。ふうかさんは「今日のIG作るのでプロンプトください」としか言わない運用なので、
**入口が無いと型に辿り着けなかった**。`CLAUDE.md` §7-b に曜日→ファイルの表と手順を置いた。

Notion に置いても同じ問題（読もうと判断したときだけ読む）。型はリポジトリ側に置く。
理由はスクリプトの引数と数値が同じ場所にあること、Notion MCP は落ちることがあること。

### その他

- 9/20（日）に外枠のリールを1本出した。「連休の夜」3カット・13.0秒。
  尺は9/19の17秒から短くした（平均再生が2秒しかなく完走していないため）
- 共通プロンプトに `No gold, no brass, no metallic parts.` を追加。
  9/20 の鍵の生成で真鍮リングが出た。CLAUDE.md §4 が旧ゴールド路線を禁じているため
- ChatGPT の画像生成は**1日8枚まで**（ふうか申告）。以前の「節約する」前提は不要になった

---

## 2026-09-14〜19（Claude Code セッション：IG 4型の確定と、プロンプト・版面の資産化）

> この週で **IG の週3枠すべてに型がついた**。正は `docs/sns/series/` の4ファイル、
> スクリプトは `docs/sns/tools/ig-carousel/`。以降は「型を読む → その回の `【SCENE】` を考える」だけで回る。

### 出した4投稿

| 日 | 枠 | 形式 | 型の正 |
|---|---|---|---|
| 9/14 | 外枠（型検証） | カルーセル6枚 | `series/iinikui-kimochi.md` |
| 9/15 | 火＝理由層 | カルーセル5枚 | `series/kinda-pair-28.md`「画像の型」 |
| 9/17 | 木＝深さ層 | カルーセル4枚 | `series/tsukuru-nikki.md` |
| 9/19 | 土＝入口層 | リール17.9秒 | `series/nyuguchi-reel.md` |

### いちばん効いた判断：絵を「情景」から「もの1つ」に変えた

9/12 までは木の台座つきミニチュアジオラマだったが、9/14 から
**背景 `#F5EEE6` の単色＋小さなクレイのもの1つ**に変えた。ふうかさんの「やっと見たかったミニチュア感が出た」。

副次的に効いたのが**制作量**。情景を毎回作ると週10枚を超えるが、もの1つなら週3〜4枚で足りる。
さらに、リールも同じスタイルにしたことで**1秒目の文字が立つ**ようになった（情報量の多い絵は文と competing になる）。
9/12 のジオラマ方式キットには「置き換え済み」を明記した。

### 週あたりの生成枚数を3〜4枚に抑えた設計

- 火曜：1枚目のモチーフのみ。2〜4枚目は文字だけ、5枚目は**28週使い回す固定画像**
- 木曜：1枚目のモチーフのみ。2枚目以降は文字だけ
- 土曜：3枚。**そのうち1枚が翌火曜のカルーセルに回る**ので、火曜の生成が0枚になる

生成は **1:1 で行い、スクリプト側で 4:5 と 9:16 に置き直す**。同じ絵を両方の枠で使える。

### 文面で直した4か所（いずれもふうかさんの指摘から）

1. **最終枚に一人称を置かない**（9/14）。1〜5枚は主語がないのに最終枚だけ「（私たちが）つけています」と
   発話者が出るため、そこだけ広告に切り替わって読める。**Kinda に繋げたことが原因ではなく、声が変わったことが原因**。
   接続はキャプションに逃がし、画像は問いだけを残す
2. **層名を1つだけ大きく置かない**（9/15）。「ふれる」単独だと婚活の文脈では別の意味に読める。
   **4つの層名を毎回そろえて並べる**ことで、説明せずに解決した。
   「話にふれる、という意味です」のような注釈は書かない（避けたい読みを名指しすると、かえって意識させる）
3. **「いちばん外側の」を書かない**（9/15）。位置の説明のつもりでも、浅い＝遅れている、と読める
4. **`#ひとり開発` を使わない**（9/17）。見ているのは開発者で、フォローされても結婚相談所を探さない。
   フォロワー0の段階でコンバートしない層が混ざると、その人たちが婚活の投稿に反応しないぶん
   エンゲージメント率が下がり、配信が鈍る。枠は回遊タグで婚活側に返した

### 技術メモ（次に同じ作業をする人向け）

- **フォントは npm から取る。** `fonts.google.com` はセッションの egress で塞がれている。
  `@fontsource/shippori-mincho` の woff2 を headless Chromium で読ませて文字を焼く
- **ffmpeg も同様。** Playwright 同梱のものは VP8/WebM だけの縮小ビルドで H.264 を吐けない。
  PyPI の `imageio-ffmpeg` が libx264・aac・xfade・zoompan 入りのバイナリを持っている
- **モチーフの検出はエッジ基準。** 輝度差で取ると柔らかい落ち影を物体と誤検出し、中心が横にズレる
  （9/14 に実際に左へ40pxズレた）
- **`letter-spacing` の値を切り出して `margin-right` に流用しない。**
  `'.08em'.slice(1)` は `'08em'` になり、Chromium はこれを **-8em** と解釈する。
  文字ブロックが約170px 右へずれて右端が切れる（9/15 に発生）

### ハマったこと：ドキュメントを feature branch に置いたまま忘れた

「次回はこのファイルを読んで」と何度も書いたのに、**その7コミットが main に入っていなかった**。
新しいセッションは `origin/main` から始まるので、そのままなら週次の型が全部存在しないことになっていた。
ふうかさんに指摘して同日マージ。**CLAUDE.md §7 に「未マージのブランチを毎セッション確認する」を追加した。**

---

## 2026-09-15（Claude Code セッション：Kinda act に2店追加、シーン語彙の作り直し）

PR #45（マージコミット `95bd285`）。おだしもん トレッサ横浜店とむさしの森珈琲 久が原店を
Kinda act に載せた。実在店は5件になった。

### 席配置図を2枚起こした

`public/images/act/seating/odashimon-tressa-yokohama.svg` と `musashinomori-kugahara.svg`。
どちらも対面のボックス席。**図には番号だけを置き、説明は詳細ページの番号付きリストで読ませる**
形は既存2枚と同じ（図の中に文字を書くと狭い画面で潰れて横スクロールが要る）。

トレッサの図は最初、写真から読み取れない間仕切りと寸法線を描いてしまい、レンダリングを
見て消した。**写真に写っていないものを図に足さない。**

### 「店の空気」を独立した項目にした

`ActObservations.fit` に `atmosphere` を追加。取材ログを見返したら「落ち着いた雰囲気」の
ような観察が毎回 `客層` に入っていた。**誰がいるか（crowd）と、その場がどんな感じか
（atmosphere）は別の軸**で、同じ欄に入れると両方読みにくくなる。Notion 側も `店の空気`
列を足し、`客層` の選択肢から「落ち着いた雰囲気」を削除した。

### シーンの語彙を作り直した（いちばん大きい変更）

プレビューを見たふうかさんから「おだしもんもむさしの森も初回デートではない気がする。
むさしの森は周りに特に何もないので初回デートはハードルが高い」という指摘。

整理すると **周辺環境が効くのは初回デートだけ**だった。お見合いは1時間で二軒目に移らない
ので周りに何もなくても成立する。何度か会ったあとも、場所に会話を助けてもらう必要がない。
初回デートだけが「店を出たあと」に依存する。

そこで `何度か会ってから` を新設し、2店とも `お見合い / 何度か会ってから` にした。
使われていなかった `顔合わせ・待ち合わせ` は廃止（ランデブーラウンジ1件のみ保持していた）。

`scenes` は Supabase の `text[]` で **enum 制約もコード側の許可リストもない**。
`src/app/places/[id]/page.tsx` は配列をそのまま描画するだけなので、語彙を足しても
コード変更が要らない代わりに**表記ゆれを止める仕組みもない**。不変則として TODO.md に書いた。

### ハマったこと

- `scenes` は `jsonb` ではなく `text[]`。`'[...]'::jsonb` を代入すると 42804 で落ちる。
  `ARRAY[...]::text[]` を使う
- リールカードは `aspect-ratio: 9/16` に `objectFit: cover`。正方形画像を入れると
  **左右の 44% が切れる**。生成プロンプトに「左右に余白を残す」と明記して回避した

### 今回の判断の記録

- 久が原の最寄駅は、店名の「久が原」（東急池上線・徒歩23分）より西馬込（都営浅草線・
  徒歩14分）のほうが近い。**両方載せて、歩いた分数が短い順に並べた。**
  店名の駅を書かないと、そこから来た人が着いてから驚く
- トレッサの徒歩最寄りは大倉山だが26〜30分あり「歩ける距離ではない」ため落とし、
  バス4系統だけにした

---

## 2026-09-12（Claude Code セッション：IG運用の実行開始、計測の仕組み、朝Routineの秘書化）

> 全体像は `docs/sns/sns-operations-2026-09.md` にまとめた。ここには経緯と判断だけ残す。

### 完了

**9/12 の IG リールを実際に出した（入口層・`l1-holiday`）**
- 画像5枚を ChatGPT で生成 → Claude が Shippori Mincho で文字入れ → 投稿、という流れが確立した
- **公開40分・閲覧72の時点で3秒視聴継続率100%**（W27 は12〜20%）。変えたのは1秒目だけで、
  世界観のコピーをやめて `topics.ts` の `ask` をそのまま置いた。確定値待ち
- カット1は最初「玄関全体」で生成したが、靴とスリッパが小さく1秒で二択が読めなかった。
  寄りで撮り直し、上40%を無地の壁にして文字の置き場所を絵の側に作った。**この作り直しが正解だった**
- 最初の玄関全体の絵はカット5（オチ）に回した。ひとつの床に同じ光で靴とスリッパが並ぶ絵のほうが、
  「どちらが良いという話ではありません」を絵の側で言えている

**ハッシュタグを1投稿5個に絞った**
- Instagram の上限。9/12〜9/29 の8投稿すべてが6〜10個付いていたので組み直した
- 枠の型：顕在ワード2つ＋内容に一致2つ＋回遊タグ1つ
- `#kinda` は全部から外した。フォロワー0では検索されず、5枠のうち1つを使う価値がない

**IG の週3固定を 9/12〜9/29 まで投入（＋9/14 に型検証枠）**
- 連載カルーセルの曜日が2つの設計ドキュメントで食い違っていた（`series/kinda-pair-28.md` は木曜、
  `ig-strategy-2026-09.md` は火曜）。新しい戦略に合わせて火曜へ移した。X の連載も火曜なので2媒体が揃う

**計測の仕組みを作った**
- X投稿カレンダーに7列（計測日・インプレッション・エンゲージメント・プロフィールアクセス・
  リンククリック・詳細クリック・振り返りメモ）、IG投稿カレンダーに8列（閲覧数・フォロー数・
  3秒視聴継続率・スキップ率・平均再生時間_秒・尺_秒・リールタブ比率・発見比率）を追加
- 「計測日」が埋まっているかで入力漏れを判定する設計
- 運用資料を `docs/sns/metrics-operations.md` に作成

**朝の Routine を秘書モードに変更**
- 構成を固定（今日やること／今日の投稿／数字の入力／期限のあるTODO／発信の間隔／今日の推奨）
- **催促を有効にした。** ただし締切のあるTODOと数字の入力だけ。発信の間隔は事実の提示にとどめる。
  4方向から毎朝追われると続かないため
- 数字は自動で取れない（IG / X のインサイトに繋がるツールが無い）。ふうかさんが報告し Claude が転記する

**Y.K さんの活動期間を4ヶ月目に確定**（仮置きの3から差し替え）

### 参考にしたもの

**`@kawaisounakiwi`（可哀想なキーウィ！）**
- 69投稿・2.1万フォロワー・**ほぼ全部カルーセル**。いいね5.9万に対しDMシェア5,076（約8.6%）
- 発明は「説明せずに、まだ言葉になっていない気持ちに名前をつける」こと。
  **Kinda note の天気の言葉と構造が同じ**なので、真似ではなく既存資産の見せ方を変えればよい
- 採らないのは「可哀想」で笑う型。キウィが非人間だから成立している
- この気づきから 9/14 に単色背景・1枚1文のカルーセルを型検証として1本入れた

### ハマったこと

- **X・Instagram・Threads・LINE STORE が通信ポリシーでブロックされている。**
  外部アカウントの調査は、ふうかさんにスクショを貼ってもらうしかない
- **Notion の画像の署名URLは5分で切れる。** 画像の受け渡しはチャットに直接貼るのが最速で確実
- SNS用画像を `public/images/` に置くとサイトから配信されてしまう。リポジトリから外した

### 次にやること

- **IG の bio を二層構造に更新（9/15 の連載初回より前に）。** Routine が 9/15 まで毎朝リマインドする
- ピン留め②③の画像を作る（原稿は `docs/sns/ig-foundation-2026-09.md` にある）
- 「住む場所・引っ越し・転勤」のコラム。9/29 の連載第3週のセルフリプ2が空いている
- 9/12 のリールの確定数字を転記する
- 10月分のカレンダーを9月末に作る

---

## 2026-09-13（Claude Code セッション：GSC インデックス停滞の原因特定と4点修正）

### 完了

**GSC の未登録84件の原因を実測で特定した（PR #44・未マージ）**

推測で手を打つ前に、内訳を見てもらったのが分かれ目だった。
**「検出 - インデックス未登録」78件／「クロール済み - インデックス未登録」1件**。
「内容が薄いから落とされている」のではなく **そもそもクロールされていない**。
サマリーのグラフも、8月20日前後に灰色の線が約5→約83へ**垂直に跳ね上がってから4週間真横**で、
「sitemap で一気に発見したが1本もクロールしていない」形をしていた。

先に潰しておいた確認：本番HTMLを実際に取得して、canonical・meta description・OGP・robots.txt が
すべて正しく出ていることを確認した。**技術タグ側の不具合はゼロ**。ここで時間を使わずに済んだ。

**① `sitemap.ts` が mock を読んでいて、実在カウンセラーの詳細ページが1本も送信されていなかった**
- `import { COUNSELORS } from "@/lib/data"` → `.filter((c) => !c.isDemo)` としていたが、
  mock 配列の4名は**全員 `isDemo: true`**。つまり常に0件だった
- 実在カウンセラーは Supabase 側にいる（小山楓華さん1名）。`getPublicCounselors()` に変更
- ビルド時に固定されると新規カウンセラーが載らないので `export const revalidate = 3600` を追加
- **教訓**：mock フォールバックを持つデータ源を sitemap から参照すると、
  フォールバック側の性質（ここでは全件デモ）で出力が静かに空になる

**② 掲載0名の一覧ページ11本を sitemap から外した**
- Supabase で確認したら小山さんは `area: "全国"` / `matching_types: []`。
  エリア5ページもタイプ6ページも**全部0名**だった（本番HTMLでも「0名掲載中」を確認）
- 件数で判定して0件なら出さない形にしたので、**掲載が増えれば自動で戻る**。手動作業は要らない
- ページ側と sitemap で判定を共有するため `AREA_MAP` を `src/lib/talk-areas.ts` に切り出した

**③ `/note/weather/[slug]` 20本を `noindex, follow` に**
- 本文が**約400字**しかなく、中身は紐づくコラム本体へのティーザー。検索意図も重複していた
- `follow` は残したのでコラム本体へのリンクは引き続き辿られる
- 副産物：`weatherDescriptions.ts` の `body_essence` / `body_scenes` / `body_science` が
  **5件分だけ存在してどこからも描画されていない**（死にデータ）。
  厚い解説ページに方針転換するならこの資産が使える

**④ `/columns` の「もっと見る」がリンクではなくボタンで、15本が深さ3に沈んでいた**
- これが今回いちばん効く修正。`ColumnsClient.tsx` の「すべて」表示は
  各カテゴリ先頭6件（`SECTION_PREVIEW_COUNT`）しかカードを描画せず、
  `CategorySectionHeader` の `onSeeAll` が **`setActiveCategory()` を呼ぶだけの `onClick`**。
  クローラーから見るとリンクが存在しない
- 「気持ちの整理」22本のうち16本が一覧から辿れず、他記事の関連記事欄からしか行けない状態だった
- プレビュー6枚のグリッドは変えず、その下にテキストリンク一覧（`.kv-more-links`）を追加

### 測り方（次も同じ手が使える）

推測で「内部リンクが弱そう」と言わずに済んだのは、**ビルド成果物を直接解析した**から。

```
npm run build
→ .next/server/app/**/*.html（110ページ分の実HTML）が出る
→ href="/..." を正規表現で拾ってリンクグラフを作る
→ "/" から BFS してクリック深度を出す
```

修正前後の実測：

| | 深さ1 | 深さ2 | 深さ3 | `/columns` から直接リンク |
|---|---|---|---|---|
| 修正前 | 3本 | 20本 | **15本** | 23 / 38 |
| 修正後 | 3本 | 35本 | **0本** | **38 / 38** |

孤立ページ（被リンク0）は修正前も0本だった。**「孤立」ではなく「深さ」が問題**という切り分けが、
この計測をやって初めてついた。送信URLは 103 → 73 本。
...
- 本番HTMLは 60〜80KB あってツール出力の上限を超える。保存されたファイルを
...
- **カテゴリアーカイブ + ページネーション（＝大手の標準構造）は今回やらない**。
  78本が未クロールで渋滞しているところに新規URLを6本足すのは順序が逆。
  **新規URL 0本**で深さを解消できるテキストリンクを先に採った。
...
## 2026-09-11（Claude Code セッション：about の紹介文、Kinda story 追加、毎朝リマインド、IG運用の着手）

### 完了

**`/about` のふうか紹介を一問一答にした（PR #42・本番反映済み）**
- 3段落のうち1段落が丸ごと「なぜ Kinda をつくったか」で、`/about/founder` の
  インタビューと重複していた。削ってリンク側に任せ、リンク文言も
  「Kinda をつくった理由を読む」に変えて行き先を明示した
- 経歴の羅列だと人柄が出ないため、雑誌の巻頭インタビュー形式（6問の一問一答）に。
  `dl` / `dt` / `dd` でマークアップしている
- **結婚相談所の勤務年数を4箇所から削除**。本文だけ伏せても意味がないため、
  `/about` の JSON-LD、`/about/founder` の metadata と Article 構造化データも同時に。
  ファウンダーストーリーの meta description は検索結果のスニペットに出ていた
- 前職は固有名を出さず「車を売るというよりコンセプトを見せるための場所」と性質で書いた

**Kinda story に Y.K さん（40代・活動中）を追加（PR #43・本番反映済み）**
- 「人数を増やすよりいま会っている方を深く知る時間にしたい」が、相談所の
  「並行が標準・数をこなす」の常識を外していて、この物語でいちばん強い
- 相談所名は同意範囲外。`counselorId` / `agencyId` は 0、名前は空文字
- うまくいっている話は読む人を追い詰めうるので
  「これが誰にでも起きることだとは思っていません」を本文に残した
- タグに性別を入れていない（素材に記載がないため）
- 初稿は13段落に細かく割って読点も多く、指摘を受けて**7段落に統合**した。
  一文で言い切れるところは区切らない。次からこの密度で書く

**コラム「お見合いの会話、何を話す？」を本番へ（PR #41）**
- 週次集計 SQL・28週連載の設計・IG戦略ドキュメントが同じ PR に同梱されていた
- PR 本文が警告していた「tip が docs のみで本番ビルドがスキップされる」件は、
  **squash マージで `.mdx` が同じコミットに入ったため発生しなかった**

**毎朝の発信リマインドを Routine 化**
- 朝7:00 JST に通知（push＋メール）。SNS投稿・コラム・Kinda story・Kinda act の
  4つを見て「今日やること」を1つだけ出す
- Kinda act は締切が無く後回しになるため、**他と並んだら優先して選ぶ**ルールを明記
- 作成時に「Routine に connector を渡せない」警告が出たため、Notion が無い場合は
  リポジトリ側だけで提案するフォールバックをプロンプトに入れてある
  （その後ふうかが claude.ai の Routines 画面で接続したため現在は Notion を読める）

**X 投稿カレンダーを 9/12〜9/30 まで毎日埋めた（16件）**
- 投稿時間は過去実績から柱ごとに決めた。B 相談所リアル=朝、A 天気の言葉 / C 問いかけ /
  F 中の人=夜、D つくる日記 / E 告知=昼
- 9/29 は連載第3週 `l3-area`。`ask` は `topics.ts` からそのまま引用

**Instagram の運用に着手**
- `docs/sns/ig-foundation-2026-09.md` を追加。§9 で未作成だった
  ピン留め②「Kinda を開くと何があるか」・③「連載の入口」の原稿、bio の二層構造、
  ハイライト、画像トーンの固定値
- IG カレンダーを 9/12〜9/29 の週3固定（火=連載／木=つくる日記／土=リール）で埋めた
- リールは1秒目に `topics.ts` の `ask` をそのまま置く。世界観・ロゴは1枚目に置かない。
  「プロフィールのリンクから」CTA は全削除し保存誘導に統一

### ハマったこと・判断

**ドキュメント2つで連載の曜日が食い違っていた**
- `series/kinda-pair-28.md`（9/10）は IG 連載を**木曜20:00**、
  `ig-strategy-2026-09.md`（9/11）は**火曜20:00**（木曜はつくる日記）と書いていた
- Notion の下書き2件は古いほうに従って木曜に入っていた。**新しい戦略に合わせて
  火曜へ移した**。X の連載が既に火曜なので、2媒体が同じ日に同じ話題で並ぶ形になる
- 教訓：設計ドキュメントを更新したら、先に投入済みの Notion 行も同時に直す

**この環境から `kinda.jp` に直接アクセスできない**
- 外向き通信ポリシーでブロックされる（`CONNECT tunnel failed, 403`）
- 本番確認は `mcp__Vercel__web_fetch_vercel_url` でデプロイ URL を取得して行う。
  同じビルドなので内容は同一。curl のループは無言で失敗し続けるので注意

**Vercel の production ビルドが CANCELED でも問題ないケースがある**
- PR #42 のビルド中に #43 をマージしたため、Vercel が古いほうを打ち切って
  新しいコミットでビルドし直した。最新ビルドは両方を含む main から作られている
- CLAUDE.md §10 が警告する「docs のみでスキップ」とは別物。commit の中身を見て判断する

### 次にやること

- **Y.K さんの活動期間を差し替える**。`periodLabel` は「活動数ヶ月目」、
  `periodMonths` は `3` を仮置きしたまま本番に出ている（一覧の期間フィルタに使われる）
- 既存9本の story の句読点整理をするか決める。**寄稿ものは本人が書いた文**なので
  勝手に直せない
- IG の bio 更新は連載開始と同日（9/15）に。先に bio を直してから初回を出す
- 9/29 の連載リプ2に置くコラムが無い（住む場所の話題に合う記事が既存38本に無い）
- 10月分の X / IG カレンダーを9月末に作る
- `ig-strategy-2026-09.md` §11「繰り返し登場する顔を作るか」は**保留で決裁済み**。
  週3固定を3〜4週回して3秒視聴継続率が動くか見てから再検討する
