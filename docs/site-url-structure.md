...
### sitemap の `lastmod` 方針 ★（2026-08-15 に実害が出た）

**実際の更新日を持たないページに `lastmod` を付けない。**

- 以前は `sitemap.ts` が実更新日を持たないページに `new Date()`（ビルド時刻）を入れており、
  **92 URL 中 56 URL の lastmod がデプロイのたびに「今日」**になっていた
- Google のガイダンスは「正確な lastmod を出せないなら省略する」。信頼できない lastmod は
  **サイト全体で無視される**。実際 Search Console のサイトマップ最終読み込みが
  **2026-07-17 で約1か月止まり**、検出 URL も 81（実際は 92）のままだった
- 修正後：lastmod を持つのは**コラム36本だけ**（`updatedAt` → `publishedAt` の順）。
  「今日」の日付は0件。ふうかの再送信で検出 URL が 81 → 92 に更新されたことを確認済み

> 新しいページ種別を sitemap に足すとき、**その場しのぎで `new Date()` を入れないこと。**
> 実日付を持てないなら lastmod は付けない。ページ単位の更新日を持てるようになったら復活させる。

### Search Console の運用メモ

- プロパティは**ドメインプロパティ**（`sc-domain:kinda.jp`）。www 有無・旧 `my-app-rp9u.vercel.app`
  も同じ土俵で見られるため、**入口ページには必ず自己参照 canonical を置く**
  （2026-08-15 にトップ・`/columns`・`/kinda-story` の抜けを修正）
- **「ページ」レポートは2〜3日遅れ**。インデックス登録リクエスト直後に数字が動かないのは正常
- 登録リクエストは「優先クロール待ち行列に入れる」だけで、保証も高速化もされない
- 「ページのリダイレクト」3件は `/note` `/search` `/partners` の意図的リダイレクト。**放置で正しい**
- サイトマップの再読み込みは、GSC → サイトマップ → 「新しいサイトマップの追加」に
  `sitemap.xml` と入れて**再送信**するだけでよい（登録済みでも可）
...
| 2026-08-15 | sitemap の lastmod を実日付だけにする方針を明記（実害が出たため）。Search Console の運用メモと、入口ページの自己参照 canonical を追記 |
