...
### 4.【重大・要設計判断】slots テーブルの UPDATE が全ログインユーザーに無制限

- ポリシー `slots_update_authed` が `USING(true) WITH CHECK(true)`。**一般会員のアカウントがあれば、全カウンセラーの全スロットを直接 UPDATE できる**（例：全部 `booked` にして予約を止める、他人の `locked` を `open` に戻して横取りする等）。Supabase アドバイザーでも WARN 検出。
- ただちに修正しなかった理由：予約フロー（ユーザーサイトの booking・`src/lib/reservations.ts` のスロット解放、counselor カレンダー、admin スロット管理）が**クライアントからの直接 UPDATE に依存**しており、ポリシーを絞ると予約動線が壊れる。
- 推奨対応（別セッションで計画的に）：
  1. スロットの状態遷移（lock / book / release）を SECURITY DEFINER RPC（`lock_slot_rpc` 等：呼び出し元・予約との整合を検証）に移す。CLAUDE.md §6「状態遷移は RPC 経由のみ」の原則どおり。
  2. その後 `slots_update_authed` を削除し、counselor / agency owner / admin スコープの UPDATE ポリシーへ置換。
