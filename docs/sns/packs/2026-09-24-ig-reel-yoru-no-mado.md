...
- ズームしない。**尺は7秒**・ループさせる。音源は §9（のほほんはことさん専用だったため取り消し）
- カバーは1カット目（＝この1枚）
...
## 9. 音源（2026-09-23 夜・訂正）

**のほほんは使わない。** ことさん専用の曲だった（`ig-strategy` §14 の訂正）。夜の絵にも合わない（ふうかさん指摘）。

**選ぶ基準**（IG の音源ライブラリで、ふうかさんが耳で選ぶ）

| 基準 | 理由 |
|---|---|
| **歌なし**（インスト） | 画面は「探す」時間。歌詞があると耳が言葉を追って、窓を見る目が止まる |
| **ゆっくり**（体感で 60〜80 BPM） | 夜・逆光の静けさに合わせる。明るい跳ねるリズムは合わない |
| ピアノ／アコースティックギター／オルゴール／やわらかい lo-fi | クレイの手ざわりと合う音色 |
| **悲しい短調に寄りすぎない** | 12番の消灯も「いない夜も並列」。寂しさを煽ると評価になる |
| 頭の7秒がきれい（ループで継ぎ目が気にならない） | 7秒でループさせるため。IG で使う区間を選べるので、静かな入りの区間を選ぶ |
