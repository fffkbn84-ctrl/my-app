...
- セルフリプ1：`https://kinda.jp/kinda-pair/topics`（28件の聞き方を全部そのまま公開しています）
- セルフリプ2：`https://kinda.jp/columns/omiai-kaiwa-wadai`（お見合いの会話の組み立て）
  - ⚠️ **「会う頻度・一人の時間」に正面から当たるコラムが38本の中に無い。**
    9/29 の「住む場所」と同じく、コラムの空きとして記録する（`sns-operations-2026-09.md`）
