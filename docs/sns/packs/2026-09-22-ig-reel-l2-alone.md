...
毎週火曜、ふたりの話題をひとつずつ置いていきます。

#結婚相談所 #婚活 #仮交際 #婚活中の人と繋がりたい #結婚相談所選び
```

- **最初の2行が「続きを読む」の前に出る。** そこだけで意味が通るように書く
- **URL も「プロフィールのリンクから」も置かない。** IG のキャプションのリンクは踏めないうえ、
  入れた瞬間にキャプションが広告になる（`iinikui-kimochi.md` §3 の実績）。
  28件公開していることは5カット目が言っているので、キャプションで繰り返さない
- 3行目は**連載の約束**（プロフィール2行目と同じ文）。フォローの理由をここに置く。
  この1行は毎週変えない
- ハッシュタグは5個・絵文字なし

> 初稿は3行目に `kinda.jp/kinda-pair/topics` を置いていたが、第1週の実績（リード2行＋ハッシュタグ）と
> 「導線文を入れない」線に合わせて落とした（2026-09-22）。
