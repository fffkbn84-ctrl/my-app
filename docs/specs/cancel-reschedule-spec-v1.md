...
> ⚠️ **【一部廃止】2026-06-05 の返金モデル改定前のドキュメント。** 本書の前提である「24時間キャンセル猶予・猶予内/日程変更は返金」は**現行モデルでは廃止**。
> 現行は **予約確定で即時課金・以後は原則返金なし（例外は運営の個別判断）**（CLAUDE.md §12）。
> 以下の §1 ポリシー表および 24h 猶予・自動返金を前提とした記述は無効。状態遷移・UI 設計のうち返金ロジックに依存しない部分のみ歴史的参考とすること。
>
> 作成日：2026-05-30
