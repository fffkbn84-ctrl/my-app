...
## 作業時に常時意識する5原則（2026-05-04 追加）

すべての画面追加・修正で以下を自動的に守る。コードレビューの基準でもある。

### 1. アクセシビリティ（a11y）
- SVG アイコンには `aria-hidden="true"`、ボタン・リンクに `aria-label` を付ける
- テキストと背景のコントラスト比 4.5:1 以上を維持（大きい文字は 3:1）
- キーボード操作で全機能に到達可能。`:focus-visible` のフォーカスリングを消さない
- `role` / `aria-current` / `aria-valuenow` 等の意味付けを正しく使う
- 自動再生・自動遷移は避ける。アニメーションは `prefers-reduced-motion` を尊重

### 2. プライバシー・信頼
- 個人情報を扱う画面（ログイン・予約・口コミ投稿等）は「何を集め、なぜ、どう守るか」を一言添える
- 外部サービスへ送信する前にユーザー同意を得る（同意なき自動送信禁止）
- フォームの入力データを不要にローカル保存しない
- `.env.local` に置くべき機密値をソースコードや UI に出さない

### 3. パフォーマンス
- 画像は WebP 化し、`next/image` で表示。LCP 候補画像には `priority` を付ける
- フォントは `display: swap`、Google Fonts は preconnect で先読み
- クライアントコンポーネント（`'use client'`）を不要に増やさない（Server Component が原則）
- 大きいライブラリは `dynamic import` で遅延ロード
- Core Web Vitals の目標：LCP < 2.5s / INP < 200ms / CLS < 0.1

### 4. インクルーシビティ・ブランドの誠実さ
- 「結婚」「夫」「妻」を主役にしない。「ふたり」「好きな人」「一緒に過ごす日々」を使う
- 画像・SVG アバターは年齢・性別・体型・髪型の多様性を意識
- LGBTQ+ も自然に使える文言設計（「異性」「男女」を当たり前にしない）
- 「自分のため」「自分のペース」を肯定する。焦らせない。比較しない
- 「診断」「結婚活動」「成婚」等の医療・業務ワードはフロントに極力出さない（v2-patch 方針）

### 5. 保守性
- inline style より CSS クラス（プレフィックス `.kt-` `.ktl-` `.ktq-` `.ktr-` 等で衝突回避）
- ハードコード色禁止。必ず `var(--accent)` `var(--ink)` 等の CSS 変数経由
- 3 回以上使うパターンはコンポーネント化
- 1 ファイル 600 行を超えたら分割を検討
- TODO コメントは必ず「いつ・誰が・何を」で書く
- TypeScript の `any` は原則禁止。必要な箇所はコメントで理由を明記
...
## 実装済み機能（2026-05-08 追記） — Phase A ヒーロー再設計 + Kinda 各ページ修正バンドル

> ブランチ: `claude/implement-kinda-talk-uDUoW`（唯一の作業ブランチ）
> Vercel プロジェクト: `my-app-rp9u`（preview branch alias: `my-app-rp9u-git-claude-implement-291d77-fffkbn84-4095s-projects.vercel.app`）

### 1. Phase A — トップページヒーローの垂直分離レイアウト再設計

**コミット:** `10bd7bb` `feat(hero): Phase A — ヒーロー垂直分離レイアウトに再設計`
**指示書:** `docs/phase-A-hero-redesign-instructions.md`

旧ヒーローはフルブリード画像 + 暗いグラデーションオーバーレイ + 白文字で、画像と文字が干渉して可読性が崩れていた。**ビジュアルとテキストを垂直に分離した4ブロック構造**に再構築。

| Block | 内容 | 背景 / 文字色 |
|---|---|---|
| 1 ビジュアル | 村のミニチュア画像のみ。文字オーバーレイ廃止。aspect 4:5、max-height 60svh | `#F5EEE6`（画像のみ） |
| 2 メインコピー | H1（好きな人を…）+ H2（カウンセラー × …） | ベージュ単色 / 濃ブラウン |
| 3 主CTA | 「言葉にならないモヤモヤを、60秒で。」 + Kinda note ボタン + マイクロコピー | ベージュ単色 |
| 4 副CTA帯 | 「やりたいことが決まっている方はこちら →」（後の修正で統合） | 一段濃いベージュ + 上下ボーダー |

**変更ファイル:**
- `src/app/page.tsx` — A セクションの JSX を 4 ブロック構造に書き換え
- `src/app/globals.css` — `.ktp-hero-*` クラス群を新レイアウト用に置換（`.ktp-hero-overlay` / `.ktp-hero-content` / `.ktp-hero-divider` 等の旧クラスを削除し、`.ktp-hero-visual` / `.ktp-hero-copy` / `.ktp-hero-cta-block` / `.ktp-hero-sub-block` を新設）

**デザイントークン:**
- 新規追加なし。既存の `--ink` / `--mid` / `--muted` / `--accent` / `--accent-deep` / `--accent-glow` / `--accent-shadow` / `--border` のみ使用

---

### 2. Phase A 仕上げ第1弾 — キャッチコピー濃ブラウン化 + 可読性 + ヒーロー内ロゴ削除

**コミット:** `ecc906a` `fix(hero): Phase A 仕上げ — キャッチコピー濃ブラウン化 / マイクロコピー可読性 / ヒーロー内ロゴ削除`
**指示書:** `docs/phase-A-finishing-touches.md`

| # | 修正内容 |
|---|---|
| 1 | `.ktp-hero-cta-tagline` の `color: var(--accent-deep)` → `var(--ink)`（暫定対応・後で第2弾で再変更） |
| 2 | `.ktp-hero-micro` の `color: var(--muted)` → `var(--mid)`（ベージュ背景で薄すぎたのを副CTA帯リードと同等の可読性に） |
| 3 | Block 2 内の Kinda ふたりへロゴ（`/images/toppage_name.PNG`）を削除。ヘッダーに常時ロゴが出ているので二重表示になっていた。`.ktp-hero-logo` / `.ktp-hero-logo img` の CSS も削除 |

---

### 3. Phase A 仕上げ第2弾 — キャッチコピーの「囁き」復帰 + 副CTA をページ内スクロールへ

**コミット:** `5f7a5ad` `fix(hero): Phase A 仕上げ第2弾 — キャッチコピーを囁きトーンに戻す + 副CTA をページ内 #section-b へ`
**指示書:** `docs/phase-A-finishing-touches-v2.md`

第1弾で濃ブラウンにしたキャッチコピーが H1 と張り合ってしまった反省を踏まえ、役割分担を再定義：

| 要素 | 役割 | トーン |
|---|---|---|
| H1 | ブランドの約束 | 強（濃ブラウン・Mincho 26〜28px） |
| サブコピー | サービス全容 | 中（mid・Sans 13〜14px） |
| キャッチコピー | ボタンを押す囁き | **弱・装飾**（accent + opacity .82・Mincho 15〜17px） |
| ボタン | ゴール | 最強（ローズゴールド + glow） |

**`.ktp-hero-cta-tagline` 全面再調整:**
- `color: var(--ink)` → `var(--accent)` + `opacity: .82`
- `font-weight: 500` → `400`
- `font-size: clamp(18px, 5vw, 20px)` → `clamp(15px, 4vw, 17px)`
- `margin: 0 0 18px` → `margin: 0 0 14px`

**副CTA の遷移先変更:** `/kinda-type` → `#section-b`（ページ内 4 カードセクション）。「やりたいことが決まっている」ユーザーが診断を求めているとは限らないため、4 つの選択肢が並ぶ B セクションに着地させるのが正解。

**スムーススクロール対応:** `globals.css` の `html` セレクタに `scroll-behavior: smooth` と `scroll-padding-top: var(--header-height, 56px)` を追加。`prefers-reduced-motion` ユーザーには `auto` に戻すフォールバックも設置。

---

### 4. 副CTA帯の統合 — 全幅クリック領域 + 文言を行動ベースに

**コミット:** `ef70555` `fix(hero): 副CTA帯を1つの全幅クリック領域に統合 + 文言を行動ベースに`

リード `<p>「やりたいことが決まっている方は」` + リンク `<Link>「自分に合う担当を見つける →」` の2要素構成は、タップ可能領域が分散して伝わりにくかった。**帯全体を 1 つの `<Link>` に統合**し、文言を **「やりたいことが決まっている方はこちら →」** に一本化。

- `.ktp-hero-sub-block` の `padding` を `<a>` 側に移譲（タップ領域を帯全体に拡張）
- `.ktp-hero-sub-link` を `display: flex; width: 100%; padding: 18px 24px` に変更
- hover/active のフィードバック強化：hover で `rgba(212,160,144,.08)` 背景 + opacity .85、active でさらに濃く
- `.ktp-hero-sub-lead` クラスは削除（リード `<p>` を削除したため不要）

---

### 5. Kinda note 結果画面 — 自由記述の表示／非表示トグル

**コミット:** `39deeca`（Kinda 各ページ修正バンドル）

最終問で書いた自由記述を結果画面に表示する `FreeTextSection` を新規追加。「結果に含める」チェックボックスで以下を一括制御：

| 状態 | 結果画面の表示 | ShareCard（画像保存） | コピーメモ |
|---|---|---|---|
| ON（デフォルト） | 自由記述カードを表示 | 含む | 含む |
| OFF | 「非表示にしています」メッセージに切替 | 除外 | 除外 |

**変更ファイル:** `src/app/kinda-note/result/ResultContent.tsx`
- `useState(true)` の `includeFreeText` フラグを追加
- `FreeTextSection`（同ファイル末尾の subcomponent）：白カード + 左 accent ボーダー、Hiragino Mincho italic 14px
- 既存 `handleCopy` の `buildMemoText` 呼び出し時に `freeTexts: includeFreeText ? freeTexts : {}` を渡す
- `<ShareCard>` への `freeText={includeFreeText ? firstFreeText : ""}` も同様

---

### 6. Kinda act / Kinda talk — フィルター UI の根本修正

**コミット:** `39deeca` → `8de583e`

#### 問題

`.kt-filter-scroll` には `mask-image: linear-gradient(... transparent 100%)`（右端フェード）が設定されている。この配下にエリアトグル（ドロップダウン付き）を入れると、iOS Safari でドロップダウンの末尾（都道府県セクション）が透明にフェードしてタップ不能になる。`overflow: visible` を被せた回避策は、今度は横スクロールを殺してしまう（Kinda talk のタイプフィルターが「下のおすすめ順／評価が高い順と違ってスライドできない」という症状）。

#### 解決策

**両ページで「エリア行を独立コンテナに切り出す」リファクタ**。`.kt-filter-scroll` の中にはピル類だけを残し、エリアトグルは外側の独立行（`padding: 0 16px` の単純な flex コンテナ）に配置する。

| ページ | Before | After |
|---|---|---|
| `/kinda-act` | カテゴリピル + エリアトグル を 1 つの `.kt-filter-scroll`（`overflow:visible` 上書き） | エリア行（独立）→ カテゴリピル行（`.kt-filter-scroll` で横スクロール可） |
| `/kinda-talk` | エリアトグル + タイプピル を 1 つの `.kt-filter-scroll`（`overflow:visible` 上書き） | エリア行（独立）→ タイプピル行（`.kt-filter-scroll` で横スクロール可） |

#### Kinda act の追加修正：ドロップダウンが画面左外にはみ出す

エリアトグルが行頭に移動したことで、ドロップダウンの旧アンカー `right: 0; left: auto` が画面**左外**に展開される問題が発生。`left: 0; right: auto` に変更してトリガー右側に展開するように修正。

**変更ファイル:** `src/app/kinda-act/KindaActClient.tsx` / `src/app/kinda-talk/KindaTalkClient.tsx`

---

### 7. サンプルカウンセラーのプロフィール詳細ページ対応

**コミット:** `39deeca` → `8de583e`

#### 問題1: numeric ID 101〜105 で 404

`/counselors/[id]` の hardcoded `counselors{}` には ID `"1"〜"6"` しかなく、demo COUNSELORS（id: 101〜105）でクリックすると `notFound()` で 404 になっていた。

#### 問題2: Supabase に seed 済みの demo は UUID

実は本番では Supabase にデモカウンセラーが seed されており、ID は auto-generated UUID（`073af60f-...` など）。`counselors["073af60f-..."]` は当然マッチしない → mockCounselor undefined → notFound → 404。

#### 解決策

1. **ID 101〜105 の demo エントリを `/counselors/[id]/page.tsx` の `counselors{}` に追加** — bio / メッセージ / 料金プラン / 資格・診断タイプを含む完全なプロフィールをサンプル明示の文言付きで掲載
2. **UUID URL からのフォールバック**：`supabaseCounselor` が見つかったが mockCounselor がない場合、`Object.values(counselors)` を name で検索してサンプルプロフィールに橋渡し
3. **`CounselorReelModal` の demo ガード撤去**：「プロフィールを見る」リンクから `preventDefault` + DemoNoticeModal を削除。デモ表示込みで「ここまでがサンプルです」と通しで見せられるよう統一
4. **avgRating の安全弁**：口コミ 0 件で NaN になるのを `counselor.rating` フォールバックで防止

**変更ファイル:** `src/app/counselors/[id]/page.tsx` / `src/components/kinda-talk/CounselorReelModal.tsx`

---

### 8. 新人カウンセラー（経験年数 1 年未満）の自動バッジ付与

**コミット:** `8de583e`

ふうかさんの確認事項：「経験年数1年未満のカウンセラーには新規だと分かるタブが自動で付与される仕組みで合ってる？」→ **以前は仕組みが無かったので新規実装**。

**新規ファイル:** `src/components/kinda-talk/NewBadge.tsx`（`新人` ラベル）

**`globals.css` 追加:** `.kt-new-badge` クラス（accent カラー塗り）。サンプルバッジ（中立トーン）と区別するため、新人バッジは accent 色塗り。

**`counselor.experience < 1`（詳細ページは `counselor.yearsExp < 1`）で自動表示する 3 箇所:**
- `CounselorReelCard`（kinda-talk のリール一覧カード右上、demo バッジと並べて）
- `CounselorReelModal`（フルスクリーンモーダル内、名前の隣にインライン表示）
- `/counselors/[id]` ヒーロー（名前の隣にバッジ表示）

現在の在籍カウンセラー（mock）と demo seed の経験年数は 5〜15 年なので、すぐには表示されないが、新規入会のカウンセラー（experience: 0）が登録された瞬間から自動でバッジが付く。

---

### コミット履歴サマリ（このセッションでの作業順）

| コミット | 概要 |
|---|---|
| `10bd7bb` | Phase A ヒーロー垂直分離レイアウトに再設計 |
| `39deeca` | Kinda note 自由記述トグル + Kinda act/talk フィルター UI 修正 + サンプル ID 101〜105 を /counselors/[id] に追加 |
| `8de583e` | Kinda act ドロップダウン左寄せ + サンプル UUID 対応 + 新人バッジ NewBadge 新設 |
| `ecc906a` | Phase A 仕上げ第1弾（キャッチコピー濃ブラウン化・マイクロコピー可読性・ヒーロー内ロゴ削除） |
| `5f7a5ad` | Phase A 仕上げ第2弾（キャッチコピー囁きトーン復帰・副CTA を #section-b へ・スムーススクロール） |
| `ef70555` | 副CTA帯を全幅 1 リンクに統合（文言：「やりたいことが決まっている方はこちら →」） |

### 関連ファイル（このセッションで触った主なもの）

- ヒーロー: `src/app/page.tsx` / `src/app/globals.css`（`.ktp-hero-*` クラス群と `html` の scroll-behavior）
- Kinda note: `src/app/kinda-note/result/ResultContent.tsx`
- Kinda act: `src/app/kinda-act/KindaActClient.tsx`
- Kinda talk: `src/app/kinda-talk/KindaTalkClient.tsx`
- カウンセラー詳細: `src/app/counselors/[id]/page.tsx`
- リールカード/モーダル: `src/components/kinda-talk/CounselorReelCard.tsx` / `CounselorReelModal.tsx`
- 新規: `src/components/kinda-talk/NewBadge.tsx`
- 設計書: `docs/phase-A-hero-redesign-instructions.md` / `docs/phase-A-finishing-touches.md` / `docs/phase-A-finishing-touches-v2.md`

### 既知の残課題・次フェーズへのメモ

- **Phase B（後半セクション補強）と Phase C（SEO・速度）** は別タスク（Phase A 指示書冒頭で明記）。今回のスコープ外
- **production target deploy（main ブランチ）** にはまだ統合されていない。最新動作は preview の branch alias で確認可能
- ヒーロー画像の `priority` + `fetchPriority="high"` + `<link rel="preload">`（layout.tsx）の組み合わせで LCP は維持。Lighthouse 計測は天気アイコン WebP 化が完了したタイミング（前セッション課題）と合わせて再実施推奨

---

## 実装済み機能（2026-05-09 追記） — Kinda note セクション改善 + ブランド一貫化大刷新

> ブランチ: `claude/implement-kinda-talk-uDUoW`
> このセッションでは Kinda note セクションの細部修正から始まり、ブランド体系（Kinda シリーズ）の一貫した認知強化、`/about` ページの大幅刷新まで一気に進めた。最終コミット: `a7ccce5`

---

### A. ホーム Kinda note セクション改善（v2 仕様）

**ファイル:** `src/app/page.tsx`、`src/app/globals.css`

#### A-1. 説明文の置き換え + 天気アイコン浮遊アニメ
- 5アイコン直下の説明文を「**並んでいるのは、ほんの一部。/ あなたの天気は、20の中にあります。**」に変更（Georgia serif / 14px / `var(--muted)` / line-height 1.9 / margin: 24px 0 32px / 詩的余韻）
- 5枚の天気アイコンに `weather-icon-float` 浮遊アニメ追加：
  - `@keyframes weather-icon-float`：3s ease-in-out で `translateY(0)` ↔ `translateY(-6px)`
  - delay を `${i * 0.6}s` でインライン適用（0 / 0.6 / 1.2 / 1.8 / 2.4s）
  - `prefers-reduced-motion: reduce` で停止
  - 日本語ラベルは動かさない（画像コンテナにのみ適用）

#### A-2. 「Kinda note は、こんな時に使えます」セクション完全削除
- セクション全体（見出し・8項目・締め文・CTAボタン）を丸ごと削除
- 未使用となった `KN_USECASES_PAUSE` / `KN_USECASES_MOVE` 定数も削除
- `globals.css` の `.kn-usecase-h3` / `.kn-usecase-box` / `.kn-usecase-list` も dead code として削除
- 8項目はコラム化予定（本セッション範囲外）

#### A-3. ヒーロー H2 改行修正
- `「あなたの気持ちは、いま、どんな天気？」` → 2行で明示的に改行：
  - `あなたの気持ちは` / `いま、どんな天気？`
  - `<br />` で iPhone 16 Pro の不格好な折り返しを解消

---

### B. SectionDivider コンポーネント新規作成

**新規ファイル:** `src/components/ui/SectionDivider.tsx`

トップページ B / B' / C / C' / D が同じ `#FEFCFA` 背景で連続して目が止まらない問題を解消。

**仕様:**
- 中央：**ぷっくり丸み・もちもち感のあるハート SVG**（Kindaロゴ風）
  - radial gradient で 3D 光沢（`#F4CFC0` → `#DCA897` → `#B0866F`）
  - 上部左に白の楕円ハイライトを2重に重ねて「もちもち」質感
  - 完全透過 SVG（旧 `ornamental-heartwopal.webp` は VP8 でアルファ無し → 灰色背景が出てしまうため SVG に置換）
- 両サイド：`var(--light)` 細線（max-width 320px 中央寄せ）
- `aria-hidden="true"` 装飾扱い
- `ornament` prop で `heart` / `starfish` 切替可（シーズン演出用に残してある）
- デフォルトサイズ 44px

**配置:** `B / B' / C / C' / D` の各セクション冒頭に `<SectionDivider />`

**重複削除:**
- `SectionLabel` コンポーネントの上下40px線を削除（C / C' 適用）
- B セクション「やりたいことが決まっている方へ」の左右 flanking lines を削除
- 「線が連続して見える」冗長性を解消し、SectionDivider を境界の一本化担当に

---

### C. Bパターン統一（Kinda シリーズの認知強化）

> ふうかさんの方針：「Kinda の仕組みを覚えてほしい・サイト内の役割として馴染んでほしい」
> サイト全体で `Kinda + xxx`（Mincho + DM Serif italic accent）の書体ルールを反復させてシリーズ認知を強化。

**書体ルール（全サイト統一）:**
```
Kinda  ← Shippori Mincho 500
xxx    ← DM Serif Display italic、サービス固有のaccent色
```

#### C-1. ホーム A' / B' のセクションラベル格上げ
- **A' Kinda note 説明セクション**:
  - `KINDA NOTE`（DM Sans 11px uppercase eyebrow）→ `Kinda note`（Mincho 18px + italic accent `#D4A090`）
  - H2「あなたの気持ちは / いま、どんな天気？」は据え置き（詩的フック温存）
- **B' カウンセラーカルーセル**:
  - `Kinda talk より`（DM Sans 11px uppercase eyebrow）→ `Kinda talk`（Mincho 18px + italic accent `#B89A4A`、「より」削除）
  - H2「今いるカウンセラーたち」は据え置き
- **C ふたりの物語**: Cパターン（日本語主役）のまま据え置き — 詩的な余韻を保つため意図的に例外

#### C-2. サブページの重複 eyebrow 削除
- `/kinda-talk` / `/kinda-act` / `/kinda-glow` のヒーローで、`kt-hero-eyebrow` と H1 の両方が「Kinda talk/act/glow」と表示する**冗長**を解消
- 全3ページから `<div className="kt-hero-eyebrow">Kinda XXX</div>` を削除
- `/kinda-type` / `/kinda-note` は Cパターン（Japanese-led H1）だが、URL とブレッドクラムで Kinda XXX が明示済みのため現状維持

#### C-3. Kinda talk のコピーをポジティブに振り直し
> 英語 idiom "my kinda guy/type" =「私の好みの〜」のニュアンスを日本語側で表現。カウンセラー選びは高スタンスな決定なので「なんとなく話す」より「ぴったりの担当者と出会う」方向で訴求。

3か所更新:

| 場所 | Before | After |
|---|---|---|
| ホーム DECIDED_CARDS | カウンセラー・相談所を直接見る | **ぴったりのカウンセラーに直接会う** |
| /about Kinda のしくみ | カウンセラー・相談所を直接見る | **ぴったりのカウンセラーに直接会う** |
| /kinda-talk hero sub | 相談したい・迷っている | **ぴったりの担当者を、見つける** |

---

### D. `/about` ページ大幅刷新

**変更ファイル:** `src/app/about/page.tsx`、`src/app/globals.css`

#### D-1. ヒーロー語源注記の追加と拡張
ヒーロー詩的セクション末尾「その瞬間のために、Kinda ふたりへ」の下に語源注記を追加：

```
Kinda — 英語で「なんとなく」。
そして "my kinda" と言えば、「私にぴったりの」。
決めなくていい余白と、しっくりくる感覚から、
ふたりは始まる。
```

スタイル: Mincho italic / `rgba(255,255,255,.55)` / clamp(13-15px) / 詩的トーン
**ブランド名 Kinda の二重の意味**（なんとなく＋私好みの）を統合してユーザーに腑落ちさせる。

#### D-2. 「Kinda のしくみ」セクション新規追加（⑤と⑥の間）
**section ⑤'**: KINDA SERIES eyebrow + 「5つの Kinda、ひとつの場所で。」H2 + リード文 + 5サービスカード（2列グリッド）

各カードの色（B カードと完全に同期）:

| Kinda | desc | bg | accent |
|---|---|---|---|
| note | 言葉にならない気持ちを整理する | `#FAF1EB` | `#D4A090` |
| type | 自分に合うカウンセラーを見つける | `#E0ECF8` | `#5A7FAF` |
| talk | ぴったりのカウンセラーに直接会う | `#FAF3DE` | `#B89A4A` |
| act | お見合いやデートに使いやすい場所 | `#F5E1E0` | `#B86E68` |
| glow | 好きな人に会う前に、自分を整える | `#EDE0F4` | `#8A66B0` |

**カードのタップ可能感UP:**
- minHeight 124px で整列確保
- 矢印 `→` を右下に追加（accent色・SVG 16px）
- box-shadow `0 2px 10px rgba(180,140,90,.10) + inset 0 1px 0 rgba(255,255,255,.5)`（クレイ感）
- border `.04` → `.06`

#### D-3. 4点修正

**① ABOUT FUTARIVE → ABOUT US**
旧ブランド名「futarive」を撤去。

**② OUR BELIEF 4タイトルの語尾を動詞・宣言形に統一（マニフェスト調）**

| # | Before | After |
|---|---|---|
| 01 | カウンセラーは、人で選ぶ**べきだ** | カウンセラーは、人で**選ぶ** |
| 02 | 口コミは、体験した人だけが**書ける** | 口コミは、体験した人だけが**書く** |
| 03 | 婚活の孤独を、一人で抱え**なくていい** | 婚活の孤独は、ひとり**にしない** |
| 04 | ふたりの時間に、いい場所**を** | ふたりの時間に、いい場所を**渡す** |

**③ チームアバターの刷新**
線画SVGの髪型描写は廃止。**パステルグラデーション円 + DM Serif italic イニシャル（F R T M A S）** に統一。各メンバーのカラー（ベージュ・セージ・ブルー・ペールブルー・ピンク・パープル）は維持して "その人らしさ" の手がかりは残す。

**④ Kinda 村のクレイ風背景を全体に適用（Kinda act パターン）**
- `<div className="about-village-bg" aria-hidden="true" />` を `/about` 最上位に追加
- `globals.css` に `.about-village-bg` 定義
  - 画像: `/images/Toontown-background.webp`（main から取り込み）
  - `filter: blur(5px) brightness(1.06) saturate(1.18)` / `transform: scale(1.04)`
  - `::after` で薄い暖色クリームの被せ
- main を transparent にして明るいセクションは rgba 半透明背景に：
  - pale section: `rgba(240,238,235,.55)`
  - white section: `rgba(255,255,255,.55)`
  - cream（旧bg無し）section: `rgba(255,253,247,.5)`
- 暗いセクション（hero / OUR BELIEF / 締めCTA）は **`#231A12`**（深い温かみのあるダーク = 村の影色、純黒 `var(--black)` から差し替え）で詩的ドラマを保つ
- アイブロウ `fontWeight: 200` → `400` に統一して村背景上でも読めるように

#### D-4. WHAT YOU CAN DO の3アイコンをアップグレード
村背景上で 48px 円・15% opacity・1.3px stroke だと完全にかすんでいた問題を解消：
- サイズ: 48 → **56px**
- 背景: 15% 半透明 → **パステルグラデーション**（団体アバターと同じ書法でブランド一貫）
- ストローク色を半トーン濃く / 幅 1.3 → 1.5
- box-shadow + inset highlight でクレイ感

| アイコン | 背景グラデ | ストローク色 |
|---|---|---|
| 担当者を自分で選ぶ | `#F4E5CC → #E0CDA8` | `#A88858` |
| ふたりのお店を見つける | `#DCE8DA → #B4CDB6` | `#5A8068` |
| 孤独じゃない婚活を | `#DAE3F0 → #B6C4DC` | `#4F76A8` |

#### D-5. 締めCTAをヒーローと同二段構えに統一
旧: btn-wh「カウンセラーを探す」+ btn-gl「ふたりのお店を探す」
新:
- **主CTA**: 「**いまの気持ちを整理する →**」 → `/kinda-note`（ローズゴールド `#D4A090` + glow shadow / ヒーロー主CTAと完全統一）
- **副CTA**: 「**カウンセラーを見てみる →**」 → `/kinda-talk`（ghost テキストリンク・白70%）

Kinda note を主役に押し出す戦略をヒーローと締めで反復。

---

### E. ホーム D セクションを `/about` ティザーに刷新

旧コピー（既存のレビューサイトは… / Kinda ふたりへは… / 不安なまま相談所に飛び込まなくていい）→ `/about` ヒーローと同期した圧縮詩的版へ刷新：
...
その瞬間のために、Kinda ふたりへがいます。

Kinda — 英語で「なんとなく」。
そして "my kinda" と言えば、「私にぴったりの」。
```

末尾に CTA「**Kinda ふたりへについて、もっと知る →**」を配置 → タップで `/about` の詳細へ遷移。`.btn-outline` 風の控えめな ghost ボタン。
...
| コミット | 概要 |
|---|---|
| `f7a4563` | Kinda note 説明セクションのテキスト改善＋天気アイコン浮遊アニメ / 「こんな時に使えます」削除 |
| `41727ca` | セクション間にクレイ風ハートのSectionDivider配置 |
| `2bf272f` | SectionDivider のハート画像クロップ + 重複線削除（後に SVG 化） |
| `90ccd28` | ハートをSVG化（もちもち Kinda ロゴ風） |
| `f6eebdc` | A' / B' セクションラベルを Bパターン統一（Kinda 主役化） |
| `856e982` | /kinda-talk /kinda-act /kinda-glow ヒーローの重複 eyebrow 削除 |
| `555526c` | /about に「なぜ Kinda？」一行 + 「Kinda のしくみ」セクションを追加 |
| `5e9d770` | Kinda talk をポジティブに振り直し + ホーム D を /about ティザーに刷新 |
| `bb8114d` | /about 4点修正（eyebrow / OUR BELIEF / チームアバター / Kinda村背景） |
| `f2c6554` | Toontown-background.webp 取り込み + CSS パス修正 |
| `1a42daa` | 4点調整（黒を温かいダークに / 村ぼかし弱く / 透過強く / アイブロウ可読性UP） |
| `075ad6b` | WHAT YOU CAN DO 3アイコンを村背景でも視認できるアップグレード |
| `a7ccce5` | Kinda series カードにタップ可能感UP + 締めCTAをヒーローと同型に統一 |

---

### 重要な書体・色のリファレンス（次セッション以降の参照用）
...
/* xxx 部分（DM Serif italic accent） */
font-style: italic;
font-family: 'DM Serif Display', serif;
color: <サービス固有色>;
```

#### Kinda サービスごとの accent 色（B カード / Kinda series カード共通）
- **note**: `#D4A090`（ローズゴールド）
- **type**: `#5A7FAF`（ブルー）
- **talk**: `#B89A4A`（ゴールド）
- **act**: `#B86E68`（コーラルピンク）
...
#### SectionDivider のハートグラデーション
`#F4CFC0` → `#DCA897` → `#B0866F`（radial gradient cx 38% cy 30% r 72%）

---

### 次セッションへの引き継ぎ

- 残課題候補：実機チェック / Kinda glow の本番コンテンツ / Supabase 連携 / Lighthouse 計測 / 体験談・コラム拡充など
