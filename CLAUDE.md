...
- 異議申立てに対する自動通知（プッシュ通知）— メールは 2026-05-21 に実装済み
- 支払いステータス（`paid_at` カラム）と請求 ID の紐付け

---

## 実装済み機能（2026-05-21 追記）

### J-2: 課金異議申立て 自動メール通知（Resend 連携）
...
```
カウンセラー側 /billing で異議申立て
  → submit_billing_dispute RPC
  → billing_events.status を 'disputed' に UPDATE
  → Supabase Database Webhook が発火
  → Vercel /api/webhooks/billing-disputed に POST
  → service_role で関連テーブル解決
  → Resend 経由で運営宛にメール送信
```
...
| ファイル | 内容 |
|---|---|
| `futarive-admin/lib/email.ts` | Resend SDK のラッパー（`sendEmail` 関数） |
| `futarive-admin/app/api/webhooks/billing-disputed/route.ts` | Webhook 受信 + メール送信 Route Handler |
| `futarive-admin/lib/config.ts` | `FRONTSITE_URL` 一元管理 |
...
| `ADMIN_NOTIFY_EMAIL` | 運営宛通知の送信先メール |
| `SUPABASE_SERVICE_ROLE_KEY` | webhook ハンドラーで RLS をバイパス |
| `SUPABASE_WEBHOOK_SECRET` | Authorization ヘッダーで Supabase Webhook を認証 |

#### メールに含める判定材料

- 相談所名・カウンセラー名・利用者名
- 予約日（reservations.created_at）
- 面談予定（reservations.start_at）
- 異議申立日（billing_events.dispute_at）
- 金額（amount_jpy）
- **相談所からの異議内容**（dispute_note）
- **利用者からの事前メッセージ**（reservations.notes）
- **相談所からの最後のメッセージ**（reservations.agency_message + agency_message_at）— 連絡なしの場合は警告表示
- 統括管理画面への CTA リンク

同じ情報を `/admin/billing` の解決モーダルにも表示する（modal-box に `max-height: 90vh; overflow-y: auto`）。

#### Supabase Database Webhook 設定

| 項目 | 値 |
|---|---|
| Name | `billing-disputed-notify` |
| Table | `billing_events` |
| Events | Update のみ |
| Type | HTTP Request |
| Method | POST |
| URL | `https://futarive-admin-git-claude-futari-fcf6db-fffkbn84-4095s-projects.vercel.app/api/webhooks/billing-disputed`（**branch alias**） |
| Headers | `Authorization: Bearer <SUPABASE_WEBHOOK_SECRET>` |

#### ⚠ main マージ時の Webhook URL 変更が必要

現在の Webhook URL は **branch alias**（`futarive-admin-git-claude-futari-fcf6db-...`）。これは `claude/futarive-admin-dashboard-iKBfw` 専用 URL。

**`main` ブランチにマージしたタイミングで、Supabase Webhook の URL を以下に変更すること**：

```
https://futarive-admin-fffkbn84-4095s-projects.vercel.app/api/webhooks/billing-disputed
```

（または、本番カスタムドメイン設定後はそれに置き換える）

これを忘れると、main マージ後に branch を削除した時に Webhook が 404 になる。

#### Vercel 設定上の注意

| 設定 | 状態 |
|---|---|
| **Deployment Protection: Vercel Authentication** | **Disabled** にしてある（外部 Webhook 受信のため） |
| **middleware.ts** | `/api/webhooks/*` を認証ガードから除外（先頭で early return） |

#### 設計上の落とし穴と回避策

| 落とし穴 | 対策 |
|---|---|
| Webhook 先 URL を Production URL（main 用）にすると、main に deploy が無い場合古い deploy にルーティングされる | branch alias の URL を使う |
| middleware.ts が `/api/webhooks/*` も認証ガードでリダイレクト → 405 Method Not Allowed | middleware 先頭で `pathname.startsWith('/api/webhooks/')` で early return |
| Vercel Deployment Protection（Vercel Authentication）が外部 POST を 401 で蹴る | Settings → Deployment Protection → Vercel Authentication を Disabled |
| service_role キーがないと billing_events 関連テーブルが RLS でアクセス不可 | `SUPABASE_SERVICE_ROLE_KEY` 環境変数を Vercel に追加 |
| Resend デフォルト送信元 `onboarding@resend.dev` は登録メールアドレス以外には送れない（無料プラン）| 運営通知用なら問題なし。ユーザー宛メール送信にはカスタムドメイン Verify が必要 |

---

### 送客料の正式表記：¥5,000 / 1件（成果報酬のみ）

利用規約・transparency・partners ページに合わせ、送客料を **10,000円 → 5,000円** に統一。
...
| DB トリガー `billing_events_create_from_reservation()` | 固定 INSERT 値 5000 |
| `futarive-admin/supabase/migrations/006_billing_events.sql` | コメント・DEFAULT・固定値 |
| `futarive-counselor/supabase/migrations/025_billing_events.sql` | 同上 |

#### counselor /billing 表記改善（Q2 案A+B ハイブリッド）

**ブランチ：** `claude/fix-profile-creation-1clpG`

- ページタイトル：「課金履歴」→「**Kinda 請求履歴**」
- サイドバーラベル：「課金履歴」→「**Kinda 請求履歴**」
- ページ冒頭に説明セクション追加（accent ボーダー pale 背景）:
