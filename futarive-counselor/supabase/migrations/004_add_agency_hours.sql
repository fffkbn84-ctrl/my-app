-- ============================================================
-- agencies テーブルに営業時間・面談可能時間・定休日・所要時間カラムを追加
-- ============================================================
...
alter table public.agencies
  ...
  add column if not exists default_slot_minutes      integer;      -- 1枠あたりの所要時間（分）
...
-- 所要時間は 15分単位の正の値のみ（任意）
alter table public.agencies
  drop constraint if exists agencies_slot_minutes_check;
alter table public.agencies
  add constraint agencies_slot_minutes_check
  check (
    default_slot_minutes is null
    or (default_slot_minutes > 0 and default_slot_minutes % 15 = 0)
  );
...
set
  ...
  default_slot_minutes = coalesce(default_slot_minutes, 60)
where consultation_start_time is null
   or consultation_end_time is null
   or default_slot_minutes is null;
