...
export default function ShopReviewForm({
  ...
  const id = shopId;
  const shop = { name: shopName, location: shopLocation, categoryLabel: shopCategory };
    ...
    try {
      const res = await fetch("/api/shop-reviews", {
        ...
        body: JSON.stringify({
          ...
          title: data.title,
          body: data.body,
          author_label: data.authorLabel,
          website: "", // ハニーポット（人間は空のまま送る）
        }),
      });
      ...
      if (!res.ok || !json.ok) {
        ...
        setSendError(
          res.status === 429
            ? "短時間に続けて送信されたようです。少し時間をおいてお試しください。"
        ...
        <div className="max-w-2xl mx-auto px-6 py-10 md:py-14">
          <form onSubmit={handleSubmit} noValidate>
            ...
            {sendError && (
              <p
                role="alert"
                className="text-xs mb-4 rounded-lg px-4 py-3"
                style={{ color: "#B8806E", background: "rgba(212,160,144,.10)" }}
              >
                {sendError}
              </p>
            )}
